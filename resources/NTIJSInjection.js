...
function NTIInlineNoteMakeFromSelectionOrAt( html, x, y )
{
	...
	if( doBefore ) {
		...
		if( parent.nextSibling ) {
			...
			parent.parentElement.insertBefore( newChild, parent );
		}
	}
	else {
		log += " do direct; ";
...
function NTIInlineNoteGetHTMLAt( x, y )
{
	var note = NTIInlineNoteFind( x, y );
	return note ? note.innerHTML : null;
}
...
function NTIInlineNoteFind( x, y )
{
	...
	while( e ) {
		if( e.tagName == 'SPAN' && $(e).hasClass( 'inlinenote' ) ) {
			result = e;
			break;
		}
		e = e.parentNode;
...
{
	var notes = $(".inlinenote" );
	for( var i = 0; i < notes.length; i++ ) {
		notes[i].parentNode.removeChild( notes[i] );
	}
}

function NTIInlineNoteUpdate( html, x, y )
{
	var note = NTIInlineNoteFind( x, y );
	if( note ) {
		note.innerHTML = html;
...
function unHighlightNode(node) {
	if(isTextNode(node)){
		...
		if( e.tagName == 'SPAN' && (e.className.indexOf('highlight') != -1)) {
		...
		if(nodeWithHighlight.className.indexOf('highlight') != -1) {
			nodeWithHighlight.className = nodeWithHighlight.className.replace('highlight', '');
			return true;
		}
		else {
			return false;
		...
		if(node.className.indexOf('highlight') != -1) {
...
{
	return prefix + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
}
...
function NTIPresentAnswers( data, selector )
{
	...
	if( results['Last Modified'] ) {
		var date = modifiedStringWithPrefix( new Date( results['Last Modified'] * 1000 ), "Attempt: " );
	...
	for( var i = 0; i < fields.length; i++ ) {
		...
		var answer = answers[fields[i].id];
		...
		fields[i].value = answer['Response'];
		...
		rightAnswer.className = "mathjax tex2jax_process";
		...
		var color = 'rgb(255, 227, 196)';
		if( !right ) {
			fields[i].style.backgroundColor = color;
			yourResponse.style.backgroundColor = color;
			rightAnswer.style.backgroundColor = color;
		}
...
function NTIShowAnswers( evt, key )
{
	getObjects( username, 'quizresults', key, null, function( data ) {
		...
		NTIPresentAnswers( data["Items"], "input[type]" );

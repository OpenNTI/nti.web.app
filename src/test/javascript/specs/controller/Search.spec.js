describe('Search Controller Tests', function() {
	...
	function createHit(mime) {
		return NextThought.model.Hit.create({
			'ContainerId': 'book1',
			'TargetMimeType': 'application/vnd.nextthought.' + mime,
			'Type': Ext.String.capitalize(mime)
		});
	}
	...
	describe('results loading', function() {
		...
		afterEach(function() {
			...
			var s = Ext.getCmp('search-results');

			//JSG: This fixme is wrong. Menus do get cleaned up, it depends on who owns them. Their parent component is responsible for cleanup.
		...
		describe('Results grouping', function() {
			...
			it('Known groups get shown', function() {
				...
				names.sort();
				expect(names).toEqual(['Books', 'Chats', 'Forums', 'Highlights', 'Notes', 'Thoughts']);
			});

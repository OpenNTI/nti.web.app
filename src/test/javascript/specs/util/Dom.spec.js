describe('Dom.js Tests', function() {
	...
	describe('getVideosFromDom', function() {
		...
		it('One video', function() {
			...
			result = DomUtil.getVideosFromDom(videoContainer)[0];

			for (key in videoObject) {
				if (videoObject.hasOwnProperty(key)) {
					expect(result[key]).toEqual(videoObject[key]);
				}
			}
		});
		...
		it('Three Videos', function() {
			...
			result.forEach(function(r, i) {
				var o = videoObject[i], key;

				for (key in o) {
					if (o.hasOwnProperty(key)) {
						expect(r[key]).toEqual(o[key]);
					}
				}
			});

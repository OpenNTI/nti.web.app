...
class Test_Login(WebAppTestBase):
    ...
    def test_login(self):
        self.login()
        assert_that(self.text_in_tree('title', 'NextThought App'), is_(True))
    
    def test_failed_user_login(self):
        self.login('incorrect_user', 'incorrect_password')
        assert_that(self.text_in_tree('title', 'NextThought Login'), is_(True))
    ...
    def test_failed_password_login(self):
        ...
        assert_that(self.text_in_tree('div', 'Please try again, there was a problem logging in.'), is_(True))

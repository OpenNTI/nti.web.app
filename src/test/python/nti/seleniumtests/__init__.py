...
def values_of_node(node, element, html=None):    
    items = []    
    html = html 
    #or get_page_source()
...
def wait_for_page_to_load (driver = None, timeout = 10):
    for _ in range(timeout):
        
        print 'looking for the loading mask'
        elt = 'loading-mask'
        elts = values_of_node (node = 'div', element='id', html = driver.page_source)
        try: 
            #t = driver.find_element_by_id ('loading-mask')
            if elt not in elts:
                print 'element not there, cool'
                break
...
def wait_for_title (driver = None, title = 'NextThought Login', timeout = 10):

Ext.define('NextThought.providers.Location', {
	...
	requires :[
		'NextThought.Library', 'NextThought.ContentAPIRegistry', 'NextThought.view.video.Window'
	],
	...
	setLocation:function (ntiid, callback, fromHistory) {
		var me = this, e = Ext.getCmp('viewport').getEl(), rootId = this.getLineage(ntiid).last();
	...
	resolvePageInfo:function (ntiid, finish, hasCallback) {
		var me = this, service = $AppConfig.service;
		...
		function failure(q, r) {
			...
			if (r.status === 403 || !r.responseText) {
				...
				alert(r.status === 403 ? 'You don\'t have access to that content.' : 'Oops!\nSomething went wrong.');
	...
	getLocation:function (id) {
		...
		me.timers[i] = setTimeout(function () {
			delete me.cache[i];
		}, 15000);
	...
	getLineage:function (ntiid, justLabels) {
		var leaf = this.find(ntiid || this.currentNTIID) || {}, node = leaf.location, lineage = [], id;
	...
	getSortIndexes:function (ntiid) {
		var noLeaf = {}, leaf = this.find(ntiid || this.currentNTIID) || noLeaf, node = leaf.location, indexes = [], id, i, cn, j, t;
	...
	getContentRoot:function (ntiid) {
		var bookId = LocationProvider.getLineage(ntiid || this.currentNTIID).last(), title = Library.getTitle(bookId);
	...
	getNavigationInfo:function (ntiid) {
		var loc = this.find(ntiid), info = {}, topicOrTocRegex = /topic|toc/i, slice = Array.prototype.slice;
		...
		function isTopicOrToc(node) {
			...
			var result = false, topicOrToc = topicOrTocRegex.test(node.tagName), href = (node.getAttribute) ? node.getAttribute('href') : null;
		...
		function child(n, first) {
			var v, topics = n && n.childNodes ? slice.call(n.childNodes) : [];

			if (first) {
				topics.reverse();
			}

			while (topics.length && !isTopicOrToc(topics.peek())) {
				topics.pop();
			}
			if (n && topics.length) {
				v = topics.peek();
	...
	getRelated:function (givenNtiid) {
		var me = this, ntiid = givenNtiid || me.currentNTIID, map = {}, info = this.find(ntiid), related = info ? info.location.getElementsByTagName('Related') : null;
		...
		Ext.each(related, function (r) {
			...
			do {
				...
				var tag = r.tagName, id = r.getAttribute('ntiid'), type = r.getAttribute('type'), qual = r.getAttribute('qualifier'),

					target = tag === 'page' ? this.find(id) : null, location = target ? target.location : null,

					label = location ? location.getAttribute('label') : r.getAttribute('title'), href = (location || r ).getAttribute('href');
	...
	updatePreferences:function (pi) {
		var sharing = pi.get('sharingPreference'), piId = pi.getId(), rootId = this.getLineage(piId).last();
	...
	getPreferences:function (ntiid) {
		...
		Ext.each(lineage, function (l) {
			return !Boolean(result = this.preferenceMap[l]);
		}, this);
		return result;
	}

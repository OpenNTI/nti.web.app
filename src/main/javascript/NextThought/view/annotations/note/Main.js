Ext.define('NextThought.view.annotations.note.Main', {
		...
		requires:[
			'NextThought.cache.UserRepository', 'NextThought.view.annotations.note.EditorActions', 'NextThought.view.annotations.note.Templates'
		],
		...
		afterRender:function () {
			...
			try {
				...
				me.mon(this.liked, 'click', function () {
					me.getRecord().like(me.liked);
				}, this);
				me.mon(this.favorites, 'click', function () {
					me.getRecord().favorite(me.favorites);
				}, this);
		...
		setContent:function (text) {
			...
			Ext.each(this.text.query('.whiteboard-thumbnail'), function (wb) {
		...
		loadReplies:function (record) {
			var me = this, store = NextThought.store.PageItem.create(), responses = me.up('window').down('note-responses');
		...
		recordChanged:Ext.Function.createBuffered(function () {
			this.updateFromRecord();
		}, 100),
		recordUpdated:Ext.Function.createBuffered(function () {
			this.updateFromRecord();
		}, 100),
		...
		editorSaved:function () {
			var v = this.editorActions.getValue(), me = this, r = me.record, re = /(&nbsp;)|(<br>)|(<div>)|(<\/div>)*/g;
			...
			if (this.editMode) {
				...
				r.save({callback:function (record, request) {
					var success = request.success, rec = success ? record : null;
					if (success) {
						r.fireEvent('changed');
					}
		...
		this.prototype.renderTpl = Ext.DomHelper.markup([
...

			{
				cls:'respond',
				cn :[
					TemplatesForNotes.getReplyOptions(), TemplatesForNotes.getEditorTpl()
				]
			}
		]);
	});

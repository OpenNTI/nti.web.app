Ext.define('NextThought.view.annotations.note.Carousel', {
	...
	renderTpl:Ext.DomHelper.markup([
		...
		{ cls   :'slide-nav backward', cn:[
			{cls:'slide backward'}
		], title:'Slide left' },
		{ cls   :'slide-nav forward', cn:[
			{cls:'slide forward'}
		], title:'Slide right'}
	]),
	...
	syncIt:function () {
		var s = this.getEl() ? this.getSize() : null, o = this.navContainer;
		if (!s) {
	...
	filterChanged:function (filter, value) {
		...
		Ext.defer(function () {
			...
			} else if (this.items.findBy(function (o) {
				return o.record === rec;
			}, null) >= 1) {
				this.setRecord(rec);
			} else {
	...
	setRecord:function (rec) {
		var me = this, myWindow = me.up('window');
	...
	afterRender:function () {
		var me = this, o = me.selected || null;
	...
	updateBigArrows:function (item) {
		var hasNext, hasPrev, me = this, editorActive = this.up('window').editorActive();
	...
	updateSlide:function (pos) {
		var dom = this.body.dom, count = this.items.getCount(), canSlideLeft, canSlideRight, value, min, newBgx;//, shift;
	...
	centerBackgroundOn:function (item) {
		var ir = item ? item.getEl().dom.getBoundingClientRect() : {left:0, width:0},
			...
			cr = this.getEl().dom.getBoundingClientRect(), cm = Math.round(cr.left + (cr.width / 2)), im = Math.round(ir.left + (ir.width / 2)),

			start = (cr.width - this.BACKGROUND_WIDTH) / 2, offset = im - cm,
	...
	moveSelection:function (dir, el) {
		...
		var newSel, sel = this.down('note-carousel-item[selected]'), fn = !sel ? null : dir === this.NEXT ? sel.next : dir === this.PREV ? sel.prev : null;

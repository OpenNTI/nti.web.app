Ext.define('NextThought.view.annotations.Base', {
	...
	requires:[
		'NextThought.view.annotations.renderer.Manager', 'Ext.String'
	],
	...
	constructor:function (config) {
		var me = this, c = config.reader, r = config.record, container = c.body.dom;
		...
		if (typeof r.data.sharedWith !== 'undefined') {
			try {
				...
				console.warn('attempted to setup dragging on ', r.$className, Globals.getError(e));
			}
		}
	...
	cleanup:function () {
		...
		var me = this, r = me.record, id = r.getId(), c = me.ownerCmp;
	...
	render:function () {
		console.warn(Ext.String.format('{0} does not implement render()', this.$className));
	},
	...
	attachEvent:function (event, dom, fn, scope) {
		...
		Ext.each(event, function (event) {
			...
			if (dom.addEventListener) {
				dom.addEventListener(event, f, false);
			} else if (dom.attachEvent) {
				dom.attachEvent(event, f);
			}
		});
	},

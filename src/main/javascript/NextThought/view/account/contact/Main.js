Ext.define('NextThought.view.account.contact.Main', {
	...
	items: [
		...
		{xtype: 'container', cls: 'submit', layout: {type: 'hbox', pack: 'end'}, items: [
						{xtype: 'button', ui: 'secondary', scale: 'large', name: 'cancel', text: getString('NextThought.view.account.contact.Main.cancel'), handler: function(b) {
				b.up('window').close();
						}},
						{xtype: 'button', ui: 'primary', scale: 'large', name: 'submit', text: getString('NextThought.view.account.contact.Main.submit')}
		]}
	],

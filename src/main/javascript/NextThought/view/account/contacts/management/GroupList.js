Ext.define('NextThought.view.account.contacts.management.GroupList', {
	...
	refresh:function () {
		var el = this.getEl(), ul, selection = this.getSelectionModel(), blocked = this.blocked;
	...
	block:function (username) {
		this.blocked = Ext.Array.merge(this.blocked || [], Ext.isArray(username) ? username : [username]);
		this.refresh();
	},
	...
	getInnerTpl:function (displayField) {
		...
		return ['<div class="name">', '<tpl if="realname == \'\'">{Username}' + '<tpl else>', '{realname}', '</tpl>', '</div>'
		].join('');
	},

Ext.define('NextThought.assessment.Main', {
			...
			function pageInfoSuccess(pageInfo) {
				...
				for (;i < questions.length; i++) {
					...
					for (;j < questions.length; j++) {
						if (qdiv.parentNode.getAttribute('data-ntiid') == question.data.NTIID) {
							break;
						}
					}
					...
					for (j = 0; j < question.data.parts.length; j++) {
						...
						//A stopgap to test the matching questions
	...
	showAssessmentResult: function(doc, result, questionId) {
		...
		for (i = 0; i < parts.length; i++) {
			...
			pt = parts[i];
			//w.$('#'+pte.dom.id+' .answer-text').remove();
			...
			//w.$('#'+pte.dom.id+' .input').attr('value','');
			if (pte.down('input')) {
				Ext.getDom(pte.down('input')).value = '';
			}
				
			prefix = pte.dom.className.indexOf('freeresponsepart') >= 0

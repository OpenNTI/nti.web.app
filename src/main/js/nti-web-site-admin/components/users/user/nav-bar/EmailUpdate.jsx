...
export default function UserEmailUpdate ({user}) {
	...
	const onSubmit = React.useCallback(({json}) => user.putToLink(Link, {email: json.email}), [user]);

	if (!user.hasLink(Link)) { return null; }

	return (
		<EmailUpdateCard>
			<EmailUpdateButton onClick={doOpen}>{t('trigger')}</EmailUpdateButton>
			{open && (
				<Form.Prompt onSubmit={onSubmit} afterSubmit={doClose} onCancel={doClose} actionLabel="Submit">
					<EmailLabel user={user} localeKey={getUsername} />
					<Form.Input.Text layout="box" name="oldEmail" label={t('oldEmail')} value={user.email} readOnly />
					<Form.Input.Text layout="box" name="email" label={t('newEmail')} />
				</Form.Prompt>
			)}
		</EmailUpdateCard>
	);

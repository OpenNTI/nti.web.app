...
const t = scoped('nti-web-site-admin.components.users.list.table.controls.Activation', {
...
function UserActivationConfirmation ({users, deactivating, activating, onConfirm, onCancel}) {
	...
	const body = (
		<div className="activation-confirmation-body">
			...
			<Text.Label className={Styles.label} as="div" color={Text.Colors.tertiaryGrey}>{getString('label')}</Text.Label>
...
function UserActivationButton ({selectedUsers, canDeactivateUsers, canActivateUsers, deactivateUsers, activateUsers}) {
	const [confirming, setConfirming] = React.useState(false);

	const {activated, deactivated} = (selectedUsers ?? []).reduce((acc, user) => {
		if (user.Deactivated) {
	...
	const available = (activated.length && canDeactivateUsers) || (deactivated.length && canActivateUsers);
	...
	if (mixed || !available) { return null; }

	return (
		<>
			<Button className={cx('button', 'activate-button', {activating, deactivating})} plain onClick={() => setConfirming(true)}>

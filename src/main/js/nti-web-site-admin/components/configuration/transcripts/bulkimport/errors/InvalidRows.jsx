...
const t = scoped(
	'web-site-admin.components.advanced.transcripts.bulkimport.error.InvalidRowsError',
	{
		message: {
...
export default class InvalidRowsError extends React.Component {
	static handles = error =>
		error && error.code && error.code === 'InvalidRowsError';
	...
	render() {
		...
		const footer =
			rows.length > MAX_ROWS ? (
				<div className="more">
					{t('more', { number: rows.length - MAX_ROWS })}
				</div>
			) : null;

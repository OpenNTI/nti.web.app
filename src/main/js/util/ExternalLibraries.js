...
(function(global) {
	var prefix = global.testRoot !== undefined ? '' : '/app/',
		script;
		...
		Ext.each(libs, function(o) {
			Ext.Object.each(o, function waiting(name, v) {
				...
				Ext.Loader.loadScript({
					url: url,
					onError: function() {maybeFinish(name); console.error(name + ' failed to load');},
					onLoad: function() { maybeFinish(name); Ext.callback(cb); /*console.debug(name+' finshed loading');*/ }
				});
			});
		});
	...
	if (!global.libsLoaded) {
		if (global.testRoot !== undefined) {
			script = global.testRoot + 'libs.js';
		} else {
			script = prefix + 'js/libs.js';
		}

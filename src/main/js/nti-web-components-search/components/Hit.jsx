...
export default class Hit extends React.Component {
	...
	constructor(props) {
		...
		if (this.props.resolvePath) {
			Promise.resolve(this.props.resolvePath()).then(p => {
	...
	navigate(hit, fragments, navigateToSearchHit) {
		resolveNavigateToSearchHit(hit, fragments[0]).then(
			({ obj, fragIndex, containerId }) => {
	...
	render() {
		...
		let clickHandler = () => {
			this.navigate(hit, fragments, navigateToSearchHit);
		};

		return (
			<div
				className="search-result-react"
				data-target-mimetype={hit.TargetMimeType}
			>
				<div className="hit-title" onClick={clickHandler}>
					{this.props.title}
				</div>
				<Fragments
					fragments={this.props.fragments}
					hit={this.props.hit}
					navigateToSearchHit={this.props.navigateToSearchHit}
				/>

...
describe ('<Pager />', () => {
	...
	test ('should render a `.pagination-container`', () => {
		...
		expect(container.querySelectorAll('.pagination-container').length).toBe(1);
	});

	test ('should render at least one `.pagination-item`', () => {
		const {container} = render(<Pager pagesToShow={pagesToShow} currentPage={currentPage} showNext={showNext} loadPage={loadPage} showMoreButton={showMoreButton}/>);
		expect(container.querySelectorAll('.pagination-item').length).toBe(12);
	});

	test ('should render a `.next-results-page-button`', () => {
		const {container} = render(<Pager pagesToShow={pagesToShow} currentPage={currentPage} showNext={showNext} loadPage={loadPage} showMoreButton={showMoreButton}/>);
		expect(container.querySelectorAll('.next-results-page-button').length).toBe(1);
	});
	...
	test ('simulates clicks on a page number', async () => {
		const {findByText} = render(<Pager pagesToShow={pagesToShow} currentPage={currentPage} showNext={showNext} loadPage={loadPage} showMoreButton={showMoreButton}/>);

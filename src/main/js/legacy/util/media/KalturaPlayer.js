	...
	exports =
		Ext.define('NextThought.util.media.KalturaPlayer', {
					...
					cn: [
						{
							...
							cn: [
								...
								},
								{
									...
									src:
										'{scheme}//cdnapisec.kaltura.com/p/{partnerid}' +
			...
			constructor(config) {
				...
				this.height =
					config.height +
					((config.reserveControlSpace && this.CONTROL_HEIGHT) || 0);
			...
			playerSetup: function () {
				...
				function stopTask(msg) {
					console[Ext.isEmpty(msg) ? 'log' : 'warn'](
						me.id,
						' Stopping setup task ',
						msg || ''
					);
				...
				this.el = this.PLAYER_TPL.append(this.parentEl, data, true);
			...
			handleMessage: function (event) {
				...
				if (!Ext.isFunction(this[handlerName])) {
					console.warn(
						this.id,
						' Player does not handle: ' + handlerName,
						eventData
					);
					return;
				}
			...
			sendMessage: function (type, name, data) {
				...
				console.log(
					this.id +
						' Posting message to kaltura player(' +
						context.playerId +
						')',
					type,
					name,
				...
				context.postMessage(
					Ext.encode({ type: type, name: name, data: data }),
					'*'
				);
			},
			...
			sendCommand: function (name, data, force) {
				var buffer =
					!force && !Ext.Array.contains(this.neverQueue, name);
				if (this.changingMediaSource && buffer) {
					console.log(
						'Enqueing command ',
						name,
						' because we are chaining sources and it wasnt forced'
					);
					this.commandQueue.push(['command', name, data]);
					return;
				}
				console.debug(
					this.id + ': Invoking command ',
					name,
					'with',
					data
				);
			...
			getPlayerContext: function () {
				...
				return (
					iframe &&
					(iframe.contentWindow || window.frames[iframe.name])
				);
			},
			...
			load: function (src, offset, force) {
				...
				var source = src,
					kalturaData,
					me = this,
					sourceActuallyChanging = source !== this.currentSource;
				...
				if (this.changingMediaSource && !force) {
					console.log(
						'Load in process so defering new load of source',
						arguments
					);
				...
				if (!sourceActuallyChanging && !force) {
					console.log(
						'Short circuiting load because currentSource is already source',
						source,
						this.currentSource
					);
				...
				this.changeMediaTimeout = setTimeout(function () {
					var args;
					console.error(
						'Change media timed out',
						me.changeMediaAttempt,
						me.maxChangeMediaAttempts,
						me.bufferedLoad
					);
					if (me.changeMediaAttempt <= me.maxChangeMediaAttempts) {
						if (!me.maybeDoBufferedLoad(true)) {
							...
							Ext.defer(
								me.load,
								me.changeMediaAttemptIntervalMillis,
								me,
								args
							);
						} else {
						...
						console.error(
							'Media still didnt change after',
							me.maxChangeMediaAttempts,
							'attempts'
						);
			...
			deactivate: function () {
				if (this.playerDeactivated) {
					console.warn(
						'Attempting to deactivate a deactivated player'
					);
					return;
				}
			...
			activate: function (sourceId) {
				...
				if (!this.el && !this.settingUp) {
					...
					this.playerSetup();
					this.onReadyLoadSource =
						sourceId || (this.currentLoadAttempt || [])[0];
				} else {
					console.log(
						this.id,
						'Not doing player setup, but making player ready',
						this.el,
						this.settingUp
					);
					this.readyHandler();
			...
			playerStateChangeHandler: function (event) {
				...
				if (!this.el) {
					...
					console.warn(
						'Still got events for a Video that is not visible: ' +
							state
					);
					if (state === 'playing') {
						console.warn(
							'Stopping hidden player and short circuit state change handler.'
						);
						this.stop();
						return;
					}
			...
			makeNotReady: function () {
				console.log(
					this.id,
					' KALTURA player is making self not ready'
				);
				this.isReady = false;
			},

			readyHandler: (function () {
				return Ext.Function.createBuffered(
					function () {
			...
			doPlayHandler: function () {
				...
				console.warn(
					this.id,
					' kaltura fired play handler called',
					this.currentState,
					arguments
				);
			...
			onPlaybackError: function () {
				...
				Error.raiseForReport({
					msg: Ext.String.format(
						'Kaltura Playback Error: PartnerID: {0}, UIConf: {1}, LEAD_HTML5: {3}, Mode: {4}, Current Source: {2}',
						me.PARTNER_ID,
						me.UICONF_ID,
						me.currentSource,
						me.LEAD_HTML5,
				...
				inject: function () {
					...
					function makeHandler(name) {
						...
						window[newName] = function (/*id*/) {
							...
							if (/error/i.test(name) && seen[name] < 2) {
								console.warn(
									window.playerId +
										' Ignoring intial errors about no source',
									name,
									arguments
								);
								return;
					...
					function playerReady() {
						...
						for (i; i >= 0; i--) {
							player.addJsListener(
								events[i],
								makeHandler(events[i])
							);
						}

						window.playerMode =
							player.tagName === 'OBJECT' ? 'Flash' : 'HTML5';
						console.log(
							window.playerId,
							'Player is ',
							window.playerMode
					...
					function handleMessage(event) {
						...
						try {
							...
							if (
								eventData.name === 'doStop' &&
								state &&
								state !== 'playing'
							) {
								console.warn(
									'Skipping doStop commmand because player is not playing: ' +
										state
								);
								return;
							}
					...
					mw.setConfig(
						...
						true
					); //Enables HTML5 controls with native full screen

...
const KalturaPlayer = (module.exports = exports = Ext.define(
	'NextThought.util.media.KalturaPlayer',
	{
		...
		getPlayerContext: function () {
			...
			return (
				iframe && (iframe.contentWindow || window.frames[iframe.name])
			);
		},
		...
		playerStateChangeHandler: function (event) {
			...
			if (!this.el) {
				...
				console.warn(
					'Still got events for a Video that is not visible: ' + state
				);
				if (state === 'playing') {
					console.warn(

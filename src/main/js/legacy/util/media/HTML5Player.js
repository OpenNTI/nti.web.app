...
module.exports = exports = Ext.define('NextThought.util.media.HTML5Player', {
	...
	playerSetup: function () {
		...
		this.playerTpl.append(this.parentEl, {
			id: this.id,
			height: this.height,
			width: this.width,
		});
	...
	load: function (source, offset, size, captions) {
		const trackTpl = Ext.DomHelper.createTemplate({
			tag: 'track',
			src: '{src}',
			srclang: '{srclng}',
			label: '{label}',
		...
		var sourceTpl = Ext.DomHelper.createTemplate({
				tag: 'source',
				src: '{src}',
				type: '{type}',
			}),
		...
		for (let caption of captions) {
			trackTpl.append(player, {
				src: caption.src,
				srclang: caption.lang,
				kind: caption.purpose,
				label: `Captions:${caption.lang}`,
			});
		}

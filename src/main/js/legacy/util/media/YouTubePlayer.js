...
module.exports = exports = Ext.define(
	'NextThought.util.media.YouTubePlayer',
	{
		...
		playerError: function (error) {
			...
			if (error.data === 5) {
				this.isReady = false;
				console.warn(
					'There was an issue with the YouTube HTML5 player. Cleaning-up and trying again.'
				);
				this.cleanup();
				this.playerSetup();
				oldSource = this.currentSource;
				this.currentSource = null;
				this.parent.issueCommand('youtube', 'load', [
					oldSource,
					this.currentStartAt,
					'medium',
				]);
				...
				console.warn(
					'The YouTube Player experienced an unrecoverable error.'
				);
		...
		play: function () {
			if (this.player && this.player.playVideo) {
				if (Ext.is.iOS && !this.userActivatedPlayer) {
					//we have to wait for the player to have been touched, but after that, we can interact with it just as normal.
					return;
				}
		...
		seek: function (offset, seekAhead) {
			...
			if (duration === 0) {
				this.on({
					single: true,
					'player-event-play': this.seek.bind(
						this,
						offset,
						seekAhead
					),
	...
	function () {
		...
		window.onYouTubeIframeAPIReady = Ext.Function.createSequence(
			onReady,

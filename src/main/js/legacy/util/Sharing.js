...
module.exports = exports = Ext.define('NextThought.util.Sharing', {
	shareWith: function (sharable, people, callback) {
		var sharedTo = Ext.Array.merge(
			sharable.get('sharedWith') || [],
			people
		);
	...
	resolveValue: function (value) {
		...
		Ext.each(result, function (r, i, a) {
			if (lazy.ParseUtils.isNTIID(r)) {
				a[i] =
					UserRepository.getStore().findRecord(
						'NTIID',
						r,
						0,
						false,
	...
	sharedWithToSharedInfo: function (sharedWith, scopeProvider) {
		...
		if (isPublic) {
			Ext.each(sharedWith, function (i) {
				if (
					!Ext.Array.contains(publicEntities, i.getId ? i.getId() : i)
				) {
					list.push(i);
				}
			});
	...
	getLongTextFromShareInfo: function (shareInfo, tpl, maxLength) {
		...
		return UserRepository.getUser(explicitEntities).then(function (
			resolvedUsers
		) {
			...
			if (tpl) {
				names = Ext.Array.map(names, function (name, idx) {
					var u = nameMap[idx],
						tplArgs = [
							name,
							u && u.getProfileUrl && u.getProfileUrl()
								? idx
								: '',
						];
			...
			if (others) {
				suffix = Ext.String.format(
					'and {0}',
					Ext.util.Format.plural(others, 'other')
				);
	...
	getShortTextFromShareInfo: function (shareInfo) {
		...
		return UserRepository.getUser(explicitEntities.first()).then(function (
			resolved
		) {
	...
	getLongSharingDisplayText: function (
		shareWith,
		callback,
		scope,
		tpl,
		...
		return this.getLongTextFromShareInfo(shareInfo, tpl, maxLength).then(
			function (str) {
				if (callback) {
	...
	getShortSharingDisplayText: function (shareWith, callback, scope) {
		...
		return this.getShortTextFromShareInfo(shareInfo).then(function (str) {
			if (callback) {
				callback.call(scope, str);
			}
	...
	getTagSharingLongText: function (
		sharedWith,
		tags,
		published,
		callback,
	...
	getTagSharingShortText: function (
		sharedWith,
		tags,
		published,
		callback,

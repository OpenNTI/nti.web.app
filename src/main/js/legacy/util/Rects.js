...
module.exports = exports = Ext.define('NextThought.util.Rects', {
	...
	trimCrazies: function (rects, lineHeight, clientWidth) {
		...
		function isCovered(i) {
			...
			for (; j < rects.length; j++) {
				if (
					rects[j].top > rects[i].top &&
					rects[j].bottom < rects[i].bottom
				) {
					return true;
				}
			}
		...
		for (; i >= 0; i--) {
			...
			if (o.height && o.height < lineHeight) {
				o.height = lineHeight;
			} //round up to look nice
			h = o.height;
			w = o.width;
			if (
				w > 0 &&
				(w <= clientWidth || !clientWidth) &&
				notTooShort(h) &&
				(notTooTall(h) || !isCovered(i))
			) {
				out.push(o);
			}
		}
	...
	contains: function (refRect, testRect, allowances) {
		...
		if (!Ext.isObject(a)) {
			Ext.Error.raise('Invalid allowances value');
		}

		Ext.applyIf(a, { top: 0, left: 0, right: 0, bottom: 0 });

		return (
			refRect.top + a.top <= testRect.top &&

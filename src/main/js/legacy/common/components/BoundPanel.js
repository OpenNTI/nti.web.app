...
module.exports = exports = Ext.define(
	'NextThought.common.components.BoundPanel',
	{
		...
		getComponentConfigForRecord: function (rec) {
			...
			return {
				record: rec,
				recordId: id && lazy.ParseUtils.escapeId(id),
			};
		},
		...
		shouldHide: function (records) {
			...
			Ext.each(records, function (item) {
				allHidden =
					allHidden &&
					!!(item.hidden || (me.filter && !me.filter(item)));
			});
		...
		doRefresh: function (store) {
			...
			this.refreshTask = Ext.defer(this.onBoundStoreLoad, 100, this, [
				store,
			]);
		},

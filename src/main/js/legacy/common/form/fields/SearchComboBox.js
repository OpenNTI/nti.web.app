...
module.exports = exports = Ext.define(
	'NextThought.common.form.fields.SearchComboBox',
	{
		...
		alignOptions: function () {
			...
			this.scrollListener = this.mon(
				this.scrollParent || Ext.getBody(),
				'scroll',
				this.__align
			);
			this.__align();
		...
		__fillInPreview: function (option) {
			...
			if (
				preview &&
				preview.toLowerCase().indexOf(value.toLowerCase()) >= 0
			) {
		...
		__selectSibling: function (options, direction) {
			...
			if (item) {
				...
				this.__fillInPreview(
					this.options[item.getAttribute('data-index')]
				);
			} else {
				this.__fillInPreview();
			}
		...
		onInputKeyDown: function (e) {
			...
			if (charCode === e.DOWN) {
				...
				exactMatch &&
				autoComplete &&
				unfilteredOptions.length === 1
			) {
				this.__selectOption(exactMatch);
			} else if (
				(charCode === e.ENTER || charCode === e.RIGHT) &&
				unfilteredOptions.length > 0 &&
				open
			) {
		...
		onInputKeyUp: function () {
			...
			hasSelected = unfilteredOptions.reduce(function (acc, option) {
				...
				if (active) {
					me.__fillInPreview(
						me.options[option.getAttribute('data-index')]
					);
				}
				return active || acc;
			}, false);
			...
			if (!hasSelected && unfilteredOptions[0]) {
				...
				me.__fillInPreview(
					me.options[unfilteredOptions[0].getAttribute('data-index')]
				);
			} else if (!hasSelected) {
				me.__fillInPreview();
			}
		...
		onInputBlur: function () {
			...
			wait(100).then(this.hideOptions.bind(this));
		},

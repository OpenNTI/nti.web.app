...
module.exports = exports = Ext.define(
	'NextThought.common.form.fields.LegacySearchComboBox',
	{
		...
		showOptions: function () {
			...
			this.optionsEl.el.dom.style.maxHeight =
				Ext.Element.getViewportHeight() - top - 10 + 'px';
		},
		...
		filterOptions: function (value, show) {
			...
			if (options.length === 1) {
				...
				if (
					value &&
					value.toLowerCase() === options[0].text.toLowerCase()
				) {
			...
			if (this.activeValue) {
				active = this.optionsEl.down(
					'li[data-value="' + this.activeValue + '"]'
				);
			} else {
		...
		selectNextMatch: function (e) {
			...
			if (/[A-Z]/i.test(ch)) {
				if (
					me.__inputBuffer.charAt(me.__inputBuffer.length - 1) !== ch
				) {
					me.__inputBuffer += ch;
				}
		...
		inputKeyDown: function (e) {
			...
			if (charCode === e.DOWN) {
				if (open) {
					next =
						current.dom.nextSibling ||
						current.dom.parentNode.firstChild;
				} else {
					this.showOptions();
				}
			}
			...
			if (charCode === e.UP) {
				next =
					current.dom.previousSibling ||
					current.dom.parentNode.lastChild;
			}
		...
		inputBlur: function () {
			const value = (this.inputEl.getValue() || '').toLowerCase(),
				...
				isValid =
					this.options.filter(o => o.text.toLowerCase() === value)
						.length === 1;
			...
			this.__hideOptionsTimer = setTimeout(

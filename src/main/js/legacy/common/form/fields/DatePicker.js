...
module.exports = exports = Ext.define(
	'NextThought.common.form.fields.DatePicker',
	{
		...
		afterRender: function () {
			...
			if (this.hourEl) {
				this.hourEl.dom.addEventListener(
					'blur',
				...
				this.hourEl.dom.addEventListener('mousewheel', function (e) {
					e.preventDefault();
				});
				this.hourEl.dom.addEventListener(
					'keydown',
			...
			if (this.minuteEl) {
				this.minuteEl.dom.addEventListener(
					'blur',
					this.onDateChange.bind(this)
				);
				this.minuteEl.dom.addEventListener(
				...
				this.minuteEl.dom.addEventListener('mousewheel', function (e) {
					e.preventDefault();
				});
			}
		...
		handleSelect: function (datepicker, value) {
			var newlySelectedEl = datepicker.el.down(
				'.' + datepicker.selectedCls
			);
		...
		clearSelectedDate: function () {
			var currentClickedEl =
				this.datepicker &&
				this.datepicker.el.down('.' + this.clickedCls);
		...
		getDisplayValue: function () {
			var p = this.datepicker,
				...
				meridiemVal =
					this.meridiemEl &&
					this.meridiemEl.dom.getAttribute('data-value'),
				date;
		...
		getErrors: function () {
			...
			if (!this.selectedDate) {
				errors.push({
					name: 'date',
					error: 'Please select a date above.',
				});
			}
			...
			for (let i = 0; i < fields.length; i++) {
				...
				if (val > max || val < min) {
					errors.push({
						name: name,
						error: 'The value for ' + name + ' is out of range',
					});
				}
			}

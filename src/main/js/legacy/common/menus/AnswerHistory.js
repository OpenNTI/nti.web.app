...
module.exports = exports = Ext.define(
	'NextThought.common.menus.AnswerHistory',
	{
		...
		items: [
			{
				text: getString('NextThought.view.menus.AnswerHistory.answer'),
				cls: 'answer-title',
				allowUncheck: false,
				answerHistoryTitle: true,
			},
			{
				text: getString('NextThought.view.menus.AnswerHistory.loading'),
				allowUncheck: false,
				noAnswerHistory: true,
			},
		],
		...
		reload: function () {
			...
			if (items.length === 1) {
				items.push({
					text: getString(
						'NextThought.view.menus.AnswerHistory.noanswer'
					),
					cls: 'no-answer-history',
					allowUncheck: false,
					noAnswerHistory: true,
				});
			}
		...
		handleClick: function (item) {
			if (
				!item.is('[answerHistoryTitle]') &&
				!item.is('[noAnswerHistory]')
			) {

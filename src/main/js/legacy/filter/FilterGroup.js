...
module.exports = exports = Ext.define('NextThought.filter.FilterGroup', {
	...
	constructor: function (scope, operation, filteres) {
		var me = this;
		me.callParent(null);
		me.operation = this.clamp(
			[this.self.OPERATION_INTERSECTION, this.self.OPERATION_UNION],
			operation
		);
		me.value = [];
		me.scope = scope;
	...
	addFilter: function (filter) {
		if (
			...
			(filter.isFilterGroup || !this.contains(filter))
		) {

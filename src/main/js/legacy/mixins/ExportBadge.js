...
module.exports = exports = Ext.define('NextThought.mixins.ExportBadge', {
	...
	exportToBackPack: function (record, targetEl) {
		...
		return record
			...
			.then(function () {
				return record
					.pushToMozillaBackpack()
					.then(function (successes) {
						console.log(
							'Congratulations, your badge was sent to backpack: ' +
								successes
						);
					})
	...
	askForEmailVerification: function (nextActionName, record, targetEl) {
		...
		this.emailVerificationPrompt = this.PromptActions.prompt(
			'badge-exporting',
			{
				...
				subTitle:
					'Verifying your email allows you to export your completion badge',
	...
	askForEmailLock: function (nextActionName, record, targetEl) {
		...
		this.emailVerificationPrompt = this.PromptActions.prompt(
			'badge-exporting',
			{
				...
				badge: record,
				title: getString(
					'NextThought.mixins.ExportBadge.LockEmail.Title'
				),
				subTitle: getString(
					'NextThought.mixins.ExportBadge.LockEmail.SubTitle'
				),
				saveText: action.buttonTitle,
			}
		).then(function () {

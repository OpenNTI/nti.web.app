...
const logger = Logger.get(
	'nextthought:extjs:mixins:note-feature:GetLatestReply'
);
			...
			function cb(store, records) {
				...
				store.each(function (r) {
					if (r.getId() === (focusRecord && focusRecord.getId())) {
						...
						(!rec ||
							rec.get('CreatedTime') < r.get('CreatedTime')) &&
						r.get('inReplyTo') === me.record.getId()
					) {
						rec = r;
					}
				});

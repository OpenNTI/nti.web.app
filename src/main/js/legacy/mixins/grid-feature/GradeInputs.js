	...
	'NextThought.mixins.grid-feature.GradeInputs',
	{
		...
		monitorSubTree: function () {
			var MutationObserver =
				...
				observer,
				view = this.__getGridView();

			if (!MutationObserver) {
				alert(
					getString(
						'NextThought.view.courseware.assessment.admin.Grid.oldbrowser'
					)
				);
				return false;
			}
			...
			observer.observe(Ext.getDom(this.__getGridView().getEl()), {
				childList: true,
				subtree: true,
			});
		...
		bindInputs: function () {
			...
			var mons = {
				...
				inputs = this.__getGridView()
					.getEl()
					.select(this.__inputSelector);
		...
		getEditRecordFor(record) {
			...
			if (!this.editFunctions[id]) {
				this.editFunctions[id] = Ext.Function.createBuffered(
					this.editGrade.bind(this),
					100
				);
			}

...
module.exports = exports = Ext.define(
	'NextThought.mixins.PresentationResources',
	{
		...
		getPresentationResource() {
			const resources =
				(this.get && this.get('PlatformPresentationResources')) || [];
		...
		getAssetRoot: function () {
			...
			this.presentationroot = rootPath
				? getURL(rootPath)
				: this.getDefaultAssetRoot();
		...
		getImgAsset: function (name) {
			var assetPath = this.ASSET_MAP[name] || {
					name: 'missing-' + name + '-asset.png',
				},
			...
			if (Ext.isEmpty(root)) {
				...
				p = Service.request({
					...
					.catch(function () {
						...
						return (
							'/app/resources/images/default-course/' +
							assetPath.name
						);
					});
			} else {
		...
		__ensureAsset: function (key, asset) {
			...
			if (!existing) {
				existing = this.getImgAsset(asset || key).then(function (url) {
					me.set(key, url);
					return url;
				}, me.set.bind(me, [key, null]));

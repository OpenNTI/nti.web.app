		...
		Ext.define('NextThought.mixins.Avatar', {
			statics: {
				...
				getDefaultBackgroundForUsername: function (username) {
					...
					return (
						'/app/resources/images/profile-backgrounds/profile_bg_' +
						idx +
						'.jpg'
					);
				},
				...
				getBackgroundColorForUsername: function (username) {
					var hash = this.getUsernameHash(username),
						idx =
							Math.abs(hash) %
							this.DEFAULT_AVATAR_BG_COLORS.length;
			...
			initAvatar: async function () {
				...
				const [avatarURL, avatarInitials, avatarBGColor] =
					await Promise.all([
						this.__getAvatar(),
						this.__getInitials(),
						this.__getBGColor(),
			...
			__getBGColor: function () {
				return Avatar.getBackgroundColorForUsername(
					this.get('Username')
				);
			},
			...
			getBackgroundImage: function () {
				...
				return Promise.resolve(
					Avatar.getDefaultBackgroundForUsername(username)
				);
			},
		}));

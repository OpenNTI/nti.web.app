...
module.exports = exports = Ext.define('NextThought.mixins.ModelWithPublish', {
	...
	publish: function (widget, cb, scope) {
		...
		Ext.callback(widget.markAsPublished, widget, [
			'publish',
			!currentValue,
		]);
	...
	getSharingInfo: function () {
		var sharingInfo,
			entities = Ext.Array.filter(
				this.get('headline').get('tags'),
				function (t) {
		...
		if (this.isExplicit()) {
			sharingInfo = SharingUtils.tagShareToSharedInfo(
				this.get('sharedWith'),
				entities
			);
		} else {
			sharingInfo = {
				publicToggleOn: this.isPublished(),
				entities: entities,
			};
		}
	...
	doPublish(data) {
		...
		return Service.post(link, data).then(response => {

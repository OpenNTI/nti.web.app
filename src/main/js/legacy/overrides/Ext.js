...
module.exports = exports = Ext.define(
	'NextThought.overrides.Ext',
	{
		...
		applyRecursively: function applyRecursively(o, cfg) {
			...
			if (Ext.isArray(o)) {
				Ext.each(o, function (v, i, a) {
					a[i] = applyRecursively(v, cfg);
				});
			} else if (Ext.isObject(o)) {
				Ext.Object.each(o, function (k, v) {
					o[k] = applyRecursively(v, cfg);
				});
				o = Ext.apply(o, cfg);
			}
		...
		Ext.getElementById = function (id) {
			...
			function testFrame(frame) {
				var win = frame.contentWindow || window.frames[frame.name],
					src;
				try {
					src = Globals.HOST_PREFIX_PATTERN.exec(
						frame.getAttribute('src')
					);
					if (
						src &&
						(src[3] === document.domain || src[1] === 'javascript')
					) {
					...
					if (EC.hasOwnProperty(eid)) {
						...
						try {
							clean =
								!d ||
								!d.parentNode ||
								(!d.offsetParent && !Ext.getElementById(eid));
							if (d.id !== eid) {

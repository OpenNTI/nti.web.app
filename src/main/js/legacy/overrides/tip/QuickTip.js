...
module.exports = exports = Ext.define('NextThought.overrides.tip.QuickTip', {
	...
	getTargetXY: function getTargetXY() {
		...
		this.anchorTarget = this.activeTarget
			? Ext.getDom(getTarget(this.activeTarget.el))
			: this.anchorTarget;
		this.anchor = 'bottom';
	...
	showAt: function (xy) {
		...
		if (
			size.width !== sizeAfter.width ||
			size.height !== sizeAfter.height
		) {

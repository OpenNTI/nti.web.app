...
module.exports = exports = Ext.define('NextThought.overrides.app.Application', {
	...
	registerInitializeTask: function (task) {
		...
		method = method.$owner
			? method.$owner.$className + '.' + method.$name
			: method.name;

		if (!task) {
			console.error(
				method + ' called registerInitializeTask without a token'
			);
			return;
		}
		...
		task.timerId = setTimeout(function () {
			console.debug(
				'Abandoned init task from: ' + method,
				task.name || task
			);
	...
	finishInitializeTask: function (task) {
		...
		if (!task) {
			console.error(
				method + ' called finishInitializeTask without a token'
			);
			return;
		}
		if (task.expired) {
		...
		if (!this.initTasks.length) {
			this.registerInitializeTask = this.finishInitializeTask =
				Ext.emptyFn;

	...
	'NextThought.model.store.StripePricedPurchasable',
	{
		...
		calculatePurchaseDiscount: function () {
			...
			if (!Ext.isNumber(pp) || !Ext.isNumber(ndp)) {
				Ext.Error.raise(
					'Need numbers for PurchasePrice and NonDiscountedPrice to calulate savings'
				);
			}

			if (pp > ndp) {
				Ext.Error.raise(
					'PurchasePrice should not be greator than NonDiscountedPrice'
				);
			}

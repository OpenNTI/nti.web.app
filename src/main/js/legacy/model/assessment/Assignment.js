...
module.exports = exports = Ext.define(
	'NextThought.model.assessment.Assignment',
	{
		...
		fields: [
			...
			{
				name: 'availableBeginning',
				type: 'ISODate',
				mapping: 'available_for_submission_beginning',
			},
			{
				name: 'availableEnding',
				type: 'ISODate',
				mapping: 'available_for_submission_ending',
			},
			...
			{ name: 'UserCompletionCount', type: 'int' },
			{
				name: 'SubmittedCount',
				type: 'int',
				mapping: 'GradeAssignmentSubmittedCount',
			},
			{
				name: 'SubmittedCountTotalPossible',
				type: 'int',
				mapping: 'GradeSubmittedStudentPopulationCount',
			},
		...
		containsId: function (id) {
			var parts = this.get('parts') || [],
				items = parts.filter(function (p) {
					...
					const questionSetID =
						p.get('QuestionSetId') || (qSet && qSet.getId());
			...
			return Service.request(url)
				...
				.catch(function (reason) {
					if (reason.status !== 404) {
						console.error(
							'Failed to get the assignment save point: ',
							reason
						);
					}
				});
		...
		doNotShow: function () {
			return (
				this.isDeleted ||
				(this.isNoSubmit() && this.get('title') === 'Final Grade')
			);
		},
		...
		findMyCourse: function () {
			...
			var link = (this.getLink('History') || this.get('href')).replace(
				/\/AssignmentHistories.*/,
				''
			);
		...
		start: function () {
			...
			return Service.post(link).then(response => {
		...
		getDuration: function () {
			throw new Error(
				'Duration is no longer available on the assignment metadata, you have to get it from the history item'
			);
		},
	}
);

...
const User = (module.exports = exports = Ext.define(
	'NextThought.model.User',
	{
			...
			{ name: 'ignoring', type: 'UserList' },
			{
				...
				fn: function (record) {
					...
					try {
						...
						return (
							(p && p.getDisplayText && p.getDisplayText()) ||
							null
						);
					} catch (e) {
		...
		getCommunities: function (excludeDFLs) {
			...
			Ext.each(this.get('DynamicMemberships'), function (c) {
				...
				u = User.Repository.store.findRecord(
					field,
					c,
					0,
					false,
				...
				if (u) {
					...
					console.warn(
						'Dropping unresolvable community: ' + Ext.encode(c)
					);
				}
			});

			if (excludeDFLs) {
				return Ext.Array.filter(r, function (i) {
					return i.isCommunity;
				});
			}
			return r;
		},
		...
		getAvatarInitials: function () {
			...
			return Avatar.getAvatarInitials(
				this.raw,
				this.get('FirstName'),
				this.get('LastName'),
				this.getName()
		...
		getSchema: function () {
			...
			if (link) {
				this.loadSchema = Service.request(link).then(function (
					response
				) {
		...
		getMemberships: function (force) {
			...
			if (link) {
				this.loadMemberships = Service.request(link).then(function (
					response
				) {
		...
		__filterMemberships: function (fn) {
			return this.getMemberships().then(function (memberships) {
		...
		statics: {
					...
					u = new User(
						{
							...
							affiliation: 'Unknown',
							status: '',
							Presence: PresenceInfo.createFromPresenceString(
								'Offline'
							),
						},
						username
			...
			getUsername: function (usernameSeed) {
				var sitePattern = getString(
						'UnresolvedUsernamePattern',
						'username'
					),
					...
					hash =
						(usernameSeed && Math.abs(usernameSeed.hash())) || -1,
					hashPlaceholder = /(#+)/g;
		...
		sendEmailVerification: function () {
			...
			return Service.post(reqLink).then(function () {
				return Promise.resolve();
			});
		},
		...
		verifyEmailToken: function (token) {
			...
			var link = this.getLink('VerifyEmailWithToken'),
				me = this;
			return Service.post(link, { token: token }).then(function (
				response
			) {
		...
		getSuggestContacts: function () {
			if (
				...
				!(this.hasLink('SuggestContacts') || this.hasLink('Classmates'))
			) {
				return Promise.reject();
			}

			var link =
				this.getLink('SuggestContacts') || this.getLink('Classmates');
		...
		removeFirstTimeLoginLink: function () {
			...
			return Service.requestDelete(link).then(function () {
				me.deleteLink(rel);
				return Promise.resolve();
			});
		},

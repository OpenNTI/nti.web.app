		...
		Ext.define(
			'NextThought.model.User',
			{
				...
				getCommunities: function (excludeDFLs) {
					...
					Ext.each(this.get('DynamicMemberships'), function (c) {
						...
						if (u) {
							...
							console.warn(
								'Dropping unresolvable community: ' +
									Ext.encode(c)
							);
						}
					});
				...
				getPresence: function () {
					...
					return (
						presence ||
						PresenceInfo.createFromPresenceString('Offline')
					);
				},
				...
				getMemberships: function (force) {
					...
					if (link) {
						this.loadMemberships = Service.request(link).then(
							function (response) {
				...
				statics: {
					BLANK_AVATAR:
						'/app/resources/images/icons/unresolved-user.png',
				...
				getSuggestContacts: function () {
					...
					var link =
						this.getLink('SuggestContacts') ||
						this.getLink('Classmates');

	...
	'NextThought.model.courses.navigation.Node',
	{
		...
		fields: [
			...
			//(nti)id of parent
			{
				...
				convert: function (v) {
					return (
						v &&
						(v.getAttribute('topic-ntiid') ||
							v.getAttribute('ntiid'))
					);
				},
			},
			...
			//string displayed in the UI
			{
				...
				convert: function (v, m) {
					...
					return (
						v ||
						(n && Ext.DomQuery.selectValue(this.mapping, n)) ||
						'Failed'
					);
				},
			},
			...
			//due date
			{
				...
				mapping: '@date',
				dateFormat: 'c',
				exampleValue: '2013-10-16T00:00:00+00:00',
				convert: function (v, m) {
					if (Ext.isEmpty(v)) {
						return null;
					}
		...
		getAssociatedNode: function () {
			...
			if (!this.associatedNode) {
				...
				this.associatedNode = /topic/i.test(n.nodeName)
					? n
					: Ext.DomQuery.selectNode(
							'topic[ntiid="' +
								lazy.ParseUtils.escapeId(ntiid) +
								'"]',
							n.ownerDocument
					  );
				if (!this.associatedNode) {

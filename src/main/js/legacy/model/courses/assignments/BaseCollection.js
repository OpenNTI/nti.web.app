	...
	'NextThought.model.courses.assignments.BaseCollection',
	{
		...
		inheritableStatics: {
			...
			parseData: function (assignments, nonAssignments) {
				...
				nonAssignments =
					nonAssignments && (nonAssignments.Items || nonAssignments);
				...
				function filter(i) {
					return (
						(i.MimeType === assignmentMimeType ||
							i.MimeType === timedAssignmentMimeType) &&
						i.NTIID
					);
				}
		...
		constructor: function () {
			...
			this.cacheForShortPeriod(
				this.assignmentUpdateKey,
		...
		findItem: function (id) {
			var assignment = this.getItem(id),
				nonAssignment =
					!assignment && this.getItem(id, 'NonAssignments');
		...
		getItem: function (id, field) {
			...
			items = items.filter(function (rec) {
				return (
					rec.getId().toLowerCase() === id.toLowerCase() ||
					(rec.containsId && rec.containsId(id))
				);
			});
		...
		fetchAssignment: function (id) {
			...
			return Service.getObjectWithinBundle(
				assignmentId,
				this.get('bundle')
			).then(function (response) {
		...
		getFinalGradeAssignment: function () {
			if (this.__finalGrade !== undefined) {
		...
		isFinalGradeAssignment: function (assignment) {
			return (
				...
				assignment.get('title') === 'Final Grade'
			);
		},

...
module.exports = exports = Ext.define(
	'NextThought.model.courses.CourseCatalogEntry',
	{
		...
		fields: [
			...
			{
				name: 'NTI_FiveminuteEnrollmentCapable',
				type: 'bool',
				persist: false,
			},
...

			{
				name: 'DropCutOff',
				type: 'ISODate',
				mapping: 'OU_DropCutOffDate',
				persist: false,
			},
			{
				name: 'EnrollForCreditCutOff',
				type: 'ISODate',
				mapping: 'OU_EnrollCutOffDate',
				persist: false,
			},
			...
			},
			{
				name: 'icon',
				type: 'string',
				mapping: 'LegacyPurchasableThumbnail',
			}, //These look backwards, they are not... and are ONLY for fallback
		...
		getAuthorLine: function () {
			...
			return (
				(instructors &&
					instructors.filter(notTA).map(makeName).join(', ')) ||
				''
			);
		},
		...
		getCatalogFamily: function () {
			var catalogFamilies = this.get('CatalogFamilies'),
				families =
					(catalogFamilies && catalogFamilies.get('Items')) || [];

			if (families.length > 1) {
				console.warn(
					'More than one catalog family only using the first one'
				);
			}
		...
		updateEnrollmentState: function (status, open, admin) {
			var options = this.get('EnrollmentOptions'),
				...
				fmaepOption =
					options && options.getType('FiveminuteEnrollment'),
		...
		inSameFamily: function (catalog) {
			...
			return families.hasInstersectionWith(
				catalog.get('CatalogFamilies')
			);
		},
		...
		isEnrollable: function () {
			...
			return (
				enrollmentOptions &&
				enrollmentOptions &&
				enrollmentOptions.isEnrollable()
			);
		},
		...
		isExpired: function () {
			...
			try {
				...
				if (!Ext.isEmpty(duration)) {
					d =
						now.getTime() -
						new Duration(duration).inSeconds() * 1000;
		...
		getEnrollAndPayLink: function () {
			return (
				this.enrollandpayLink || this.getLink('fmaep.pay.and.enroll')
			);
		},
		...
		getCourseInstance() {
			...
			if (!link) {
				return Promise.reject('No CourseInstance link found.');
			}

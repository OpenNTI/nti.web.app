...
module.exports = exports = Ext.define(
	'NextThought.model.courses.ScormInstance',
	{
		...
		fields: [{ name: 'Metadata', type: 'singleItem' }],
				...
				this.precachePromise = new Promise(function (fulfill, reject) {
					...
					if (!url) {
						return reject(
							'Course Instance (' +
								me.getId() +
								') has a null link for "CourseCatalogEntry".'
						);
					}
					...
					Cls.load(null, {
						...
						callback: function (rec) {
							...
							if (rec) {
								rec
									.get('Links')
									.getRelLink('CourseInstance').href =
									me.get('href');
		...
		getPresentationProperties: function (id) {
			...
			return (
				cce.getPresentationProperties &&
				cce.getPresentationProperties(id)
			);
		},
		...
		getThumbnail: function () {
			const cce = this.getCourseCatalogEntry();
			return (
				(cce.getThumbnail && cce.getThumbnail()) || Promise.resolve('')
			);
		},
		...
		getVendorIconImage: function () {
			const cce = this.getCourseCatalogEntry();
			return (
				(cce.getVendorIcon && cce.getVendorIcon()) ||
				Promise.resolve('')
			);
		},
	}

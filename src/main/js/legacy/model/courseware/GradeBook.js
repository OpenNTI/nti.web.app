...
module.exports = exports = Ext.define(
	'NextThought.model.courseware.GradeBook',
	{
		...
		fields: [{ name: 'Items', type: 'collectionItem' }],
		...
		getItem: function (key, book, assignmentId) {
			...
			try {
				if (
					assignmentId &&
					(!i || i.get('AssignmentId') !== assignmentId)
				) {
					i = this.findGradeBookEntryFor(assignmentId) || [];
					i = i.reduce(function (a, c) {
						return a.getFieldItem('Items', c);
					}, this);
		...
		add: function (grade, assignmentId) {
			var path = grade
				...
				book =
					this.getFieldItem('Items', path[0]) ||
					this.buildBook(path[0]),
				entry =
					book &&
					(book.getFieldItem('Items', path[1]) ||
						book.buildEntry(path[1], assignmentId));
			...
			if (entry) {
				...
				console.warn(
					'Coud not add built grade object to the gradebook.' + path,
					book,
					entry
				);
			}

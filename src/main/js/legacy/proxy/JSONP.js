	...
	'NextThought.proxy.JSONP',
	{
		...
		request: function (options) {
			...
			if (!options.jsonpUrl) {
				...
				if (!Ext.String.endsWith(options.url, 'json', true)) {
					Ext.log.warn(
						'Assuming JSONP is the same name with .jsonp at the end!'
					);
					postfix = '.jsonp';
				}
		...
		requestJSONP: function (options) {
			...
			return new Promise(function (fulfill, reject) {
				...
				function jsonp(element) {
					...
					console.log(
						'JSONP.request completed',
						resp.responseText.length
					);
					try {
						opts.callback.call(
							opts.scope || window,
				...
				t = setTimeout(function () {
					onError(script, 'Timeout');
				}, 60000);

				script = Globals.loadScript(
					opts.jsonpUrl,
					jsonp,
					onError,
					this
		...
		receiveContent: function (content) {
			...
			if (type === 'application/xml') {
				type = 'text/xml';
				console.warn(
					'Forcing content type to text/xml from application/xml',
					content.ntiid
				);
			}

...
module.exports = exports = Ext.define('NextThought.proxy.reader.Json', {
	...
	readRecords: function (data) {
		...
		if (
			this.collectionTypes[mimeType] ||
			(mimeType === undefined && items)
		) {
			for (key in items) {
				if (items.hasOwnProperty(key)) {
					...
					if (typeof item === 'string') {
						console.warn(
							'IGNORING: Received string item at key:',
							key,
							item
						);
						continue;
					...
					if (!this.findModel(item)) {
						console.warn(
							'IGNORING: Received object that does not match a model',
							item
						);
						continue;
					}
		...
		try {
			...
			if (lastViewed) {
				result.lastViewed = Ext.Date.parse(
					lastViewed,
					'timestamp',
					true
				);
				...
				for (i; i >= 0; i--) {
					...
					try {
						...
						if (record instanceof lazy.Base && !record.homogenous) {
							...
							if (
								record.modelName.substr(-modelName.length) !==
								modelName
							) {
								result.records[i] = this.__rebuildRecordAsType(
									this.findModel(record.raw),
									record.getId(),
									record.raw
								);
								delete record.raw;

	...
	'NextThought.app.account.contacts.management.GroupList',
	{
		...
		buildGroupListStore: function () {
			var fstore = this.GroupStore.getFriendsList(),
				...
				mycontact = 'mycontacts-' + $AppConfig.username,
				me = this;
			...
			this.store = new Ext.data.Store({
				...
				filters: [
					function (item) {
						return item.get('Username') !== mycontact;
					},
		...
		afterRender: function () {
			var tpl = {
				cls: 'toolbar',
				cn: [
					{ cls: 'title', html: 'Distribution lists' },
					{ cls: 'close', html: '' },
				],
			};
		...
		onSelect: function (view, group) {
			...
			if (
				...
				this.GroupActions.addContact(this.username, [group]).then(
					me.onSelectCallback.bind(me)
				);
			}
		},

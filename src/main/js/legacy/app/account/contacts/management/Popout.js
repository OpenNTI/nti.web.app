...
module.exports = exports = Ext.define(
	'NextThought.app.account.contacts.management.Popout',
	{
								...
								cn: [
									{
										cls: 'control lists',
										'data-qtip':
											'{{{NextThought.view.account.contacts.management.Popout.distribution-list}}}',
									},
									{
										cls: 'control options',
										'data-qtip':
											'{{{NextThought.view.account.contacts.management.Popout.options}}}',
									},
								],
							},
						],
		...
		initComponent: function () {
			...
			this.isContact = this.GroupStore.getFriendsList().isContact(
				this.record
			);
			...
			this.optionsMenu = Ext.widget({
				xtype: 'person-options-menu',
				ownerCmp: me,
				user: me.user,
		...
		afterRender: function () {
			...
			this.on(
				...
				function () {
					return (
						this.groupsList.fireEvent('beforedeactivate') &&
						this.optionsMenu.fireEvent('beforedeactivate')
					);
				},
				this
			);
		...
		getListCount: function () {
			var u = this.record.get('Username'),
				...
				k = s.queryBy(function (a) {
					return a && a.hasFriend && a.hasFriend(u) && !a.isDFL;
				}),
				c = k.getCount();
		...
		onAddContact: function () {
			...
			this.GroupActions.addContact(this.user, data.groups).then(
				function () {
					me.destroy();
				}
			);
		...
		onDeleteContact: function () {
			...
			Ext.Msg.show({
				msg: getString(
					'NextThought.view.account.contacts.management.Popout.remove-msg'
				),
				title: getString(
					'NextThought.view.account.contacts.management.Popout.remove-title'
				),
				icon: 'warning-red',
				buttons: {
					primary: {
		...
		setPresenceButton: function () {
			var pi = this.user.get('Presence'),
				...
				isOnline =
					(pi && pi.isOnline && pi.isOnline()) || this.isUserOnline();
			if (
				(this.isContact && !isOnline) ||
				(current && !current.isOnline())
			) {

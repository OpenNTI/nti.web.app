...
module.exports = exports = Ext.define(
	'NextThought.app.account.activity.Preview',
	{
		...
		moreTpl: Ext.DomHelper.createTemplate({
			tag: 'a',
			cls: 'more',
			cn: [{}, {}, {}],
		}),
		...
		getCommentCount: function (record) {
			throw new Error(
				'Do not use the base class directly. Subclass and implement this'
			);
		},
		...
		prepareRenderData: function (record) {
			...
			UserRepository.getUser(o.Creator, function (u) {
				...
				if (me.rendered) {
					me.avatar.setStyle({
						backgroundImage: 'url(' + o.avatarURL + ')',
					});
			...
			this.mon(this.editor, {
				...
				'no-body-content': function (editor, bodyEl) {
					editor.markError(
						bodyEl,
						getString(
							'NextThought.view.account.activity.Preview.no-body-content'
						)
					);
					return false;
				},

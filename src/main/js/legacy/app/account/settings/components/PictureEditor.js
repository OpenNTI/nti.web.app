...
module.exports = exports = Ext.define(
	'NextThought.app.account.settings.components.PictureEditor',
	{
			...
			{ xtype: 'picture-canvas' },
			{
				...
				defaults: {
					...
					handler: function (btn, event) {
						btn.up('picture-editor').buttonHandler(
							btn,
							Boolean(btn.save),
							event
						);
				...
				items: [
					...
					{
						text: getString(
							'NextThought.view.account.settings.PictureEditor.save'
						),
						save: true,
						ui: 'flat-blue',
						disabled: true,
					...
					{
						text: getString(
							'NextThought.view.account.settings.PictureEditor.cancel'
						),
						cls: 'cancel footer-btn',
					},
					{
						text: getString(
							'NextThought.view.account.settings.PictureEditor.rotate'
						),
						rotate: true,
						iconCls: 'rotate',
						cls: 'rotate footer-btn',
						disabled: true,
						handler: function (b) {
							b.up('picture-editor').rotate();
						},
					},
		...
		editMode: function (url) {
			this.reset();
			this.down('picture-canvas').setImage(
				url || $AppConfig.userObject.get('avatarURL')
			);
		},
			...
			if (isSave) {
				...
				u.saveField(
					...
					function bad() {
						alert({
							title: getString(
								'NextThought.view.account.settings.PictureEditor.error-title'
							),
							msg: getString(
								'NextThought.view.account.settings.PictureEditor.error-msg'
							),
						});
					}
				);

...
module.exports = exports = Ext.define(
	'NextThought.app.account.settings.components.PasswordResetForm',
	{
		...
		items: [
						...
						items: [
							{
								...
								placeholder: getString(
									'NextThought.view.form.PasswordResetForm.oldplaceholder'
								),
								allowBlank: true,
								validator: function (value) {
									if (Ext.isEmpty(value)) {
										throw getString(
											'NextThought.view.form.PasswordResetForm.emptyold'
										);
									}
									return true;
								},
							},
							{
								name: 'password',
								placeholder: getString(
									'NextThought.view.form.PasswordResetForm.newplaceholder'
								),
								//minLength: 6, // validator doesn't get called if this is set, and the value is less than
								validator: function (value) {
									if (Ext.isEmpty(value)) {
										throw getString(
											'NextThought.view.form.PasswordResetForm.emptynew'
										);
									}
									if (!Ext.String.trim(value)) {
										throw getString(
											'NextThought.view.form.PasswordResetForm.whitespace'
										);
									}
									if (value.length < 6) {
										throw getString(
											'NextThought.view.form.PasswordResetForm.short'
										);
									}
									return true;
								},
							},
							{
								...
								placeholder: getString(
									'NextThought.view.form.PasswordResetForm.verifyplaceholder'
								),
								allowBlank: true,
								listeners: {
								...
								validator: function (value) {
									...
									throw getString(
										'NextThought.view.form.PasswordResetForm.nomatch'
									);
								},
							},
						],
			...
			},
			{
				...
				items: [
					{
						text: getString(
							'NextThought.view.form.PasswordResetForm.save'
						),
						save: 1,
						ui: 'flat-blue',
						scale: 'medium',
							...
							cn: [
								{
									cls: 'title',
									html: getString(
										'reset-password-not-allowed-title'
									),
								},
								{
									cls: 'subtext',
									html: getString(
										'reset-password-not-allowed-subtext'
									),
								},
							],
						}),
		...
		setSuccess: function () {
			this.setMessage(
				getString('NextThought.view.form.PasswordResetForm.changed')
			);
		...
		afterRender: function () {
			...
			Ext.each(
				this.inputs,
				function (i) {
					this.mon(i, 'changed', this.checkValidity, this, {
						buffer: 250,
					});
				},
				this
		...
		checkValidity: function (value, input) {
			...
			if (val(input)) {
				me.setMessage();
				v = me.inputs.reduce(function (accum, o) {
					return accum && val(o, true);
				}, true);

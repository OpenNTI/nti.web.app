...
module.exports = exports = Ext.define(
	'NextThought.app.account.settings.components.PictureCanvas',
	{
		...
		syncSizeAttributes: function () {
			...
			this.el.dom.setAttribute('width', width);
			this.el.dom.setAttribute('height', height);
		},
		...
		onMouseMove: function (e) {
			...
			function doSize(corner, anchor) {
				...
				s.size = Math.round(
					clamp(newSize, 32, Math.min(i.width, i.height))
				);

				if (mX) {
					s.x = clamp(s.x + diff, 0, anchor[0] - s.size);
				}
				if (mY) {
					s.y = clamp(s.y + diff, 0, anchor[1] - s.size);
				}
			}
		},
		...
		createFileInput: function () {
			var me = this,
				old = me.fileInputEl,
				file = (me.fileInputEl = Ext.DomHelper.insertAfter(
					me.targetInputEl || me.el,
					{
						name: 'file1',
						cls: 'file-input',
						tag: 'input',
		...
		doLegacyUpload: function (fileList) {
			if (!fileList || fileList.length === 0 || !fileList[0]) {
				alert({
					title: getString(
						'avatar.upload.unsupported.title',
						'So Sorry :('
					),
					//I don't even know who would see this in our supported browsers list. (they all support FileAPI)
					msg: getString(
						'avatar.upload.unsupported.message',
						'Unforseen legacy browser issue.'
					),
				});
				return;
			}
			...
			var me = this,
				...
				fields,
				url = getURL(
					$AppConfig['server-path'] + '@@image_to_dataurl_extjs'
				);

			if (file.size > this.ONE_MEGABYTE) {
				alert({
					title: getString(
						'avatar.legacy.upload.file.size.too.big.title',
						"We're sorry, but..."
					),
					msg: getString(
						'avatar.legacy.upload.file.size.too.big.message',
		...
		readFile: function (files) {
			...
			if (!file || !/image\/.*/i.test(file.type)) {
				console.log(
					'selected file was invalid, or the browser does not support FileAPI'
				);
				return;
			}
		...
		validate: function () {
			return Boolean(this.fileInputEl.dom.value);
		},
	}
);

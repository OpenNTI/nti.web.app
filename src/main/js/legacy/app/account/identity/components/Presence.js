...
module.exports = exports = Ext.define(
	'NextThought.app.account.identity.components.Presence',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				cls: 'header',
				html: '{{{NextThought.view.menus.Presence.header}}}',
			},
			{
				cls: 'list',
				cn: [
		...
		beforeRender: function () {
			...
			me.renderData = Ext.apply(me.renderData || {}, {
				states: [
					{
						state: 'available',
						label: me.defaultStates.available,
						editable: true,
					},
					{
						state: 'away',
						label: me.defaultStates.away,
						editable: true,
					},
					{
						state: 'dnd',
						label: me.defaultStates.dnd,
						editable: true,
					},
			...
			$AppConfig.Preferences.getPreference('ChatPresence').then(function (
				value
			) {
				if (value) {
		...
		savePreferenceValues: function (record, key, type, show, status) {
			...
			if (!record || record.isFuture) {
				$AppConfig.Preferences.getPreference(key).then(function (
					value
				) {
					if (value) {
		...
		savePreference: function (type, show, status) {
			...
			if (show === 'chat') {
				...
				status = Ext.isEmpty(status)
					? this.defaultStates.available
					: status;
			} else if (show === 'away') {
		...
		restoreState: function () {
			...
			function update() {
				try {
					availableStatus =
						available && !available.isFuture
							? available.get('status')
							: me.defaultStates.available;
					me.availableEl.down('.label').update(availableStatus);

					awayStatus =
						away && !away.isFuture
							? away.get('status')
							: me.defaultStates.away;
					me.awayEl.down('.label').update(awayStatus);

					dndStatus =
						dnd && !dnd.isFuture
							? dnd.get('status')
							: me.defaultStates.dnd;
					me.dndEl.down('.label').update(dndStatus);

					if (type === 'available') {
						me.availableEl[
							show === 'chat' ? 'addCls' : 'removeCls'
						]('selected');
						me.awayEl[show === 'away' ? 'addCls' : 'removeCls'](
							'selected'
						);
						me.dndEl[show === 'dnd' ? 'addCls' : 'removeCls'](
							'selected'
						);
		...
		clicked: function (e) {
			...
			if (e.getTarget('.available')) {
				status = e.getTarget('.available', 10, true).down('.label').dom
					.innerHTML;
				show = 'chat';
				type = 'available';
			} else if (e.getTarget('.away')) {
				status = e.getTarget('.away', 10, true).down('.label').dom
					.innerHTML;
				show = 'away';
				type = 'available';
			} else if (e.getTarget('.dnd')) {
				status = e.getTarget('.dnd', 10, true).down('.label').dom
					.innerHTML;
				show = 'dnd';
				type = 'available';
			...
			this.deferHideParentMenusTimer = Ext.defer(
				this.deferHideParentMenus.bind(this),
				250
			);
		},
		...
		isStatus: function (value) {
			...
			return (
				v && v !== 'available' && v !== 'away' && v !== 'do not disturb'
			);
		},
		...
		startEditor: function (e) {
			...
			if (edit) {
				row.addCls('active');
				this.editor.field.emptyText = edit.getAttribute(
					'data-placeholder'
				);

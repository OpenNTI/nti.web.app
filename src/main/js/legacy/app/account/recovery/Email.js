	...
	'NextThought.app.account.recovery.Email',
	{
				...
				items: [
					{
						...
						placeholder: getString(
							'NextThought.view.account.recovery.Email.email'
						),
					},
				],
			},
				...
				items: [
					{
						...
						name: 'cancel',
						text: getString(
							'NextThought.view.account.recovery.Email.cancel'
						),
						handler: function () {
					...
					},
					{
						...
						name: 'submit',
						text: getString(
							'NextThought.view.account.recovery.Email.submit'
						),
						handler: function () {
		...
		setError: function (error) {
			...
			if (error.code === 'AttemptingToResendConsentEmailTooSoon') {
				//remove submit:
				Ext.each(bContainer.query('button'), function (b) {
					b.destroy();
				});
				bContainer.add({
					xtype: 'button',

	...
	'NextThought.app.annotations.note.Panel',
	{
						...
						cn: [
							{
								...
								cn: [
									...
									},
									{
										cls: 'link more',
										'data-qtip':
											'{{{NextThought.view.annotations.note.Panel.options}}}',
										html: '&nbsp;',
									},
								],
							},
		...
		afterRender: function () {
			...
			if (me.record.isFlagged()) {
				console.log(
					'TODO - this is flagged, consider an indicator, or remove this log.'
				);
			}
		...
		fillInUser: function (user) {
			...
			TemplatesForNotes.attachMoreReplyOptionsHandler(
				this,
				this.more,
				user,
				this.record
		...
		fillInShare: function (sharedWith) {
			var me = this,
				...
				sharingInfo = SharingUtils.sharedWithToSharedInfo(
			...
			me.responseBox[
				(sharedWith || []).length === 0 ? 'removeCls' : 'addCls'
			]('shared');

			SharingUtils.getLongTextFromShareInfo(sharingInfo, tpl, 150).then(
				function (str) {
					me.sharedTo.update(str);
					me.sharedTo.set({ 'data-qtip': str });
					me.sharedTo
						.select('name[data-profile-idx]')
						.on('click', function (e) {
		...
		editorSaved: function () {
			...
			function save() {
				let p;
				if (me.editMode) {
					p = me.UserDataActions.saveUpdatedNote(
						r,
						v.body || [],
						v.title
					)
			...
			if (DomUtils.isEmpty(v.body)) {
				me.editor.markError(
					me.editorEl.down('.content'),
					getString(
						'NextThought.view.annotations.note.Panel.empty-editor'
					)
				);
				return;
			}
		...
		updateFromRecord: function (newRecord, modifiedFieldName) {
			...
			try {
				...
				if (me.sharedTo) {
					UserRepository.getUser(r.get('sharedWith').slice()).then(
						function (users) {
		...
		loadReplies: function (record) {
			var me = this,
				toMask = me.el ? me.el.down('.note-replies') : null;
			if (toMask) {
				toMask.mask('Loading...');
			}
			...
			return new Promise(function (fulfill, reject) {
				function setReplies(theStore) {
					...
					if (
						items.length === 1 &&
						items[0].getId() === record.getId()
					) {
						items = (items[0].children || []).slice();
					} else {
						console.warn(
							'There was an unexpected result from the reply store.'
						);
					}
		...
		maybeOpenReplyEditor: function () {
			...
			var cmp,
				prefix = this.getRoot().replyIdPrefix();
			if (this.replyToId) {
				cmp = Ext.getCmp(
					IdCache.getComponentId(this.replyToId, null, prefix)
				);
				if (cmp) {
				...
				cmp = Ext.getCmp(
					IdCache.getComponentId(this.scrollToId, null, prefix)
				);
				if (cmp) {
					cmp.scrollIntoView();
		...
		getReplyCmp(replyId) {
			...
			const cmp = Ext.getCmp(
				IdCache.getComponentId(replyId, null, prefix)
			);
		...
		setContent: function (text) {
			...
			var search = this.up('[getSearchTerm]'),
				re;
			if (search) {
			...
			if (search) {
				...
				text = text.replace(
					re,
					'$1<span class="search-term">$2</span>$3'
				);
			}
			...
			Ext.each(
				this.text.query('.whiteboard-container'),
				function (wb) {
		...
		setEditorActive: function (cmp) {
			...
			if (root.editorActive() === active) {
				console.warn(
					'Panel already has an ' +
						(active ? 'active' : 'inactive') +
						' editor. Unbalanced calls?',
					cmp
				);
				return;
			}
		...
		deactivateReplyEditor: function () {
			...
			if (this.noteBody) {
				...
				this.el
					.select('.whiteboard-container .checkbox')
					.removeCls('checked');
		...
		addNewChild: function (child) {
			...
			if (child.get('inReplyTo') === r.getId()) {
				...
				console.log(
					'[reply] ignoring, child does not directly belong to this item:\n',
					r.getId(),
					'\n',
					child.get('inReplyTo'),
					' <- new child'
		...
		addReplies: function (records) {
			var toAdd = [],
				recordCollection,
				prefix = this.getRoot() ? this.getRoot().replyIdPrefix() : null;
			...
			recordCollection.each(function (record) {
				...
				if (record.getModelName() !== 'Note') {
					console.warn(
						'cannot add item, it is not a note and I am not prepared to handle that.'
					);
					add = false;
		...
		onRemove: function (cmp) {
			...
			var c = this.items.getCount(),
				...
				safeToCleanMe = !contains(
					Ext.Array.map(
						pluck(pluck(children, 'record'), 'placeholder'),
						Boolean
					),
					false
				),
			...
			console.debug(
				'removed child, it was deleting: ',
				cmp.deleting,
				', or any child below me is deleting: ',
				anyDeleting,
				', alse we have ' + c + ' children. Safe to delete: ',
				safeToCleanMe
			);

			if (
				(c === 0 || safeToCleanMe) &&
				(!this.record || this.record.placeholder)
			) {
				this.deleting = Boolean(deleting);
				console.debug(
					'cleaning up placeholder panel now that all children are gone.',
					this
				);
				this.destroy();
			}
		...
		startResourceTimer: function () {
			if (!this.triggerAnalyticsViews || !this.record) {
				return;
			}
		...
		wasDeleted: function () {
			console.log(
				'Deleting panel from record destroy, marking deleteing=true'
			);
			this.deleting = true;
			this.destroy();
		},
		...
		setPlaceholderContent: function () {
			...
			if (fromUpdatedRecord) {
				console.debug(
					'This record was updated to be a placeholder...references are now dirty, and disabling replys for all children'
				);
		...
		handleWhiteboardClick: function (e) {
			...
			if (t && this.wbData[guid]) {
				t = e.getTarget(
					'.reply:not(.profile-activity-reply-item)',
					null,
					true
				);
				if (t) {
					...
					this.editor.addWhiteboard(
						Ext.clone(this.wbData[guid]),
						guid + '-reply'
					);
				} else {
					t = e.getTarget('.include', null, true);
					if (t) {
						t[t.hasCls('checked') ? 'removeCls' : 'addCls'](
							'checked'
						);
						if (t.hasCls('checked')) {
							this.editor.addWhiteboard(

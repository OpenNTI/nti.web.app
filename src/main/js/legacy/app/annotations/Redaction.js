...
module.exports = exports = Ext.define(
	'NextThought.app.annotations.Redaction',
	{
		...
		buildMenu: function (items) {
			...
			items.push({
				text: getString(
					'NextThought.view.annotations.Redaction.toggle'
				),
				handler: function () {
					me.toggleRedaction();
				},
		...
		makeEditableSpanEditable: function (e) {
			e.stopEvent();
			var s = this.editableSpan,
				range,
				save = this.masterSpan.down('.edit'),
			...
			s.clearListeners();
			s.set({ contenteditable: 'true' }).on(
				'keydown',
				this.editableSpanEditorKeyDown,
				this
			);
			s.focus();
		...
		render: function () {
			...
			if (this.actionSpan) {
				return (
					this.actionSpan.getBoundingClientRect().top ||
					this.rendered[0].getBoundingClientRect().top ||
					y
				);
			}
			...
			if (this.rendered) {
				...
				this.actionSpan = this.createActionHandle(
					this.rendered[0],
					isBlock
				);
				if (isBlock) {
		...
		getRestrictedRange: function (annotationOffsets) {
			...
			if (this.masterSpan.hasCls(this.cls)) {
				...
				rr =
					annotationOffsets ||
					this.manager.getReader().getAnnotationOffsets();
		...
		visibilityChanged: function (show) {
			if (this.actionSpan) {
				Ext.fly(this.actionSpan).setVisibilityMode(
		...
		isBlockRedaction: function () {
			return (
				this.record && Boolean(this.record.get('redactionExplanation'))
			);
		...
		createActionHandle: function (before, block) {
			...
			this.masterSpan = this.actionTpl.insertBefore(
				before,
				{
		...
		saveEditorContent: function () {
			...
			this.record.set(
				'replacementContent',
				this.editableSpan.dom.textContent
			);
			this.record.save();
		...
		toggleRedaction: function (e) {
			var redactionCollapsed = !this.compElements
					.first()
					.hasCls(this.cls),
				me = this;
			...
			if (this.masterSpan) {
				...
				if (!this.masterSpan.dom) {
					this.masterSpan.dom = this.actionSpan;
				}
			...
			if (this.innerFootnotes) {
				this.innerFootnotes.each(function (footnote) {
					var count = footnote
							? footnote.getAttribute('data-redactedCount')
							: undefined,
						clsMnpFn;
		...
		containedFootnotes: function () {
			...
			this.compElements.each(function (e) {
				...
				Ext.each(fns, function (fn) {
					footnotes.push(
						Ext.DomQuery.select(fn.getAttribute('href'), me.doc)[0]
					);
				});
			});
	...
	function () {
						...
						cn: [
							{
								...
								cls: 'edit',
								'data-qtip':
									'{{{NextThought.view.annotations.Redaction.edit}}}',
							},
							{
								tag: 'span',
								'data-non-anchorable': 'true',
								cls: 'share',
								'data-qtip':
									'{{{NextThought.view.annotations.Redaction.share}}}',
							},
							{
								tag: 'span',
								'data-non-anchorable': 'true',
								cls: 'delete',
								'data-qtip':
									'{{{NextThought.view.annotations.Redaction.delete}}}',
							},
						],
					},
				],
		...
		p.footnoteRedactedTpl = new Ext.XTemplate(
			Ext.DomHelper.markup([
				{
					tag: 'span',
					cls: 'redacted-text',

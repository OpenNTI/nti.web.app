...
module.exports = exports = Ext.define(
	'NextThought.app.course.catalog.Collection',
	{
		...
		prepareData: function (data, index, record) {
			...
			let administering = getString(
				'course-enrollment.administering',
				'Administering'
			);
			let open = getString('course-enrollment.open', 'Not For Credit');
			let forCredit = getString(
				'course-enrollment.forcredit',
				'For Credit'
			);

			i.author = record.getAuthorLine();
			i.isCourse = true;
			i.enrolledText = isAdmin
				? administering
				: isOpen
				? open
				: forCredit;

...
module.exports = exports = Ext.define(
	'NextThought.app.course.catalog.TabPanel',
	{
		...
		welcomeCardTpl: new Ext.XTemplate(
		...
		getTabForCourse: function (course) {
			...
			if (
				upcoming &&
				upcoming.store &&
				upcoming.store.find('NTIID', id) >= 0
			) {
				return upcoming;
			}
			...
			if (
				archived &&
				archived.store &&
				archived.store.find('NTIID', id) > 0
			) {
				return archived;
			}
			...
			if (Ext.isEmpty(courses)) {
				...
				cmp = this.add({
					...
					autoEl: {
						...
						cls: 'empty-text',
						html: getString(
							'course.catalog.TabPanel.current.empty'
						),
					},
					title: 'Current',
				});
			...
			if (Ext.isEmpty(courses)) {
				...
				cmp = this.add({
					...
					autoEl: {
						...
						cls: 'empty-text',
						html: getString(
							'course.catalog.TabPanel.upcoming.empty'
						),
					},
					title: 'Upcoming',
				});
			...
			if (Ext.isEmpty(courses)) {
				...
				cmp = this.add({
					...
					autoEl: {
						...
						cls: 'empty-text',
						html: getString(
							'course.catalog.TabPanel.archive.empty'
						),
					},
					title: 'Archived',
				});

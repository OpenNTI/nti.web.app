...
module.exports = exports = Ext.define(
	'NextThought.app.course.enrollment.components.parts.CreditCard',
	{
		...
		afterRender() {
			...
			purchasable.getInterfaceInstance().then(model => {
		...
		addEmptyErrors() {
			...
			if (empty.name) {
				this.addCls('missing-name');
			}
			if (empty.number) {
				this.addCls('missing-number');
			}
			if (empty.expiry) {
				this.addCls('missing-expiry');
			}
			if (empty.cvc) {
				this.addCls('missing-cvc');
			}
		},

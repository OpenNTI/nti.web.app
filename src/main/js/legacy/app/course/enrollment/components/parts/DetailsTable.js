	...
	'NextThought.app.course.enrollment.components.parts.DetailsTable',
	{
						...
						cn: [
							{
								...
								cn: [
									{ cls: 'name', html: 'prerequisites' },
									{
										tag: 'tpl',
										for: 'prereqs',
										cn: [{ cls: 'value', html: '{title}' }],
									},
								],
							},
							{
								cls: 'field medium green',
								cn: [
		...
		beforeRender: function () {
			...
			this.renderData = Ext.apply(this.renderData || {}, {
				...
				prereqs: prereqs,
				credit: credit
					? credit.get('Hours') + ' credit hours available'
					: 'No credit hours available',
				start: Ext.Date.format(c.get('StartDate'), 'F j, Y'),
				end: Ext.Date.format(c.get('EndDate'), 'F j, Y'),
				school: c.get('ProviderDepartmentTitle'),
				duration:
					duration && duration.inWeeks() > 0
						? Math.floor(duration.inWeeks()) + ' Weeks'
						: null,
				type: 'Fully Online',
			});
		},
		...
		getPrice: function () {
			...
			return (
				'$' +
				(this.enrollmentOption.OU_Price || this.enrollmentOption.Price)
			);
		},
	}
);

	...
	'NextThought.app.course.enrollment.components.parts.RadioGroup',
	{
		...
		renderTpl: Ext.DomHelper.markup({
			cn: [
				{ cls: 'label', html: '{label}' },
				{
					...
					for: 'options',
					cn: {
						cls: 'enrollment-input dark full radio',
						cn: [
							{
								tag: 'input',
								id: '{parent.id}-{value}',
		...
		beforeRender: function () {
			...
			(me.options || []).forEach(function (option) {
				var width =
					(option.inputWidth && option.inputWidth + 'px') || 'auto';
		...
		setUpChangeMonitors: function () {
			...
			if (input) {
				me.mon(input, 'keydown', function () {
					...
					me.inputChangeTimeout = setTimeout(
						me.changed.bind(me),
						500
					);
				});
		...
		changed: function (e) {
			...
			var me = this,
				inputFields = this.el.dom.querySelectorAll(
					'.enrollment-input .input-container'
				),
			...
			if (!inputFields || Ext.isEmpty(inputFields)) {
				return;
			}
			...
			wait().then(function () {
				...
				Ext.each(inputFields, function (input) {
					var credit = Ext.fly(input).up('.enrollment-input'),
						checkedRadio =
							credit &&
							credit.el.dom.querySelector(
								'input[type=radio]:checked'
							);

					Ext.fly(input)[checkedRadio ? 'removeCls' : 'addCls'](
						'disabled'
					);
				});
			});
		...
		setValue: function (value) {
			...
			wait().then(function () {
				...
				if (me.dropdown) {
					...
					parent =
						input &&
						input.up('.enrollment-input').down('input[type=radio]');
	...
	*/
		__selectionValues: function () {
			...
			if (this.valType === 'number' && !Ext.isEmpty(inputvalue)) {
				...
				inputvalue = isNaN(tmp)
					? this.allowEmptyInput
						? ''
						: null
					: tmp;
			}
		...
		isValid: function () {
			if (!this.required || !this.isVisible(true)) {
				return true;
			}

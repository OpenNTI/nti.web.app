...
module.exports = exports = Ext.define(
	'NextThought.app.course.enrollment.components.Enroll',
	{
		...
		buttonCfg: [
			{
				name: getString(
					'NextThought.view.courseware.enrollment.Enroll.ConttoPay'
				),
				action: 'goto-payment',
			},
		],
		...
		beforeRender: function () {
			...
			this.renderData = Ext.apply(this.renderData || {}, {
				...
				header: this.paymentfail
					? getString(
							'NextThought.view.courseware.enrollment.Enroll.PaymentProblems'
					  )
					: getString(
							'NextThought.view.courseware.enrollment.Enroll.AppAccepted'
					  ),
				text: this.paymentfail
					? getString(
							'NextThought.view.courseware.enrollment.Enroll.TryPayAgain'
					  )
					: getString(
							'NextThought.view.courseware.enrollment.Enroll.ThanksCreditOU'
					  ),
				available: getFormattedString(
					'NextThought.view.courseware.enrollment.Enroll.CreditAvailable',
					{ title: c.getSemester() }
				),
				confirm: getString(
					'NextThought.view.courseware.enrollment.Enroll.ConfirmCourse'
				),
			});
		},
		...
		beforeShow: function () {
			...
			if (this.enrollmentOption.AllowVendorUpdates) {
				this.subscribeLabelEl.update(
					getString('SubscribeToVendor') || 'Subscribe to updates.'
				);
				this.subscribeLegalEl.update(
					getString('SubscribeToVendorLegal')
				);
		...
		stopClose: function () {
			return new Promise((fulfill, reject) => {
				Ext.Msg.show({
					msg: getString(
						'NextThought.view.courseware.enrollment.Enroll.NotFinished'
					),
					...
					buttonText: {
						ok:
							'caution: ' +
							getString(
								'NextThought.view.courseware.enrollment.Enroll.Leave'
							),
						cancel: getString(
							'NextThought.view.courseware.enrollment.Enroll.Stay'
						),
					},
					title: getString(
						'NextThought.view.courseware.enrollment.Enroll.AreYouSure'
					),
					fn: str => {
						if (str === 'ok') {
							fulfill();
		...
		showErrorMsg: function (json) {
			if (json && json.Message) {
				...
				this.fireEvent(
					'show-msg',
					getString(
						'NextThought.view.courseware.enrollment.Enroll.AlreadyEnrolled'
					),
					true,
					5000
				);
		...
		showError: function (json) {
			...
			json.ContactInformation =
				json.ContactInformation ||
				getString(
					'NextThought.view.courseware.enrollment.Enroll.HelpDesk'
				);
		...
		maybeSubmit: function () {
			...
			subscribe =
				this.subscribeEl &&
				this.subscribeEl.isVisible() &&
				this.subscribeEl.dom.checked;

			me.addMask(
				getString(
					'NextThought.view.courseware.enrollment.Enroll.ExternalPay'
				),
				'navigation'
			);

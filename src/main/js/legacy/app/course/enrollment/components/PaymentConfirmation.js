...
module.exports = exports = Ext.define(
	'NextThought.app.course.enrollment.components.PaymentConfirmation',
	{
		...
		buttonCfg: [
			{
				name: getString(
					'NextThought.view.courseware.enrollment.PaymentConfirmation.SubmitPayment'
				),
				action: 'submit-payment',
			},
			{
				name: getString(
					'NextThought.view.courseware.enrollment.PaymentConfirmation.Cancel'
				),
				action: 'go-back',
				secondary: true,
			},
			...
			if (purchaseDesc.from) {
				...
				if (purchaseDesc.sender) {
					this.giftFromEl
						.down('.name')
						.update(
							purchaseDesc.sender + ' (' + purchaseDesc.from + ')'
						);
				} else {
			...
			if (purchaseDesc.receiver) {
				...
				if (purchaseDesc.to) {
					this.giftToEl
						.down('.to-email')
						.update(
							purchaseDesc.to + ' (' + purchaseDesc.receiver + ')'
						);
				} else {
					this.giftToEl
						.down('.to-email')
						.update(purchaseDesc.receiver);
				}
			} else {
				this.giftToEl.hide();
			}
		...
		stopClose: function () {
			...
			if (me.hasMask()) {
				...
				r = new Promise(function (fulfill, reject) {
					Ext.Msg.show({
						title: getString(
							'NextThought.view.courseware.enrollment.PaymentConfirmation.PayNotSubmitted'
						),
						msg: getString(
							'NextThought.view.courseware.enrollment.PaymentConfirmation.ProgressLost'
						),
						icon: 'warning-red',
						buttons: {
							primary: {
								text: getString(
									'NextThought.view.courseware.enrollment.PaymentConfirmation.StayFinish'
								),
								handler: reject,
							},
							secondary: {
								text: getString(
									'NextThought.view.courseware.enrollment.PaymentConfirmation.LeavePage'
								),
								handler: function () {
									fulfill();
								},
		...
		showError: function (json) {
			if (json && (json.Message || json.message)) {
				...
				this.fireEvent(
					'show-msg',
					getString(
						'NextThought.view.courseware.enrollment.PaymentConfirmation.UnknownError'
					),
					true
				);
			}
		...
		maybeSubmit: function () {
			...
			me.addMask(
				getString(
					'NextThought.view.courseware.enrollment.PaymentConfirmation.SubPaymentTime'
				)
			);
			...
			me.complete(me, data)
				.then(function (result) {
					...
					me.enrollmentOption.purchaseAttempt =

...
module.exports = exports = Ext.define(
	'NextThought.app.course.enrollment.components.parts.TextInput',
	{
		...
		addFormatter: function (formatter) {
			var input =
				this.input && this.input.inputEl && this.input.inputEl.dom;
		...
		isValid: function () {
			...
			var value = this.getValue()[this.name],
				...
				paymentValidator =
					this.validator && jQuery.payment[this.validator];
		...
		isEmpty: function () {
			return Ext.isEmpty(this.input.getValue().replace(/\W/gi, ''));
		},
		...
		getValue: function (force) {
			...
			if (this.getter && Ext.isFunction(this.getter)) {
				...
				val = jQuery(this.input.inputEl.dom).payment(
					this.paymentGetter
				);
			}

	...
	'NextThought.app.course.enrollment.options.FiveminuteEnrollment',
	{
			...
			function getEnrollAndPayLink() {
				...
				if (!link) {
					link = Service.getLinkFrom(
						enrollmentOption.Links,
						'fmaep.pay.and.enroll'
					);
				}
		...
		__getEnrollmentText: function (course, option) {
			var state = {},
				...
				dropDate =
					details.DropCutOff &&
					Ext.Date.format(details.DropCutOff, this.DateFormat),
				dropText =
					dropDate && ' by ' + dropDate + ' for a full refund.';
			...
			if (details.EndDate < now) {
			...
			} else if (details.Enrolled) {
				//if the course is active and we are enrolled in this option
				state = this.getWording('enrolled', {
					date: Ext.Date.format(details.StartDate, this.DateFormat),
					drop: dropText || '',
				});
			} else if (details.AdmissionState === 'Pending') {
				//if we are pending admission
				state = this.getWording('admissionPending');
			} else if (details.API_DOWN) {
				//if we detect the admission api is down
				...
				state = !Ext.Object.isEmpty(state)
					? state
					: {
							title: 'Earn College Credit',
							information:
								'Your browser (FireFox) does not support the enrollment process. Please try Chrome, Safari, or Internet Explorer.',
					  };
				state.price = details.Price;
			} else if (details.AdmissionState === 'Rejected') {
				//if our application was rejected
				state = this.getWording('admissionRejected');
			} else {
				//we aren't enrolled
				state = this.getWording('notEnrolled', {
					date: Ext.Date.format(
						details.EnrollCutOff,
						this.DateFormat
					),
				});
				...
				if (details.AvailableSeats !== undefined) {
					...
					if (details.AvailableSeats === 0) {
						...
						state.warning = '';
						state.buttonText = '';
					} else if (details.AvailableSeats <= 10) {
						//if there are less than 10 seats left tell the user
						state.seats =
							'Only' +

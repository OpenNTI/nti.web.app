	...
	'NextThought.app.course.enrollment.options.StoreEnrollment',
	{
		...
		DefaultStrings: {
			notEnrolled: {
				title: 'Candidate',
				information:
					'Gain complete access to all exam preparation materials in the course',
				refund: 'Enrollment is not refundable.',
			},
			enrolled: {
				title: "You're Enrolled",
				information:
					'Your access to exam preparation materials begins now.',
				links: [],
				cls: 'enrolled',
			},
			archivedEnrolled: {
		...
		__getPurchasableByNTIID: function (ntiid, option) {
			var items = option.Purchasables.Items,
				i,
				item,
				purchasable;
		...
		__getGiftPurchasable: function (option) {
			var ntiid =
				option &&
				option.Purchasables &&
				option.Purchasables.DefaultGiftingNTIID;
		...
		__getPurchasable: function (option) {
			var ntiid =
				option &&
				option.Purchasables &&
				option.Purchasables.DefaultPurchaseNTIID;
			...
			me.__addStep(
				{
					...
					async complete(cmp, data) {
						...
						return new Promise(function (fulfill, reject) {
							me.StoreActions.createEnrollmentPurchase(
								cmp,
			...
			me.__addStep(
				{
					...
					async complete(cmp, data) {
						...
						return new Promise(function (fulfill, reject) {
							...
							me.StoreActions.submitEnrollmentPurchase(
								cmp,
								data.purchaseDescription,
								data.tokenObject,
								data.pricingInfo,
							...
							return new Promise(function (fulfill, reject) {
								me.CourseEnrollmentActions.refreshEnrolledCourses(
									fulfill.bind(null, result),
									reject
								);
							});
						});
			...
			me.__addStep(
				{
					...
					async complete(cmp, data) {
						...
						return new Promise(function (fulfill, reject) {
							me.StoreActions.submitGiftPurchase(
								cmp,
								data.purchaseDescription,
								data.tokenObject,
								data.pricingInfo,
		...
		__getEnrollmentText: function (course, option) {
			...
			if (details.EndDate < now) {
				...
				if (!details.Enrolled) {
					...
					//if we didn't enroll before it was archived
					state = this.getWording('archivedNotEnrolled');
					state.price = details.Price;
				}
			} else {
		...
		buildEnrollmentDetails: function (course, details) {
			...
			if (
				!option ||
				(!option.IsEnrolled && !option.IsAvailable) ||
				course.get('IsAdmin')
			) {
				return {
					name: this.NAME,
		...
		buildGiftOptions: function (course, details) {
			...
			if (
				!giftPurchasable ||
				!(
					giftPurchasable.isGiftable() ||
					giftPurchasable.isRedeemable()
				)
			) {
				return {};
			}
			...
			return {
				...
				Redeemable:
					giftPurchasable.isRedeemable() && !course.isActive(),
				doEnrollment: function (cmp, type, config) {
					cmp.fireEvent(
						'enroll-in-course',
						course,

...
module.exports = exports = Ext.define(
	'NextThought.app.course.assessment.components.admin.Grid',
	{
			...
			items: [
...

				{
					...
					renderer: function (val, col, rec) {
						...
						d.html = TimeUtils.getNaturalDuration(
							Math.abs(s - d),
							1
						);
...

				{
					...
					renderer: function (v, col, rec) {
						...
						var grade = historyItem && historyItem.get('Grade'),
							scoreTpl = Ext.DomHelper.markup({
								tag: 'input',
								type: 'text',
								value: historyItem && historyItem.get('grade'),
							}),
...

				{
					...
					renderer: function (v, col, rec) {
						const historyItem = rec.getMostRecentHistoryItem();
						var commentText =
							historyItem && historyItem.get('feedback') === 1
								? ' Comment'
								: ' Comments';
						...
						if (historyItem && historyItem.get('feedback')) {
							return Ext.DomHelper.markup({
								cls: 'feedback',
								html: historyItem.get('feedback') + commentText,
							});
						}
...

				{
					...
					renderer: function (v, col, rec) {
						const submission =
							rec.getMostRecentHistoryItem() &&
							rec.getMostRecentHistoryItem().get('submission');
						return (
							submission &&
							Ext.DomHelper.markup({ cls: 'actions' })
						);
					},
				},
			],
		...
		constructor: function (config) {
			...
			(config.nameOrder || me.nameOrder || []).forEach(function (
				name,
				i
			) {
				var col, index;
		...
		onItemClicked: function (v, record, dom, ix, e) {
			...
			if (nib) {
				AssignmentStatus.getActionsMenu(
					record,
					gradeUpdated,
					gradeUpdated
				).showBy(nib, 'tr-br');

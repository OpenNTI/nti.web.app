	...
	'NextThought.app.course.assessment.components.admin.ListHeader',
	{
				...
				cn: [
					{
						...
						cn: [
							{
								cls: 'title',
								html: '{assignmentTitle:htmlEncode}',
							},
							{
								cls: 'extras meta',
								cn: [
						...
						cn: [
							{
								tag: 'a',
								cls: 'request email',
								'data-qtip': 'Email Enrolled Students',
							},
							{ tag: 'a', cls: 'export' },
							{ tag: 'a', cls: 'settings' },
						],
		...
		afterRender: function () {
			...
			this.mon(
				this.filterEl,
				'click',
				this.fireEvent.bind(this, 'showFilters', this.filterEl)
			);
			this.mon(
				this.viewAssignmentEl,
				'click',
				this.fireEvent.bind(this, 'goToRawAssignment')
			);
			if (this.assignment && this.assignment.hasLink('edit')) {
				this.mon(
					this.assignmentDueEl,
					'click',
					this.toggleDateEditor.bind(this)
				);
			}
			this.emailEl.setVisibilityMode(Ext.dom.Element.DISPLAY);

			if (
				this.shouldAllowInstructorEmail() &&
				this.currentBundle &&
				this.currentBundle.getLink('Mail')
			) {
			...
			if (this.assignment && this.assignment.getLink('edit')) {
				this.mon(this.editAssignmentEl, 'click', () =>
					this.editAssignment()
				);
			} else {
		...
		onStudentFilterChange: function (filter) {
			...
			if (filter === 'ForCredit') {
				q = getString(
					'NextThought.view.courseware.assessment.admin.performance.Root.emailenrolled'
				);
			} else if (filter === 'Open') {
				q = getString(
					'NextThought.view.courseware.assessment.admin.performance.Root.emailopen'
				);
			} else if (filter === 'All') {
				q = getString(
					'NextThought.view.courseware.assessment.admin.performance.Root.emailall'
				);
			}
		...
		showEmailEditor: function (e) {
			...
			this.WindowActions.showWindow('new-email', null, e.getTarget(), {
				afterSave: this.onCourseEmailSent.bind(this),
			});
		},
		...
		showPageMenu: function (e) {
			if (!this.pageMenu || e.getTarget('.empty')) {
				return;
			}
		...
		updateLabelsForAssignment: function (assignment) {
			this.assignmentTitleEl.update(
				Ext.util.Format.htmlEncode(assignment.get('title'))
			);
			this.assignmentTitleEl.dom.setAttribute(
				'data-qtip',
				Ext.util.Format.htmlEncode(assignment.get('title'))
			);

			this.assignmentDueEl.update(
				AssignmentStatus.getStatusHTML({
					isDraft:
						!assignment.get('availableBeginning') &&
						!assignment.get('PublicationState'),
		...
		setAssignment: function (assignment) {
			...
			if (exportLink) {
				this.setExportURL(
					exportLink,
					getString(
						'NextThought.view.courseware.assessment.assignments.admin.Assignment.download'
					)
				);
			}
			...
			this.on(
				'destroy',
				this.assignmentStatusCmp.destroy.bind(this.assignmentStatusCmp)
			);
		},

		toggleDateEditor: function () {
			if (
				this.assignmentStatusCmp &&
				this.assignmentStatusCmp.dueDateEditorVisible()
			) {

...
const t = scoped(
	'nti-web-app.course.assessment.components.admin.performance.Student',
	{
...
module.exports = exports = Ext.define(
	'NextThought.app.course.assessment.components.admin.performance.Student',
	{
		...
		_showMask: function () {
			...
			this._maskIn = setTimeout(function () {
				if (el && el.dom) {
					el.mask(
						getString(
							'NextThought.view.courseware.assessment.admin.performance.Student.loading'
						),
						'loading',
						true
					);
		...
		updateHeader() {
			this.header.setGradeBook(
				this.summary.hasFinalGrade() && this.FinalGradeHistoryItem
			);
			this.header.setPredictedGrade(
				this.summary.get('PredictedGrade') || this.predictedGrade
			);
		},
		...
		setAssignmentsData: function (assignments) {
			...
			this.store = assignments.getStudentHistory(
				this.historiesURL,
				this.student.getId()
			);
		...
		setEnrollmentData: function (enrollment) {
			...
			if (mailLink) {
				this.header.onceRendered.then(function () {

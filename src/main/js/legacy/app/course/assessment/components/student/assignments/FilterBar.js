				...
				cn: [
					{
						...
						type: 'text',
						placeholder:
							'{{{nti-web-app.course.assessment.components.student.assignments.FilterBar.searchplaceholder}}}',
						required: 'required',
					},
					{ cls: 'clear' },
				],
		...
		createGroupByMenu: function () {
			...
			var type = this.currentGrouping,
				items = [
					...
					{
						text: getString(
							'NextThought.view.courseware.assessment.assignments.FilterBar.due'
						),
						groupBy: 'due',
						checked: type === 'due',
					},
					{
						text: getString(
							'NextThought.view.courseware.assessment.assignments.FilterBar.completion'
						),
						groupBy: 'completion',
			...
			if (this.shouldShowPublishOption) {
				items.push({
					text: getString(
						'NextThought.view.courseware.assessment.assignments.FilterBar.publicationstatus'
					),
					groupBy: 'publication',
					checked: type === 'publication',
				});
				items.push({
					text: getString(
						'NextThought.view.courseware.assessment.assignments.FilterBar.creationdate'
					),
					groupBy: 'creation',
		...
		setBundle(bundle) {
			...
			if (this.bundle.isScormCourse) {
				...
				if (this.groupLabelEl) {
					this.groupLabelEl.dom.innerHTML =
						this.groupByItems[0] && this.groupByItems[0].text;
				}
			}
		},
	}

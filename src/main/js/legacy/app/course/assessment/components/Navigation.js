	...
	'NextThought.app.course.assessment.components.Navigation',
	{
		...
		afterRender: function () {
			...
			if (this.assignmentsOptionsEl) {
				this.assignmentsOptionsEl.on(
					'click',
					this.showOptionsMenu.bind(this)
				);
		...
		updateActive: function (item, route) {
			...
			var current = this.el.down(
					'.outline-list .outline-row.x-item-selected'
				),
		...
		addItems: function (items) {
			...
			(items || []).forEach(function (item) {
				me.componentMapping[item.xtype] = me.itemTpl.append(
					me.outlineEl,
					{
						route: item.route,
						title: item.title,
		...
		onClick: function (e) {
			...
			if (!item || Ext.fly(item).hasCls('disabled')) {
				return;
			}
			...
			this.fireEvent(
				'select-route',
				item.getAttribute('data-qtip'),
				isSelected ? root : route
			);
		},
	}
);

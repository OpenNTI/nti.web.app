...
module.exports = exports = Ext.define(
	'NextThought.app.course.dashboard.components.tiles.Post',
	{
						...
						cn: [
							{ cls: 'avatar-wrapper', cn: ['{Creator:avatar}'] },
							{
								...
								cn: [
									...
									{
										tag: 'span',
										cls: 'list-item shared-with',
										html: '{SharedWith}',
									},
									{
										tag: 'span',
										cls: 'list-item created',
										html: '{CreatedTime}',
									},
								],
							},
		...
		afterRender: function () {
			...
			me.getContext().then(me.callWhenRendered.bind(me, 'setContext'));
		...
		itemClicked: function (e) {
			if (
				!e.getTarget('.body-container') &&
				!e.getTarget('.controls') &&
		...
		getCreator: function () {
			...
			if (!Ext.isString(creator)) {
				return creator;
			}

			return UserRepository.getUser(creator).then(function (user) {
		...
		setSharedWith: function (value) {
			var me = this,
				sharingInfo = SharingUtils.sharedWithToSharedInfo(
					SharingUtils.resolveValue(value)
				);

			SharingUtils.getLongTextFromShareInfo(sharingInfo, null, 2).then(
				function (text) {
					if (me.sharedWithEl) {
		...
		updateBody: function () {
			this.getBody().then(this.setBody.bind(this));
		},
		...
		setCommentCount: function (value) {
			this.commentCountEl.update(
				Ext.util.Format.plural(value, 'Comment')
			);
		},

		setContext: function (context) {
			if (Ext.isEmpty(context) || Ext.Object.isEmpty(context)) {
				return;
			}
			...
			if (context.type && this.contextEl) {
				show = true;
				context = Ext.apply(context, {
					renderTo: this.contextEl,
					width: me.width,
				});

...
module.exports = exports = Ext.define(
	'NextThought.app.course.dashboard.Index',
	{
		...
		emptyStateTpl: new Ext.XTemplate(
					...
					cn: [
						{
							cls: 'title',
							html: getString(
								'NextThought.view.courseware.dashboard.View.EmptyActivityTitle'
							),
						},
						{
							cls: 'sub',
							html: getString(
								'NextThought.view.courseware.dashboard.View.EmptyActivitySubTitle'
							),
						},
					],
				},
		...
		onRouteActivate: function () {
			...
			this.reloadTiles().then(function () {
		...
		bundleChanged: function (bundle) {
			var id = bundle && bundle.getId(),
				courseCatalog,
				date = this.self.DATE_OVERRIDE || new Date();
		...
		maybeFinishInitialLoad: function () {
			...
			if (!this.rendered || this.loaded || !this.course) {
				return;
			}
		...
		queryTiles: function (startDate, endDate, isCurrent) {
			...
			Ext.Object.each(widgets, function (clsName, cls) {
				if (cls.getTiles) {
					tiles.push(
						cls.getTiles(course, startDate, endDate, isCurrent)
					);
				}
			});
		...
		maybeLoadNextWeek: function (scrollInfo, forced) {
			var scrolledDown =
				scrollInfo.scrollTop + this.loadThreshold >
				scrollInfo.scrollHeight - scrollInfo.offSetHeight;
		...
		__loadNextWeek: function () {
			...
			tileContainer = this.add({
				xtype: 'dashboard-tile-container',
				loadTiles: this.queryTiles.bind(
					this,
					week.start,
					week.end,
					isCurrent
		...
		maybeDisplayEmptyState: function () {
			...
			if (isEmpty) {
				this.emptyState = Ext.get(
					this.emptyStateTpl.append(this.getTargetEl())
				);
			}
		},

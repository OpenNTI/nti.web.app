	...
	'NextThought.app.course.overview.components.editing.itemselection.Item',
	{
		...
		initComponent: function () {
			...
			if (this.selectionItem.isModel) {
				this.mon(this.selectionItem, 'update', x =>
					this.onSelectionItemUpdated(x)
				);
			}
		},
		...
		onItemClick: function (e) {
			...
			if (
				this.isItemDisabled &&
				this.isItemDisabled(this.selectionItem, e)
			) {
				return;
			}
			...
			if (editLink) {
				...
				const container = this.up(
					'overview-editing-video-item-selection'
				);
		...
		applySearchTerm: function (term) {
			if (
				...
				this.itemMatchesSearch(this.selectionItem, term)
			) {

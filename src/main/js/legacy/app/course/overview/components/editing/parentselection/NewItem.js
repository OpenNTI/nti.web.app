...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.parentselection.NewItem',
	{
		...
		maybeClearErrors() {
			...
			this.activeErrors = (this.activeErrors || []).reduce(
				(acc, error) => {
		...
		onSave: function () {
			...
			if (!this.parentRecord || !this.parentRecord.appendContent) {
				return;
			}

			this.clearErrors();

			this.doValidation().then(() => {
				this.addMask();
				...
				return this.parentRecord
					...
					.then(function (result) {
						...
						if (duration < minWait) {
							return wait(minWait - duration).then(function () {
								return result;
							});
						}
					...
					.catch(function (error) {
						...
						if (error) {
							console.error(
								'Unable to create section because: ' + error
							);
						}
					});
			});

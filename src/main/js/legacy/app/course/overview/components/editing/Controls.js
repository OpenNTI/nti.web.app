	...
	'NextThought.app.course.overview.components.editing.Controls',
	{
		...
		setPublishState: function () {
			...
			if (isNodePublished && isLessonPublished) {
				...
				console.warn(
					'Not expected. The node should be published if its lesson is published. ',
					node,
					lesson
				);
			}
		...
		alignPublishingMenu: function (menuContainer) {
			var box =
					(menuContainer &&
						menuContainer.dom.getBoundingClientRect()) ||
					{},
		...
		doEdit: function () {
			this.PromptActions.prompt('overview-editing', {
				record: this.record,
		...
		doAdd: function () {
			this.PromptActions.prompt('overview-creation', {
				parent: this.contents || this.record,
				root: this.root,
			});
		},
	}

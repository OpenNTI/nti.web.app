...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.controls.AvailableDateMenu',
	{
				...
				cn: [
					{
						...
						cn: [
							{
								cls: 'part selected beginning',
								'data-action': 'AvailableBeginning',
								html: 'Begin Date',
							},
							{
								cls: 'part ending',
								'data-action': 'AvailableEnding',
								html: 'Finish Date',
							},
						],
					},
		...
		setInitialState: function () {
			var startDate =
					this.record && this.record.get('AvailableBeginning'),
				endDate = this.record && this.record.get('AvailableEnding');

			// the available beginning and ending are in seconds.
			this.defaultValues = {
				AvailableBeginning: startDate
					? startDate.getTime && startDate.getTime() / 1000
					: this.getDefaultTimeForField('AvailableBeginning'),
				AvailableEnding: endDate
					? endDate.getTime && endDate.getTime() / 1000
					: this.getDefaultTimeForField('AvailableEnding'),
			};
		...
		updateDates: function () {
			var selectedEl = this.toggleEl.down('.selected'),
				field =
					selectedEl &&
					selectedEl.dom &&
					selectedEl.dom.getAttribute('data-action'),
				value = this.values && this.values[field],
				date;
		...
		dateChanged: function (datepicker) {
			var seconds = datepicker && datepicker.getValue(),
				...
				key =
					selected &&
					selected.dom &&
					selected.dom.getAttribute('data-action');
		...
		getValue: function () {
			return {
				AvailableBeginning:
					this.values && this.values.AvailableBeginning,
		...
		doSave: function (e) {
			...
			if (target && target.hasCls('disabled')) {
				return;
			}
			if (!this.isValid()) {
				return;

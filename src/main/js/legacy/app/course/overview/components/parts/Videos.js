...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.parts.Videos',
	{
		...
		alias: [
			'widget.course-overview-video',
			'widget.course-overview-ntivideo',
		],
							...
							cn: [
								{
									...
									cn: [
										...
										},
										{
											cls: 'label',
											'data-qtip':
												'{{{NextThought.view.courseware.overview.parts.Videos.playtranscript}}}',
										},
										{
											cls: 'launch-player',
											'data-qtip':
												'{{{NextThought.view.courseware.overview.parts.Videos.play}}}',
										},
									],
								},
							],
		...
		constructor: function (config) {
			...
			if (i) {
				...
				Ext.callback(this.getVideoDataLoadedCallback(), this, [
					undefined,
					0,
				]);
			}
		...
		applyVideoData: function (videoIndex) {
			...
			var reader = Ext.data.reader.Json.create({ model: PlaylistItem }),
				me = this,
				selected = this.getSelectionModel().selected,
				toRemove = [],
				count,
				store = this.getStore();
			...
			try {
				...
				if (!Ext.isEmpty(toRemove)) {
					console.warn(
						'Droping ',
						toRemove.length,
						' records that arent in video index'
					);
				...
				if (count === 0) {
					console.error(
						'Destroying video widget because there are no videos to play'
					);
					this.destroy();
				}
			} catch (e) {
				console.warn(
					'Coud not load video index because:',
					e.stack || e.message || e
				);
			} finally {
				Ext.callback(this.getVideoDataLoadedCallback(), this, [
		...
		convertItems: function (items) {
			...
			Ext.each(items, function (item) {
				...
				out.push({
					...
					comments:
						getString(
							'NextThought.view.courseware.overview.parts.Videos.loading'
						) + ' ',
					date: r && r.get('date'),
				});
			});
		...
		afterRender: function () {
			...
			if (this.screenEl) {
				this.screenEl.setVisibilityMode(
					Ext.isIE ? Ext.Element.OFFSETS : Ext.Element.VISIBILITY
				);
			}
			if (this.curtainEl) {
				this.curtainEl.setVisibilityMode(
					Ext.isIE ? Ext.Element.OFFSETS : Ext.Element.VISIBILITY
				);
			}
		...
		onPlayerError: function () {
			...
			alert(
				getString(
					'NextThought.view.courseware.overview.parts.Videos.error'
				)
			);
		},
		...
		onSelectChange: function (s, rec) {
			...
			if (this.curtainEl) {
				...
				this.playLabelEl.dom.setAttribute(
					'data-qtip',
					getString(
						'NextThought.view.courseware.overview.parts.Videos.playtranscript'
					)
			...
			if (!e.getTarget('.launch-player') && e.getTarget('.transcripts')) {
				if (me.player) {
					if (me.player.isPlaying()) {
						...
						console.warn(
							'Player did not report being in a state where the media viewer would interfere'
						);
					}

					if (me.hasCls('playing')) {
						return;
					}
				}

				this.course.getVideoForId(m.getId()).then(function (video) {
					slidedeck =
						(video && video.slidedeck) || m.get('slidedeck');
		...
		navigateToTarget: function (videoItem, basePath) {
			...
			this.course.getVideoForId(videoItem.getId()).then(function (o) {
				var video = PlaylistItem.create(
					Ext.apply({ NTIID: o.ntiid }, o)
				);
		...
		navigateToSlidedeck: function (slidedeckId, startVideo) {
			var me = this;
			if (slidedeckId) {
				Service.getObject(slidedeckId).then(function (slidedeck) {

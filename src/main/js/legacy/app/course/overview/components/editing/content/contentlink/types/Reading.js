...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.content.contentlink.types.Reading',
	{
		...
		maybeShowReadingList(selection) {
			ContentUtils.getContentPackageContainingReading(
				...
				.then(contentPackage => {
					...
					return ContentUtils.getReadings(
						this.bundle,
						false,
		...
		createReading() {
			...
			contentTemplate.getAttribute = a => {
				return contentTemplate[a];
			};
		...
		getContentPackageSelection() {
			...
			if (this.newlyCreatedReading) {
				...
				getContentPackage = Promise.resolve(
		...
		getReadingSelection: function () {
			...
			if (this.newlyCreatedReading) {
				...
				getReading = Promise.resolve(
					this.readingSelectionCmp.getSelection()[0]
				);
			} else if (this.record) {
				getReading = ContentUtils.getReading(
					this.record.get('href'),
					this.bundle
				);
			} else {
			...
			Promise.all([
				...
				.then(function (results) {
					...
					me.readingEditorCmp = me.add({
						...
						doClose: me.doClose,
						onChangeReading: () =>
							me.maybeShowReadingList(selection),
		...
		doValidation: function () {
			return this.readingEditorCmp
				? this.readingEditorCmp.doValidation()
				: Promise.resolve();
		},
		...
		onSave: function () {
			...
			if (me.newlyCreatedReading) {
				...
				return me.ContentActions.createReading(
					me.bundle,
					me.newlyCreatedReading
				).then(pack => {
					me.readingEditorCmp.contentPackage = pack;

					me.readingEditorCmp.formCmp.setValue(
						'href',
						pack.get('NTIID')
					);
					me.readingEditorCmp.formCmp.setValue(
					...
					return me.readingEditorCmp
						.onSave()
						.then(rec => {
							// navigate to newly created content's editor (still unpublished at this point)
							const route =
								`/course/${encodeForURI(this.bundle.getId())}` +
								(me.outlineNode
									? `/lessons/${encodeForURI(
											me.outlineNode.get('NTIID')
									  )}`
									: '') +
								`/content/${encodeForURI(
									pack.get('NTIID')
								)}/edit/`;
				...
				return me.readingEditorCmp.onSave().catch(function (reason) {
					me.enableSubmission();
					return Promise.reject(reason);
				});
			}

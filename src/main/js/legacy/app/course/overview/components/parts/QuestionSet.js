	...
	'NextThought.app.course.overview.components.parts.QuestionSet',
	{
		...
		constructor: function (config) {
			...
			config = Ext.apply(config, {
				...
				questionSetId: ntiid,
				total:
					n.getAttribute('question-count') ||
					(rec && rec.get('question-count')),
			});
			...
			if (config.xtype !== 'course-overview-assignment') {
				...
				console.warn(
					'Hidding Assignment widget. Assignmet was null. %o',
					config
				);
				me.hide();
			}
		...
		getButton: function () {
			...
			return {
				xtype: 'button',
				text: getString(
					'NextThought.view.courseware.overview.parts.QuestionSet.review'
				),
				ui: 'secondary',
				scale: 'large',
				cls: 'review-btn',
		...
		buildForAssessment: function () {
			...
			ContentUtils.getLineage(ntiid, me.course)
				.then(function (lineages) {
					...
					req.url = Service.getContainerUrl(
						containerId,
		...
		buildForAssignment: function () {
								...
								cn: [
									{
										cls: 'label',
										tag: 'span',
										html: this.assignment.get('title'),
									},
									{
										cls: 'points',
										tag: 'strong',
			...
			this.mon(
				this.assignment,
				'update',
				this.setAsAssignment.bind(this, this.assignment)
			);
		...
		setAsAssignment: function (assignment) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.setAsAssignment, this, arguments),
					this,
					{ single: true }
				);
				return;
			...
			assignment.getHistory().then(this.setHistory.bind(this));
			...
			if (dueDate && dueDate < now) {
				...
				if (status) {
					status.setStatus(
						getFormattedString(
							'NextThought.view.courseware.overview.parts.QuestionSet.available',
							{
			...
			this.isAvailable =
				this.isAvailable || assignment.hasLink('PracticeSubmission');
		},
		...
		setHistory: function (history) {
			...
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.setHistory, this, arguments),
					this,
					{ single: true }
				);
				return;
			...
			var status = this.down('course-assignment-status'),
				...
				completed =
					(submission && submission.get('CreatedTime')) || new Date(),
		...
		containerLoaded: function (q, s, r) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.containerLoaded, this, arguments),
					this,
					{ single: true }
				);
				return;
		...
		setAsNotStarted: function () {
			...
			if (b) {
				...
				b.setText(
					getString(
						'NextThought.view.courseware.overview.parts.QuestionSet.start'
					)
				);
		...
		updateWithScore: function (correct, total) {
			...
			if (score && !isNaN(correct)) {
				score.setValue(
					Math.floor((100 * correct) / (total || this.getTotal())) ||
						0
				);
			}
			this.updateLayout();

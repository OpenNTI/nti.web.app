	...
	'NextThought.app.course.overview.components.editing.content.video.Editor',
	{
		...
		statics: {
			getHandledMimeTypes: function () {
				return [Video.mimeType, VideoRoll.mimeType];
			},
		...
		__getExcludedVideos: function (videos) {
			var siblings = this.parentRecord
				? this.parentRecord.get('Items')
				: [];

			return siblings
				.reduce(function getVideoIds(acc, item) {
		...
		showVideoList: function (selectedItems, onEdit) {
			...
			me.addVideoBtn = me.add({
				...
				autoEl: {
					tag: 'div',
					cls: 'create-video-overview-editing',
					html: 'Add Video',
				},
				listeners: {
					click: {
			...
			me.videoSelectionCmp = me.add({
				...
				getExcludedVideos: (...args) =>
					this.__getExcludedVideos(...args),
		...
		pickVideo(ntiid, onEdit) {
			...
			this.PromptActions.prompt('video-picker', {
				bundle: this.bundle,
				video: ntiid,
		...
		showVideoEditor: function () {
			...
			this.videoEditorCmp = this.add({
				...
				selectedItems:
					this.videoSelectionCmp &&
					this.videoSelectionCmp.getSelection(),
		...
		onSave: function () {
			...
			return me.videoEditorCmp.onSave().catch(function (reason) {
				me.enableSubmission();

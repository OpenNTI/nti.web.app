...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.parentselection.Index',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				cls: 'selection closed disabled',
				'data-label': '{selectionLabel}',
				cn: [{ cls: 'active' }],
			},
			{
				cls: 'position closed disabled',
		...
		afterRender: function () {
			...
			this.selectionMenu = this.buildMenu(
				this.selectionItems,
				this.getEditor(),
				this.rootRecord
			);
			...
			if (this.lockedPosition) {
				...
				this.selectRecord(
					this.originalSelection || this.selectionItems[0]
				);
			}
			...
			this.mon(
				this.activeEl,
				'click',
				this.toggleSelectionMenu.bind(this)
			);
			this.mon(
				this.activePositionEl,
				'click',
				this.togglePositionMenu.bind(this)
			);
		},
		...
		selectRecord: function (record) {
			...
			if (!record) {
				...
				index =
					record.indexOfId &&
					this.editingRecord &&
					record.indexOfId(this.editingRecord.getId());

				if (record.getItemsCount) {
					this.positionMenu.setTotalPositions(
		...
		onBodyClick: function (e) {
			var onSelection = e.getTarget('.overview-editing-parentselection'),
				onSelectionMenu = e.getTarget(
					'.overview-editing-parentselection-menu'
				),
				onPositionMenu = e.getTarget(
					'.overview-editing-parentselection-position-menu'
				);

			onSelection =
				onSelection &&
				(e.getTarget('.selection') || e.getTarget('.position'));
			onSelection =
				onSelection && (e.getTarget('.active') || e.getTarget('.menu'));
		...
		getOriginalIndex: function () {
			if (
				...
				this.originalSelection.indexOfId &&
				this.editingRecord
			) {
				return this.originalSelection.indexOfId(
					this.editingRecord.getId()
				);
			}
		},

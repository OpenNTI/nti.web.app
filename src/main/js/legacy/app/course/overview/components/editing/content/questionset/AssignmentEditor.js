	...
	'NextThought.app.course.overview.components.editing.content.questionset.AssignmentEditor',
	{
		...
		addPreview: function (item) {
			var me = this,
				...
				parts = [
					{
						cls: 'title',
						html: Ext.util.Format.htmlEncode(item.get('title')),
					},
					{
						cls: 'due-date',
						html: Ext.Date.format(dueDate, 'l, F j, g:i a T'),
					},
				];
		...
		hasRecordChanged: function (values) {
			...
			if (!this.record) {
				...
				this.record.get('Target-NTIID') !== values['Target-NTIID']
			) {
				changed = true;
			}
		...
		onSave: function () {
			...
			return me.EditingActions.saveValues(
				values,
				me.record,
				originalPosition,
				currentPosition,

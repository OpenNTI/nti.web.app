...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.View',
	{
		...
		closeEditing: function () {
			...
			outlineInterface
				...
				.then(function (builtOutlineInterface) {
					var outline =
							builtOutlineInterface &&
							builtOutlineInterface.getOutline(),
		...
		openAuditLog: function () {
			if (
				this.currentBundle &&
				this.currentBundle.getLink('recursive_audit_log')
			) {
				this.PromptActions.prompt('audit-log', {
					parent: this,
					record: this.currentBundle,
				});
			}
		},
		...
		getNewestOutline(editing) {
			const cacheKey = editing
				? 'newest-outline-editing-cache'
				: 'newest-outline-cache';
		...
		updateOutline: function (editing, doNotCache) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					() => this.updateOutline(editing, doNotCache),
					this,
					{ single: true }
				);
				return;
			...
			var me = this,
				bundle = me.currentBundle,
				outlineInterface = editing
					? bundle.getAdminOutlineInterface(doNotCache)
					: bundle.getOutlineInterface(doNotCache);
		...
		bundleChanged: function (bundle) {
			...
			if (hadBundle) {
				const isEditingBundle =
					this.editingMap && this.editingMap[bundle.getId()];
		...
		__getRecord: function (id, record, editing, doNotCache) {
			var outline =
				editing || doNotCache
					? this.updateOutline(editing, doNotCache)
					: this.activeOutlineInterface ||
					  this.updateOutline(editing, doNotCache);

			return outline.onceBuilt().then(builtOutline => {
				let rec;
				...
				if (!rec) {
					...
					return newestOutline.onceBuilt().then(builtNewest => {
		...
		showOutlineNode: function (route, subRoute) {
			...
			return this.__getRecord(
				id,
				route.precache.outlineNode,
				false,
				changedEditing
			).then(record => {
				this.unmask();
				...
				const selectedRecord = this.navigation.selectRecord(
					record,
					true
				);
		...
		showEditOutlineNode: function (route /*, subRoute*/) {
			...
			const id =
				route.params &&
				route.params.node &&
				decodeFromURI(route.params.node);
			const changedEditing = !this.isEditing;
			...
			return this.__getRecord(
				...
				return this.body
					.editOutlineNode(theRecord, changedEditing)
					.then(() => this.alignNavigation())
					.then(() => theRecord);
			});
		},

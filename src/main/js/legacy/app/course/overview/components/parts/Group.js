	...
	'NextThought.app.course.overview.components.parts.Group',
	{
		...
		setCollection: function (collection) {
			...
			function getType(i) {
				return (
					'course-overview-' +
					(i.MimeType || i.type || '').split('.').last()
				);
			}
			function getClass(t) {
				return t && Ext.ClassManager.getByAlias('widget.' + t);
			}
			...
			items = (items || []).reduce(function (accItems, record) {
				...
				if (cls.isSection) {
					...
					if (item && item.assignment) {
						accItems.push(
							assignments
								.fetchAssignment(item['target-ntiid'])
								...
								.catch(function (reason) {
									...
									item.assignment = assignments.getItem(
										item['target-ntiid']
									);
			...
			Promise.all(items).then(function (newItem) {
				body.add(newItem);

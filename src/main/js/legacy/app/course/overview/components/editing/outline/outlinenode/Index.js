...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.outline.outlinenode.Index',
	{
			...
			const update = () => {
				const findRecord = this.outlineInterface
					? this.outlineInterface
							.onceBuilt()
							.then(x => x.findOutlineNode(this.record.getId()))
				...
				findRecord
					...
					.then(() =>
						this.showOutlineNode(this.record, this.parentRecord)
					)
					.then(() => {
						if (this.rendered) {
							this.el.unmask();
		...
		onceLoaded: function () {
			...
			return (me.loadContents || Promise.resolve()).then(function () {
		...
		showOutlineNode: function (record, parentRecord) {
			...
			return Promise.all([
				...
				if (!me.hideItemsIfEmpty || (items && items.length)) {
					cmps.push(
						me.getItemsConfig(items, record, outline, bundle)
					);
				}
		...
		onRouteDeactivate() {
			const preview = this.down(
				'overview-editing-outline-outlinenode-preview'
			);

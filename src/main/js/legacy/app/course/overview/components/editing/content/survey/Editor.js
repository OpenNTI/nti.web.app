...
const t = scoped(
	'nti-web-app.course.overview.components.editing.content.survey.Editor',
	{
...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.content.survey.Editor',
	{
		...
		getSelectionFromRecord(record) {
			return this.bundle.getAllSurveys().then(function (surveys) {
		...
		async createSurvey() {
			...
			try {
				const survey = await this.AssessmentActions.createSurveyIn(
					this.bundle
				);
				const ref = {
				...
				await this.EditingActions.saveValues(
					ref,
					null,
					null,
					{
				...
				const route = `/course/${encodeForURI(
					...
					outlineNode.getId()
				)}/content/${encodeForURI(survey.getId())}/edit`;

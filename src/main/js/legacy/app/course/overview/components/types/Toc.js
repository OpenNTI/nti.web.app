...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.types.Toc',
	{
		...
		SECTION_TITLE_MAP: {
			...
			discussions: getString(
				'NextThought.view.courseware.overview.View.discussion'
			),
			additional: getString(
				'NextThought.view.courseware.overview.View.additional'
			),
			required: getString(
				'NextThought.view.courseware.overview.View.required'
			),
			assessments: getString(
				'NextThought.view.courseware.overview.View.assessment'
			),
			'session-overview': getString(
				'NextThought.view.courseware.overview.View.session'
			),
			assignments: getString(
				'NextThought.view.courseware.overview.View.assignments'
			),
		},
		...
		buildFromToc: function (node, locInfo, assignments, course) {
			...
			Ext.each(children, function (i) {
				...
				if (i.isModel) {
					...
					if (
						/^object$/i.test(i.tagName) &&
						i.getAttribute('mimeType') ===
							'application/vnd.nextthought.relatedworkref'
					) {
						return;
					}

					i = me.getComponentForNode(
						i,
						locInfo,
						node,
						assignments,
					...
					if (t) {
						if (i.xtype !== 'course-overview-topic') {
							...
							if (!c) {
								c = sections[t] = {
									xtype:
										SECTION_CONTAINER_MAP[t] ||
										'course-overview-section',
									type: t,
									title:
										SECTION_TITLE_MAP[t] || 'Section ' + t,
									items: [],
								};
								items.push(c);
							}
			...
			this.add(
				[{ xtype: 'course-overview-header', record: node }].concat(
					items
				)
			);
		},
		...
		getComponentForNode: function (node, info, rec, assignments, course) {
			...
			if (/^content:related$/i.test(type) || /^object$/i.test(type)) {
				type =
					node.getAttribute('type') || node.getAttribute('mimeType');
				type =
					type &&
					type.replace(/^application\/vnd\.nextthought\./, '');
			}
			...
			if (cls) {
				if (cls && cls.isAssessmentWidget) {
					assignment = assignments.isAssignment(
						node.getAttribute('target-ntiid')
					);
					type = assignment ? 'course-overview-assignment' : type;
					assignment = assignments.getItem(
						node.getAttribute('target-ntiid')
					);
				}

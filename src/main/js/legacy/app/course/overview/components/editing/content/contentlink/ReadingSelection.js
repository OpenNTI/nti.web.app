				...
				cn: [
					{
						tag: 'tpl',
						if: 'hasChildren',
						cn: [{ cls: 'expand' }, { cls: 'icon folder' }],
					},
					{
						tag: 'tpl',
						if: '!hasChildren',
						cn: [{ cls: 'expand hidden' }, { cls: 'icon file' }],
					},
								...
								cn: [
									{
										...
										type: 'checkbox',
										name: 'end-date',
										id: 'filter-readings-checkbox-' + me.id,
									},
									{
										tag: 'label',
										for:
											'filter-readings-checkbox-' + me.id,
										html: 'Do not filter',
									},
								],
							},
			...
			me.breadcrumbCmp.add(
				path.map(function (part) {
					...
					if (part.tagName === 'toc') {
						...
						hasChildren =
							(me.getItemChildren(part) || []).length > 0;
					}
		...
		onSearchCleared: function () {
			...
			me.itemsContainer.items.each(function (item) {
				var selectionItem = item.selectionItem,
					selectionId =
						selectedItem && me.getSelectionItemId(selectionItem);

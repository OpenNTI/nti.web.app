	...
	'NextThought.app.course.overview.components.editing.auditlog.Item',
	{
		...
		beforeRender: function () {
			...
			const externalValues =
				(record && record.get('ExternalValue')) || {};
			...
			let fields = attributes.map(attr => {
				if (externalValues[attr]) {
					if (attr.substr(0, 9) === 'Available') {
						return `${
							me.FIELDS[attr.toLowerCase()] || attr
						} to "${DateTime.format(
							Parsing.parseDate(externalValues[attr]),
							DateTime.MONTH_DAY_TIME
						)}"`;
					} else if (typeof externalValues[attr] !== 'object') {
						return `${me.FIELDS[attr.toLowerCase()] || attr} to "${
							externalValues[attr]
						}"`;
					}
				}
			...
			if (
				[VideoRoll.mimeType, Video.mimeType].includes(
					recordable.MimeType
				)
			) {
				msg = `${type} ${title}.`;
			} else if (isChild && title !== '') {
				msg = `${type} ${fields.join(', ')} ${getMsgContext(
					type,
					title
				)}.`;
			} else {
		...
		setUser: function (user) {
			...
			if (me.rendered) {
				...
				me.mon(
					me.userObject,
					'avatarChanged',
					me.setAvatar.bind(me, me.userObject)
				);
			} else {
				me.mon(
					me.userObject,

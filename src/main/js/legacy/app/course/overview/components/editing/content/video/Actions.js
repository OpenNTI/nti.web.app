...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.content.video.Actions',
	{
		...
		__removeAndAdd: function (
			...
			if (oldParent) {
				save = oldParent.removeRecord(oldRecord).then(function () {
					if (newParent) {
						return newParent.insertContent(
							newValues,
							newPosition.index
						);
					}
		...
		__convertRollToSingle: function (
			videos,
			record,
			originalPosition,
			newPosition,
		...
		__convertSingleToRoll: function (
			videos,
			record,
			originalPosition,
			newPosition,
		...
		__updateVideoRoll: function (
			...
			if (videos.length === 1) {
				return this.__convertRollToSingle(
					videos,
					record,
					originalPosition,
					newPosition,
		...
		__updateSingleVideo: function (
			...
			if (videos.length > 1) {
				return this.__convertSingleToRoll(
					videos,
					record,
					originalPosition,
					newPosition,

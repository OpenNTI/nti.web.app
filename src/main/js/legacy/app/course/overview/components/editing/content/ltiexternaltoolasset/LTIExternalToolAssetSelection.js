	...
	'NextThought.app.course.overview.components.editing.content.ltiexternaltoolasset.LTIExternalToolAssetSelection',
	{
		...
		itemTpl: new Ext.XTemplate(
			Ext.DomHelper.markup({
				cls: 'ltiexternaltoolasset-item assignment-item {cls}',
				cn: [
					{
						tag: 'tpl',
						if: 'icon',
						cn: [{ cls: 'icon', tag: 'img', src: '{icon}' }],
					},
					{
						tag: 'tpl',
						if: '!icon',
						cn: [{ cls: 'icon-fallback icon-hyperlink', tag: 'i' }],
					},
					{
						cls: 'meta',
						cn: [
		...
		itemMatchesSearch: function (item, searchTerm) {
			return (
				this.checkForMatch(item.ID, searchTerm.toLowerCase()) ||
				this.checkForMatch(item.description, searchTerm.toLowerCase())
			);
		},

...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.parts.VideoRollItem',
	{
		...
		initComponent: function () {
			...
			const targetId =
				this.video.get('Target-NTIID') || this.video.get('NTIID');
			...
			const onChange = async value => {
				...
				this.courseInstance
					...
					.fireEvent('requiredValueChanged', {
						ntiid: targetId,
						value,
					});
			};
			...
			if (this.courseInstance.get('CompletionPolicy')) {
				this.courseInstance
					.get('CompletionPolicy')
					.on('requiredValueChanged', ({ ntiid, value }) => {
			...
			let container = this.add({
				...
				items: [
					{
						xtype: 'component',
						html: `<span class="label" data-qtip="${Ext.String.htmlEncode(
							this.video.get('label')
						)}">${this.video.get('label')}</span>`,
					},
					{
						xtype: 'component',
						cls: 'viewed',
			...
			if (this.inEditMode && isCompletableObject) {
				const transformed = {
					IsCompletionDefaultState: this.video.get(
						'IsCompletionDefaultState'
					),
					CompletionRequired: this.video.get('CompletionRequired'),
					CompletionDefaultState: this.video.get(
						'CompletionDefaultState'
					),
				};
				...
				!this.inEditMode &&
				isCompletableObject &&
				this.video.get('CompletionRequired')
			) {
				container.add({
					xtype: 'component',
					cls: 'required-value',

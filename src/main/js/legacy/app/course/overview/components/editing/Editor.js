	...
	'NextThought.app.course.overview.components.editing.Editor',
	{
		...
		ERRORS: {
			MaxFileSizeUploadLimitError: function (reason) {
				...
				fileSize =
					fileSize && FilePicker.getHumanReadableFileSize(fileSize);
		...
		showEditor: function () {
			...
			this.headerCmp = this.addHeaderCmp();
		...
		getSchema: function () {
			...
			return this.EditingActions.getSchema(this.record).then(function (
				schema
			) {
				me.schema = schema;
				return schema;
		...
		addDeleteButton: function () {
			if (this.record.getLink('edit')) {
				return this.add({
					...
					messageOverride: this.getDeleteMessage
		...
		maybeAddSwitchTypeButton: function () {
			if (
				...
				this.editorGroup &&
				this.editorGroup.getEditorCount() > 0
			) {
				return this.add({
		...
		doSwitchType: function () {
			if (this.switchRecordType) {
				this.switchRecordType(
					this.editorGroup,
					this.formCmp && this.formCmp.getValues(),
					this.visibilityCmp && this.visibilityCmp.getValue()
				);
			}
		},
		...
		isValid: function () {
			...
			if (
				...
				!this.parentSelection.getCurrentSelection()
			) {
				valid = false;
			}
		...
		maybeClearErrors: function () {
			...
			this.activeErrors = (this.activeErrors || []).reduce(function (
				acc,
				error
			) {
		...
		onSaveFailure: function (reason) {
			if (!reason || typeof reason === 'string') {
				return this.setErrorOn(null, this.UNKNOWN_ERROR);
			}

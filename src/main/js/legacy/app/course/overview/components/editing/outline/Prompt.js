	...
	'NextThought.app.course.overview.components.editing.outline.Prompt',
	{
		...
		inheritableStatics: {
			...
			getInlineEditors: function () {
				return [OutlinenodeInlineEditor, ContentnodeInlineEditor];
			},
			...
			setUpChildCreators: function () {
				...
				this.CREATORS = creators.reduce(function (acc, item) {
					var handled = item.getHandledMimeTypes
						? item.getHandledMimeTypes()
						: [];

					handled.forEach(function (type) {
						if (acc[type]) {
							console.warn(
								'Overriding type picker form mimetype: ',
								type
							);
						}
			...
			setUpEditors: function () {
				...
				this.EDITORS = editors.reduce(function (acc, editor) {
					...
					handled.forEach(function (type) {
						if (acc[type]) {
							console.warn(
								'Overriding editor for mimetype: ',
								type
							);
						}
						acc[type] = editor;
			...
			setUpTypeSwitchers: function () {
				...
				this.TYPE_SWITCHERS = editors.reduce((acc, item) => {
					const handled = item.getHandledMimeTypes
						? item.getHandledMimeTypes()
						: [];
					const typeSwitcher =
						item.getTypeSwitcher && item.getTypeSwitcher();
					...
					for (let handle of handled) {
						if (acc[handle]) {
							console.warn(
								'Overriding type switcher for mimetype: ',
								handle
							);
						}
		...
		editRecord: function (
			...
			if (
				record.get('MimeType').match(/webinarasset/) ||
				record.get('MimeType').match(/nticalendareventref/)
			) {
				this.hideFooter();
			}
		...
		addRecord: function (parentRecord, rootRecord, bundle, outlineNode) {
			...
			if (!cmp) {
				console.error(
					'No editor to add child to: ',
					parentRecord.mimeType
				);
				return;
			}

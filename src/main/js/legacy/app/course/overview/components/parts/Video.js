...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.parts.Video',
	{
		...
		constructor: function () {
			...
			this.getVideo().then(this.addVideo.bind(this));
		},
		...
		addVideo: function (video) {
			...
			this.playlist = [
				PlaylistItem.create({
					mediaId:
						this.video.title ||
						this.video.get('title') ||
						this.video.get('label'),
					sources: this.video.sources || this.video.get('sources'),
					NTIID: this.video.ntiid || this.video.getId(),
					slidedeck:
						this.video.slidedeck ||
						(this.video.get && this.video.get('slidedeck')) ||
						'',
					transcripts:
						this.video.transcripts || this.video.get('transcripts'),
				}),
			];
		...
		getVideo: function () {
			...
			if (me.record) {
				return me.course
					.getVideoForId(me.record.ntiid || me.record.getId())
					.then(function (video) {
						if (video.slidedeck) {
		...
		curtainClicked: function (e) {
			...
			if (!Ext.isEmpty(slidedeck)) {
				...
				!e.getTarget('.launch-player') &&
				e.getTarget('.transcripts')
			) {
				this.maybePauseCurrentVideo();
				this.navigateToTarget(
					video,
					this.locationInfo && this.locationInfo.root
				);
			} else {

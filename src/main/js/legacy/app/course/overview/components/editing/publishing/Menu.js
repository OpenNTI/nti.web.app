...
module.exports = exports = Ext.define(
	'NextThought.app.course.overview.components.editing.publishing.Menu',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{ cls: 'arrow' },
			{
				...
				cn: [
					...
					{
						cls: 'option publish-on-date',
						'data-action': 'publish-date',
						cn: [
		...
		initComponent: function () {
			...
			this.onWindowResizeBuffer = Ext.Function.createBuffered(
				this.realign,
				10,
				this
			);
		...
		afterRender: function () {
			...
			this.mon(
				this.publishEl,
				'click',
				this.handleSelectionClick.bind(this)
			);
			this.mon(
				this.publishOnDateEl,
				'click',
				this.onPublishOnDateClick.bind(this)
			);
			this.mon(
				this.unpublishEl,
				'click',
		...
		realign: function (unlockSide) {
			if (!this.alignedTo || !this.rendered) {
				return;
			}
			...
			if (this.lockedToSide && unlockSide !== true) {
				...
				positions.below.maxHeight >= maxMenuHeight ||
				positions.below.maxHeight >= positions.above.maxHeight
			) {
		...
		setInitialState: function () {
			var node = this.record,
				isNodePublished =
					node && node.isPublished && node.isPublished(),
				lesson = this.contents,
				isLessonPublished =
					lesson && lesson.isPublished && lesson.isPublished(),
				lessonPublishDate = lesson && lesson.get('publishBeginning'),
				hasPublishDatePassed =
					lesson &&
					lesson.hasPublishDatePassed &&
					lesson.hasPublishDatePassed(),
				date;
			...
			if (
				isNodePublished &&
				((isLessonPublished && !lessonPublishDate) ||
					hasPublishDatePassed)
			) {
				...
				this.createDatePicker(
					this.publishOnDateEl.down('.date-picker-container')
				);
			} else {
				this.select(this.unpublishEl);
				this.initialState = this.unpublishEl;
			...
			if (
				this.datepicker &&
				(!lessonPublishDate ||
					(lessonPublishDate && hasPublishDatePassed))
			) {
		...
		onSave: function (e) {
			...
			if (action === 'publish-date') {
				if (
					this.datepicker &&
					this.datepicker.isValid &&
					this.datepicker.isValid() === false
				) {
		...
		createDatePicker: function (dateContainer) {
			...
			var c = this.contents,
				parentEl = dateContainer || Ext.getBody(),
				begin = this.contents && this.contents.get('publishBeginning'),
				hasPublishDatePassed =
					c && c.hasPublishDatePassed && c.hasPublishDatePassed(),
		...
		setPublishOnDateText: function (date) {
			...
			if (targetEl) {
				targetEl.update(
					'Lesson contents will be visible to students on ' + time
				);
			}
		},

		getDisplayDateValue: function (date) {
			var hour, minutes, meridiemVal;

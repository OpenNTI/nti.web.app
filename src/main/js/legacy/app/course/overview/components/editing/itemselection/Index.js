	...
	'NextThought.app.course.overview.components.editing.itemselection.Index',
	{
		...
		afterRender: function () {
			...
			this.searchInput =
				...
				search.el &&
				search.el.dom &&
				search.el.dom.querySelector('input');
		...
		getConfigForItem(item) {
			return {
				...
				getItemChildren:
					this.getItemChildren && this.getItemChildren.bind(this),
		...
		deleteSelectionItem(deletedItemId) {
			this.setSelectionItems(
				this.selectionItems.filter(selectionItem => {
					const id =
						selectionItem.ntiid ||
						selectionItem.NTIID ||
						selectionItem.internalId;
					if (deletedItemId === id) {
		...
		unselectItem: function (selectionItem) {
			...
			me.selection = me.selection.filter(function (item) {
				return (
					me.getSelectionItemId(item) !==
					me.getSelectionItemId(selectionItem)
				);
			});

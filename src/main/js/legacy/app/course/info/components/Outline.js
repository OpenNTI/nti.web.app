	...
	'NextThought.app.course.info.components.Outline',
	{
		...
		updateCatalogEntry: function (catalogEntry) {
			...
			this.updateStartDate(
				catalogEntry.StartDate ? new Date(catalogEntry.StartDate) : null
			);
		...
		setContent: function (
			...
			var startDate = startDateValue
				? Ext.util.Format.date(startDateValue, 'F j, Y')
				: null;
							...
							cn: [
								{
									cls: 'label',
									html: getString(
										'NextThought.view.courseware.info.parts.NotStarted.starts'
									),
								},
								{
									cls: 'date',
									html: startDate || 'No start date',
								},
							],
						},
					],
			...
			if (!showRoster && info.get('IsEnrolled')) {
				...
				this.mon(
					this.openCourseInfo,
					'show-enrollment',
					this.fireEvent.bind(this, 'show-enrollment')
				);
			}

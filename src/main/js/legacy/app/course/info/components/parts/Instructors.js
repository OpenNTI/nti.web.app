...
const { Instructors } = require('internal/nti-web-course-roster');
...
module.exports = exports = Ext.define(
	'NextThought.app.course.info.components.parts.Instructors',
	{
		...
		buildStore: function () {
			...
			data.forEach(function (o, i) {
				var url = Ext.String.format(
					photo,
					root,
					Ext.String.leftPad(i + 1, 2, '0')
				);

				o.set('photo', url);
				Service.request({ method: 'HEAD', url: url }).catch(
					o.set.bind(o, 'photo', User.BLANK_AVATAR)
				);
			});
		...
		afterRender: function () {
			...
			const canManage =
				bundle &&
				(bundle.hasLink('Instructors') || bundle.hasLink('Editors'));

			this.manageEl = this.el.down('.manage-button');

			if (canManage) {
				this.el.addCls('can-manage');
			} else {
				this.el.removeCls('can-manage');
			}
		...
		getCourse() {
			...
			return getService().then(service =>
				service.getObject(bundle.getId())
			);
		},

		onManageInstructors() {
			return this.getCourse().then(course => Instructors.show(course));
		},
	}
);

	...
	'NextThought.app.course.info.components.Menu',
	{
		...
		itemTpl: new Ext.XTemplate(
			Ext.DomHelper.markup({
				cls: 'outline-row{[values.active ? " x-item-selected" : ""]}',
				'data-trigger': '{trigger}',
				'data-qtip': '{title:htmlEncode}',
				'data-route': '{route}',
				cn: [{ cls: 'label', html: '{title:htmlEncode}' }],
			})
		),

		inviteTpl: new Ext.XTemplate(
			Ext.DomHelper.markup({
				cls: 'invitation-wrapper',
				cn: [
		...
		afterRender: function () {
			...
			this.CourseStore.on(
				'modified-course',
				this.addStaticInviteCode.bind(this)
			);
		},
		...
		addMenuItems: function () {
			...
			me.itemTpl.append(me.menuEl, {
				title:
					i.length > 1 ? t('instructor.other') : t('instructor.one'),
				trigger: 'instructors',
			});
		...
		addStaticInviteCode() {
			...
			Service.request(this.inviteCodeLink).then(code => {
				let courseInvitations = JSON.parse(code),
					codes =
						courseInvitations.Items &&
						courseInvitations.Items.map(invite => invite.Code).join(
							','
						);
		...
		setActiveItem: function (route) {
			var activeItem = this.el && this.el.down('.x-item-selected'),
				activeItemRoute =
					activeItem && activeItem.getAttribute('data-route');

			if (activeItemRoute === route || !this.rendered) {
				return;
			}

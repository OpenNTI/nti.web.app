	...
	'NextThought.app.course.editing.Actions',
	{
		...
		updateAssignment(assignment, { published, available }, { due }) {
			...
			const currentAvailable =
				assignment.get('availableBeginning') || null;
			...
			return this.updateAssignmentPublish(assignment, published).then(
				() => {
					return this.updateAssignmentDates(
						assignment,
						available,
						due
					);
			...
			return Service.put(link, data)
				...
				.catch(response => {
					var conflict =
						...
						lazy.ParseUtils.parseItems(response.responseText)[0];

					if (conflict) {
						return this.__resolveConflict(conflict, data).then(
							conflictResponse => {
								if (conflictResponse) {
									assignment.syncWithResponse(
										conflictResponse
									);
								}

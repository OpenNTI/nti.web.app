...
module.exports = exports = Ext.define(
	'NextThought.app.mediaviewer.components.mode.SmallVideo',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				cls: 'sync-button',
				cn: [{ tag: 'span', html: 'sync with video', role: 'button' }],
			},
			{ cls: 'video-player' },
			{
				id: '{id}-body',
		...
		configureVideoPlayer: function () {
			...
			var width = this.self.getTargetVideoWidth(
					this.getEl(),
					this.transcriptRatio
				),
			...
			this.on(
				'jump-video-to',
				Ext.bind(
					this.videoplayer.jumpToVideoLocation,
					this.videoplayer
				),
				this
			);
		...
		getStartTime() {
			...
			if (range && range.start) {
				...
				let slides = this.resourceList
					.filter(resource => resource.xtype === 'slide-component')
					.map(slidesCmp => slidesCmp.slide);
		...
		actOnMediaHeartBeat: function () {
			...
			if (
				!Ext.isEmpty(time) &&
				this.resourceView &&
				this.resourceView.highlightAtTime
			) {
		...
		async getLocationInfo() {
			...
			const [lineage] = await ContentUtils.getLineage(
				this.video.get('NTIID'),
				bundle
			);
		...
		videoNavigation: function (video) {
			...
			var ntiid = video && video.get('NTIID'),
				...
				route =
					section &&
					encodeForURI(section) + '/video/' + encodeForURI(ntiid);
			...
			if (this.ownerCt && this.ownerCt.handleNavigation) {
				this.ownerCt.handleNavigation(video.get('title'), route, {
					video: video,
				});
			}
		},

...
module.exports = exports = Ext.define(
	'NextThought.app.mediaviewer.components.View',
	{
		...
		items: [{ xtype: 'box' }, { xtype: 'media-grid-view' }],
		...
		setContent: function (video, transcript, options) {
			...
			if (!this.rendered) {
				this.onceRendered.then(function () {
					wait().then(
						me.setContent.bind(me, video, transcript, options)
					);
				});
				return;
			}
			...
			if (
				!this.viewer ||
				this.viewer.video.getId() !== this.video.getId()
			) {
		...
		afterRender: function () {
			...
			if (this.parentContainer && this.parentContainer.exitViewer) {
				this.on(
					'exit-viewer',
					this.parentContainer.exitViewer.bind(this.parentContainer)
				);
			}
		...
		getViewerType: function (type) {
			if (
				this.lockVideoWithNoTranscript &&
				!this.transcript &&
				!this.resourceList
			) {
				return 'full-video';
			...
			return (
				type ||
				this.getStorageManager().get('media-viewer-player-type') ||
				'video-focus'
			);
		},
		...
		adjustOnResize: function () {
			var toolbarHeight =
					(this.toolbar.el && this.toolbar.getHeight()) || 0,
				messageBarHeight =
					document.documentElement &&
					document.documentElement.classList.contains('msg-bar-open')
						? 40
						: 0,
				availableHeight,
				paddingHeight = 0,
		...
		switchVideoViewer: function (type, doNotPersist) {
			if (!type || type === (this.viewer && this.viewer.viewerType)) {
			...
			return allow.then(function () {
				//store the current type so we can retrieve it later
				if (doNotPersist !== true) {
					me.getStorageManager().set(
						'media-viewer-player-type',
						playerType
					);
				}
				...
				if (
					targetViewer &&
					targetViewer.video.getId() === me.video.getId()
				) {
				...
				wait(1000).then(me.fireEvent.bind(me, 'animation-end'));
				wait(1001).then(me.adjustOnResize.bind(me));
			});
		},
		...
		showGridViewer: function (action) {
			...
			return allow.then(function () {
				if (action === 'show') {
					...
					wait(2000).then(me.adjustOnResize.bind(me));
				} else {

...
module.exports = exports = Ext.define(
	'NextThought.app.mediaviewer.components.reader.View',
	{
		...
		initComponent: function () {
			this.enableBubble([
				'presentation-parts-ready',
				'no-presentation-parts',
				'jump-video-to',
			]);
		...
		onStoreEventsAdd: function (store, records) {
			var cmps = this.MediaViewerStore.getComponentsForStore(
				store.containerId
			);
		...
		setupNoteOverlay: function () {
			...
			this.on(
				'destroy',
				'destroy',
				this.relayEvents(this.noteOverlay, [
					'editorActivated',
					'editorDeactivated',
				])
			);
			...
			function maybeDone() {
				...
				if (done && !me.isDestroyed) {
					...
					Promise.all(
						...
						.catch(function () {
							console.error(
								'*****FAILED to load UserData in the Media Viewer: ',
								arguments
							);
						});
				}
		...
		highlightAtTime: function (seconds, allowScroll) {
			...
			Ext.each(cmps, function (cmp) {
				var isTimePart =
						cmp.isTimeWithinTimeRange &&
						cmp.isTimeWithinTimeRange(seconds),
					tEl;
				if (isTimePart) {
		...
		scrollToEl: function (el, offset) {
			...
			if (shouldScroll && scrollingEl) {
				top =
					scrollingEl.getScrollTop() +
					(el.getY() - scrollingEl.getY()) -
					offset;
				this.desiredTop = top;
		...
		scrollToStartingTime: function (seconds) {
			...
			Ext.each(cmps, function (part) {
				if (
					part.isTimeWithinTimeRange &&
					part.isTimeWithinTimeRange(seconds)
				) {
					...
					if (tEl && scrollingEl) {
						...
						scrollingEl.scrollTo(
							'top',
							tEl.getBox().top - scrollingEl.getY()
						);
						return false;
					}
				}
		...
		showAnnotationView: function (store) {
			var me = this,
				classList = 'presentation-note-slider annotation-view dark';
			if (!this.annotationView) {
				classList += this.accountForScrollbars
					? ' scroll-margin-right'
					: '';
				if (Ext.is.iOS) {
		...
		mayBeHideAnnotationView: function (e) {
			...
			if (
				(!e || !e.getTarget('.annotation-view')) &&
				this.annotationView.isVisible()
			) {
				this.annotationView.hide();
		...
		getDomContextForRecord: function (r) {
			...
			Ext.each(cmps, function (cmp) {
				if (
					Ext.isFunction(cmp.getDomContextForRecord) &&
					(!Ext.isFunction(cmp.wantsRecord) || cmp.wantsRecord(r))
				) {
			...
			return {
				resizeView: function () {
					var reader = this.reader,
						w =
							reader.getWidth() -
							reader.annotationView.getWidth() -
							20,

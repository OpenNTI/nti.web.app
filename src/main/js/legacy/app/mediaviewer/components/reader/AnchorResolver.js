	...
	exports =
		Ext.define(
			'NextThought.app.mediaviewer.components.reader.AnchorResolver',
			{
				...
				createRangeDescriptionFromRange: function (
					range,
					docElement,
					cueInfo
				) {
					if (!range) {
						return this.createRangeDescriptionFromTimeRange(
							cueInfo
						);
					}
					...
					if (!range || range.collapsed) {
						...
						Ext.Error.raise(
							'Anchorable range for range could not be found'
						);
					}

					var pureRange = lazy.Anchors.purifyRange(range, docElement),
						startDomPointer = lazy.Anchors.createPointer(
							pureRange,
							'start'
						),
						endDomPointer = lazy.Anchors.createPointer(
							pureRange,
							'end'
						);

					return {
						description: TranscriptRangeDescription.create({
							start: this.createTranscriptPointer(
								startDomPointer,
								'start',
								cueInfo.startCueId,
								cueInfo.startTime
							),
							end: this.createTranscriptPointer(
								endDomPointer,
								'end',
								cueInfo.endCueId,
								cueInfo.endTime
				...
				toDomRange: function (description, doc, cleanRoot) {
					...
					if (cleanRoot) {
						...
						targetEl = Ext.fly(cleanRoot).down(
							'.cue[cue-start=' + start + ']'
						);
						if (targetEl) {
				 ...
				 */
				getDomElementForTranscriptTimeRange: function (
					description,
					cueStore,
					video
				) {
				...
				//TODO: this function is too customized for resolving time range to dom range in transcript.
				// It needs to be reworked to handle more of a general case( any time of time range to dom Range.)
				fromTimeRangeToDomRange: function (
					description,
					cueStore,
					container,
					docElement
					...
					cues.each(function (cue) {
						...
						var el = Ext.fly(container).down(
							'.cue[cue-start=' + cue.get('startTime') + ']'
						);
						if (el) {
							els.push(el);
						}
				...
				getCuesWithinRange: function (store, start, end) {
					function fn(item) {
						return (
							item.get('endTime') > 0 &&
							start.isFloatLessThanOrEqual(
								item.get('startTime')
							) &&
							end.isFloatGreaterThanOrEqual(item.get('endTime'))
						);
					}
				...
				createTranscriptPointer: function (
					rangePointer,
					role,
					cueId,
					time

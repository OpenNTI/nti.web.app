...
module.exports = exports = Ext.define(
	'NextThought.app.mediaviewer.components.reader.NoteOverlay',
	{
		...
		constructor: function (config) {
			...
			this.adjustAnnotationOverlayPosition = Ext.Function.createBuffered(
				this.adjustAnnotationOverlayPosition,
				10
			);
			...
			me.reader.relayEvents(me, [
				'save-new-note',
				'save-new-series-note',
			]);
		...
		allowNavigation: function () {
			...
			let msg =
				'You haven’t finished your comment. Do you want to leave without finishing?';
		...
		insertOverlay: function () {
			...
			this.annotationOverlay = Ext.DomHelper.insertAfter(
				this.reader.getTargetEl().first(),
				{ cls: 'note-gutter' },
				true
			);
			...
			this.mon(
				...
				'click',
				'showAnnotationsAtLine',
				this
			);
		},

		adjustAnnotationOverlayPosition: function () {
			if (!this.reader.rendered || !this.annotationOverlay) {
				return;
			}

			let cmps = Ext.isFunction(this.reader.getPartComponents)
				? this.reader.getPartComponents()
				: [];
			...
			Ext.each(cmps, function (cmp) {
				if (
					Ext.isFunction(cmp.positionAnnotationNibs) &&
					cmp.isVisible(true)
				) {
		...
		realignNotes: function () {
			...
			let cmps = Ext.isFunction(this.reader.getPartComponents)
				? this.reader.getPartComponents()
				: [];
			let visibleCmps = Ext.Array.filter(cmps, function (c) {
				return (
					Ext.isFunction(c.registerAnnotations) && c.isVisible(true)
				);
			});
		...
		editorSaved: function (editor, r, v) {
			...
			try {
				...
				if (me.data.isDomRange) {
					...
					me.UserDataActions.saveNewSeriesNote(
						title,
						note,
						range,
						me.data,
		...
		setDefaultSharingFor: function (ntiid) {
			...
			me.MediaViewerStore.getSharingPreferences(
				ntiid,
				me.reader.currentBundle
			)
				.then(function (prefs) {
		...
		registerGutterRecords: function (
			noteStore,
			records,
			view,
			onUnregistered
		) {
			if (Ext.isEmpty(noteStore)) {
				return;
			}
			...
			if (!reader.rendered) {
				reader.onceRendered.then(function () {
					wait(10).then(me.registerGutterRecords.bind(me));
				});
				return;
			}
			...
			Ext.each(records, function (n) {
				if (n.isTopLevel && n.isTopLevel()) {
					const isRegistered = me.registerNoteRecord(
						n,
						view,
						noteStore
					);
		...
		registerOrphanedRecord: function (store, note, view) {
			if (this.isRecordAlreadyAdded(note)) {
				return;
			}
		...
		rangeForDescription: function (rec, cmp, recStore) {
			...
			if (
				rec &&
				Ext.isFunction(cmp.domRangeForRecord) &&
				(!Ext.isFunction(cmp.wantsRecord) || cmp.wantsRecord(rec))
			) {
		...
		registerNoteRecord: function (rec, cmp, recStore) {
			if (this.isRecordAlreadyAdded(rec)) {
				return;
			}
		...
		unRegisterNoteRecord: function (rec) {
			let r = this.annotationManager.findBy(function (item) {
		...
		getAnnotationEl: function (line) {
			...
			let result = this.annotationOverlay.down(
				'.count[data-line=' + line + ']'
			);

			if (result || Ext.isEmpty(annotations)) {
				return result;
			}
		...
		resolveRootPageInfoFor: function (ntiid) {
			return ContentUtils.getLineage(
				ntiid,
				this.reader.currentBundle

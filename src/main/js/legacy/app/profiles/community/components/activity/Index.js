...
module.exports = exports = Ext.define(
	'NextThought.app.profiles.community.components.activity.Index',
	{
		...
		onNewPost: function () {
			if (this.postContainer && this.postContainer.getLink('add')) {
				this.WindowActions.showWindow(
					...
					{ afterClose: this.onPostSaved.bind(this) },
					{
		...
		onPostSaved: function (record) {
			...
			this.__loadItem(record).then(function (cmp) {
				if (cmp) {
					left.insert(0, cmp);
					me.clearEmpty();
				}
		...
		addItems: function (items, feedUrl) {
			if (feedUrl !== this.feedUrl) {
				console.warn(
					'Trying to add items from a different feed, dropping them on the floor'
				);
				return;
			}
			...
			return Promise.all(items)
				...
				.then(function (cmps) {
					if (feedUrl !== me.feedUrl) {
						console.warn(
							'Trying to add items for a different feed, dropping them on the floor'
						);
						return;
					}
		...
		loadPage: function (page) {
			...
			StoreUtils.loadBatch(me.feedUrl, params)
				.then(function (batch) {
					if (feedUrl !== me.feedUrl) {
						console.warn(
							'Loaded batch for a different feedurl than the active, drop it on the floor'
						);
					} else {
				...
				.catch(function (reason) {
					if (feedUrl !== me.feedUrl) {
						console.warn(
							'Failed to loaded batch for a different feedurl than the active, drop it on the floor'
						);
					} else {
		...
		onBatchError: function (error) {
			...
			if (status === 403 && this.newPostCmp) {
				wait().then(() => this.newPostCmp.hide());
			}
		},
		...
		onBatchLoad: function (batch, feedUrl) {
			...
			//if we have items
			if (batch.ItemCount) {
				this.addItems(batch.Items, feedUrl).always(
					this.loadingCmp.addClass.bind(this.loadingCmp, 'hidden')
				);

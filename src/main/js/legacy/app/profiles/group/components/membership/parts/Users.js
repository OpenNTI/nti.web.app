...
require('internal/legacy/app/profiles/user/components/membership/parts/Membership');
...
module.exports = exports = Ext.define(
	'NextThought.app.profiles.group.components.membership.parts.Users',
	{
		...
		setFriends(friends) {
			this.removeAll();
			return UserRepository.getUser(friends).then(members => {
				if (members.length) {
					members.map(member =>
						this.addEntry(this.configForUser(member))
					);
				} else {

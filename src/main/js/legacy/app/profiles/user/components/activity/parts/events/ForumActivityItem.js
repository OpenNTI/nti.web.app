...
module.exports = exports = Ext.define(
	'NextThought.app.profiles.user.components.activity.parts.events.ForumActivityItem',
	{
		...
		beforeRender: function () {
			...
			h.compileBodyContent(me.setBody, me, null, null, null, {
			...
			UserRepository.getUser(username, function (u) {
				...
				if (me.rendered) {
					...
					me.avatarEl.setStyle({
						backgroundImage: 'url(' + rd.avatarURL + ');',
					});
			...
			if (rd.PostCount > 0 && !Ext.isEmpty(url)) {
				...
				console.error(
					'We think this item has been deleted, no Content link available. Record: ',
					this.record
				);
				Ext.defer(this.destroy, 1, this);
			}

			me.PathActions.getPathToObject(r).then(me.setPath.bind(me));
		},
		...
		afterRender: function () {
			...
			if (this.commentsEl) {
				this.mon(
					this.commentsEl,
					'click',
					this.forumClickHandlerGoToComments,
					this
				);
			}
			if (this.editEl) {
		...
		setBody: function (text, insertComponents) {
			...
			if (insertComponents) {
				insertComponents(me.messageBodyEl, me).forEach(function (c) {
...
Ext.define(
	'NextThought.app.profiles.user.components.activity.parts.events.ForumActivityItemReply',
	{
							...
							cn: [
								{
									tag: 'tpl',
									if: 'isModifiable',
									cn: [{ cls: 'link edit', html: 'Edit' }],
								},
								{
									tag: 'tpl',
									if: '!isModifiable && !Deleted',
									cn: [{ cls: 'link flag', html: 'Report' }],
								},
								{
									tag: 'tpl',
									if: 'isModifiable',
		...
		beforeRender: function () {
			...
			r.compileBodyContent(me.setBody, me, null, null, null, {
				useVideoPlaceholder: true,
			});
		...
		setBody: function (html) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.setBody, this, arguments),
					this
				);
				return;
			}
			...
			this.messageBodyEl
				.select('.whiteboard-container .toolbar')
				.remove();
			this.messageBodyEl
				.select('.whiteboard-container .overlay')
				.remove();
		},
	}
);

	...
	'NextThought.app.profiles.user.components.activity.parts.events.PostReply',
	{
		...
		afterRender: function () {
			...
			function resolve(json) {
				...
				UserRepository.getUser(users).then(fillIn, failed);
			}
			...
			function fillIn(users) {
				...
				rd.someones =
					users.length === 1 ? 'their own' : other.getName() + "'s";
			...
			Service.request(postRef).then(resolve, failed);
		},
				...
				r.compileBodyContent(
					function (html, cb) {
						...
						if (Ext.isFunction(cb)) {
							me.knownBodyComponents = cb(me.body, this).map(
								function (c) {

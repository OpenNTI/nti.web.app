	...
	'NextThought.app.profiles.user.components.activity.parts.events.TranscriptSummaryItem',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				...
				style: {
					backgroundImage:
						"url('/app/resources/images/icons/chat-32-blue.png')",
				},
			},
			{
				cls: 'meta',
								...
								cn: [
									{
										tag: 'tpl',
										if: 'group',
										html: ' had a group chat with ',
									},
									{
										tag: 'tpl',
										if: 'single',
										html: ' had a chat with ',
									},
									{
										tag: 'tpl',
										if: 'lonely',
		...
		afterRender: function (cmp) {
			...
			var page,
				...
				duration = TimeUtils.timeDifference(ended, started).replace(
					/ ago/i,
					''
				),
			...
			function showRecepients(u) {
				...
				for (less = u.length; less >= 0; less--) {
					...
					if (u.length > 1) {
						occupantsString =
							owner +
							'had a group chat with' +
							me.stringifyNames(u, less - 1);
					} else {
						occupantsString =
							owner +
							'had a chat with' +
					...
					width =
						me.el.down('.title').getWidth() -
						parseInt(me.el.getStyle('padding-right'), 10);
			...
			page = GroupsStateStore.getInstance()
				.getFriendsList()
				.getById(RoomInfo.get('ContainerId'));
			...
			if (names.length === 1) {
				...
				if (less === 0) {
					...
					if (less >= names.length) {
						...
						return (
							names.slice(0, names.length - less).join(', ') +
							', and ' +
							less +
							(less === 1 ? ' other' : ' others')
						);

	...
	'NextThought.app.profiles.user.components.achievements.Index',
	{
		...
		addItems: function () {
				...
				items: [
					...
					},
					{
						...
						items: [
							{
								header: getString(
									'NextThought.view.profiles.parts.Achievements.current_title'
								),
								current: true,
								cls: 'current',
								desiredColumns: 3,
								emptyText: getString(
									'NextThought.view.profiles.parts.Achievements.current_empty'
								),
							},
							{
								header: getString(
									'NextThought.view.profiles.parts.Achievements.completed_title'
								),
								completed: true,
								cls: 'completed',
								desiredColumns: 4,
								...
								preferenceTooltip: getString(
									'NextThought.view.profiles.parts.Achievements.completed_preference'
								),
								emptyText: getString(
									'NextThought.view.profiles.parts.Achievements.completed_empty'
								),
							},
						],
					},
			...
			this.add({
				...
				header: getString(
					'NextThought.view.profiles.parts.Achievements.achievements_title'
				),
				achievements: true,
				cls: 'achievements',
				emptyText: getString(
					'NextThought.view.profiles.parts.Achievements.achievements_empty'
				),
			});
		},
		...
		onActivate: function () {
			...
			if (!this.hideCertificates && this.activeUser) {
				this.activeUser.getInterfaceInstance().then(entity => {
			...
			me.tip = Ext.widget('nt-tooltip', {
				...
				listeners: {
					beforeshow: function (tip) {
						...
						if (!record) {
							console.error(
								'No badge record to fill out the tool tip with',
								tip
							);
							return false;
						}
		...
		userChanged: function (user, isMe) {
			...
			return getAppUser().then(appUser => {
				...
				if (user.get('ID') !== appUser.ID) {
					...
					user.getInterfaceInstance().then(entity => {
						this.certificatesCmp = this.add({
							xtype: 'react',
							component: ProfileCertificates,
							entity,
				...
				this.loadingCmp =
					this.loadingCmp ||
					this.add(Globals.getContainerLoadingMask());
		...
		setEmptyState: function () {
			...
			this.emptyText = this.add({
				xtype: 'box',
				autoEl: {
					cls: 'empty-achievements-container',
					cn: [
						{
							cls: 'empty-achievements-text',
							html: getString(
								'NextThought.view.profiles.parts.Achievements.empty'
							),
						},
					],
				},

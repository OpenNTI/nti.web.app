	...
	'NextThought.app.profiles.user.components.activity.parts.events.ActivityItem',
	{
		...
		afterRender: function () {
			...
			this.on('add', function (cmp, child) {
				if (
					this.replyToId &&
					child.record &&
					child.record.getId() === this.replyToId
				) {
		...
		maybeFillIn: function () {
			var me = this,
				...
				onScreen =
					loaded ||
					(me.el &&
						me.el.first().isOnScreenRelativeTo(Ext.getBody(), {
							bottom: 1000,
						}));
		...
		setLocation: function (meta) {
			...
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.setLocation, this, arguments),
					this,
					{ single: true }
				);
				return;
			...
			try {
				meta.getPathLabel().then(function (label) {
			...
			},
			{
				...
				cn: {
					cn: [
						{
							cls: 'reply-options',
							cn: [{ cls: 'link reply', html: 'Add a comment' }],
						},
					],
				},
			},

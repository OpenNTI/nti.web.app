...
module.exports = exports = Ext.define(
	'NextThought.app.badge.Window',
	{
		...
		initComponent: function () {
			...
			this.add({
				xtype: 'badge-window-footer',
				doClose: this.doClose.bind(this),
				onExport: this.exportBadgeClicked.bind(this),
			});
		},
	...
	function () {
		...
		StateStore.registerCustomResolver(Badge.mimeType, function (id, raw) {
			...
			if (earnedLink) {
				return Service.request(earnedLink).then(function (collection) {
					var badge,
						record,
					...
					return record
						? lazy.ParseUtils.parseItems(record)[0]
						: Promise.reject();
				});
			}

			return Service.request(
				'/' + Globals.trimRoute(link) + '/' + raw
			).then(function (badge) {

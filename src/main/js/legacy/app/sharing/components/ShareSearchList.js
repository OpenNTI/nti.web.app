...
module.exports = exports = Ext.define(
	'NextThought.app.sharing.components.ShareSearchList',
	{
		...
		emptyText:
			'<div class="x-menu-item no-results">No results found.</div>',
		...
		tpl: new Ext.XTemplate(
				...
				cn: [
					{
						...
						cn: [
							{
								cls: 'x-menu-item contact-card label',
								html: '{realname}',
							},
						],
					},
			...
			}),
			{
				...
				getNameClass(model) {
					return cx(
						'name',
						toCSSClassName(this.getDisplayName(model))
					);
				},
		...
		setUpMaskListeners: function (store) {
			...
			if (!this.rendered) {
				this.on(
					'afterrender',
					this.setUpMaskListeners.bind(this, store)
				);
				return;
			}
		...
		selectNext: function () {
			...
			while (this.store.getAt(next).get('isLabel')) {
				//is label
				next += 1;
			...
			if (!this.store.getAt(next).get('isLabel')) {
				//is not label
				this.getNodeByRecord(this.store.getAt(next)).classList.add(
					'x-item-selected'
				);
				if (node) {
		...
		selectPrev: function () {
			...
			while (this.store.getAt(prev).get('isLabel')) {
				//is label
				prev -= 1;
				if (prev < 0) {
					prev = this.store.getCount() - 1;
			...
			if (!this.store.getAt(prev).get('isLabel')) {
				//is not label
				this.getNodeByRecord(this.store.getAt(prev)).classList.add(
					'x-item-selected'
				);
				if (node) {
					node.classList.remove('x-item-selected');
		...
		unselectItem: function () {
			var selected = this.getSelectionModel().selected,
				index =
					(this.store && this.store.indexOf(selected.items[0])) || -1;

...
module.exports = exports = Ext.define(
	'NextThought.app.sharing.components.UserTokenField',
	{
		...
		containsToken: function (model) {
			...
			var id = model.getId(),
				c;
			c = Ext.Array.filter(this.selections, function (o, i) {
				return o.getId() === id;
			});
			return c.length > 0;
		},
		...
		containsUnresolved: function () {
			const c = Ext.Array.filter(this.selections, function (o, i) {
		...
		searchItemSelected: function (record) {
			...
			wait().then(this.updateSize.bind(this));
		...
		setValue: function (sharingInfo) {
			if (!this.rendered) {
				...
				this.setValueAfterRenderListener = this.on({
					afterrender: Ext.bind(this.setValue, this, arguments),
					scope: this,
					single: true,
					destroyable: true,
				});
		...
		onKeyDown: function (e) {
			...
			if (Ext.is.iOS) {
				if (
					this.inputEl.dom.value.length === 1 &&
		...
		alignPicker: function () {
			...
			var minSpace = 300,
				...
				pickerScrollHeight =
					picker.el && picker.el.dom && picker.el.dom.scrollHeight,
				maxHeight,
				y,
				x = rect.left,
				above = false;
			...
			if (Ext.is.iOS) {
				if (
					this.inputEl.dom.value === '' ||
					document.activeElement !== this.inputEl.dom
				) {
					this.hidePicker();
				}
			}
			...
			if (
				pickerScrollHeight === 0 &&
				!(Ext.is.iOS && document.activeElement !== this.inputEl.dom)
			) {
				wait().then(this.alignPicker.bind(this));
			}
		...
		removeToken: function (tokenName, tokenEl) {
			...
			Ext.each(this.selections, function (o) {
				if (o.get('displayName') !== tokenName) {
					s.push(o);
				}
			});
		...
		onClick: function (e) {
			...
			if (v) {
				this.removeToken(v, p);
			}

			if (!Ext.is.iPad) {
				//Don't focus for iPad, won't bring up keyboard
				this.inputEl.focus();
			}
		},

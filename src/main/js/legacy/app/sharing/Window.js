...
module.exports = exports = Ext.define('NextThought.app.sharing.Window', {
	...
	initComponent: function () {
		var readOnly = this.isReadOnly(),
			title =
				this.titleLabel || (readOnly ? 'Item Info' : 'Share this...'),
	...
	isReadOnly: function () {
		...
		return (
			(this.record.children && this.record.children.length > 0) ||
			(!Ext.isEmpty(refCount) && refCount > 0)
		);
	},
	...
	save: function () {
		...
		this.UserDataActions.updateShareWith(
			this.record,
			this.getValue(),
			checkbox.checked,
			this.bundle

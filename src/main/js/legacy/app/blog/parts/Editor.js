...
module.exports = exports = Ext.define('NextThought.app.blog.parts.Editor', {
	...
	afterRender: function () {
		...
		if (r) {
			...
			me.setSharedWith({
				publicToggleOn: sharedWith.publicToggleOn,
				entities: [],
			});
	...
	onSave: function (e) {
		...
		if (Ext.isEmpty(v.title)) {
			me.markError(
				me.titleWrapEl,
				getString(
					'NextThought.view.profiles.parts.BlogEditor.emptytitle'
				)
			);
		...
		if (/^@{1,}/.test(v.title)) {
			...
			me.markError(
				me.titleWrapEl,
				getString('NextThought.view.profiles.parts.BlogEditor.attitle')
			);
			me.titleWrapEl.addCls('error-on-bottom');
			return;
		}

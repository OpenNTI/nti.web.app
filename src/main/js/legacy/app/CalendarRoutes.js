...
module.exports = exports = function calendarRoutes (scope) {
	...
	const handlers = {
			...
			if (obj.hasLink('JoinWebinar')) {
				...
				return async () => {
					...
					if (obj.hasLink('WebinarRegister')) {
						WindowActions.pushWindow(Base.interfaceToModel(webinar));
					}
				};
			}
			else {
		...
		[Models.calendar.AssignmentCalendarEvent.MimeType]: function (obj, context) {
			return async () => {
				...
				scope.navigateToObject(Base.interfaceToModel(libraryPathObject));
			};
		}
	};

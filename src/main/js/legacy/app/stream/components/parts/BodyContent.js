	...
	'NextThought.app.stream.components.parts.BodyContent',
	{
				...
				cn: [
					{
						...
						cn: [
							{
								tag: 'span',
								cls: 'list-item blue link comments',
					...
					},
					{
						...
						cn: [
							{
								tag: 'span',
								cls: 'list-item blue link report',
								html: 'Report',
							},
						],
					},
					{
						...
						cn: [
							{
								tag: 'span',
								cls: 'list-item blue link delete',
								html: 'Delete',
							},
						],
					},
		...
		beforeRender: function () {
			...
			if (!this.Creator.isModel) {
				UserRepository.getUser(this.Creator).then(
		...
		setUser: function (user) {
			...
			profilePic.setStyle({
				backgroudImage: 'url(' + user.get('avatarURL') + ')',
			});
		...
		setBody: function (html) {
			...
			el.select('img.whiteboard-thumbnail').each(
				elm => void elm.replace(elm.up('.body-divider'))
			);

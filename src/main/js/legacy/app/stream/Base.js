...
module.exports = exports = Ext.define('NextThought.app.stream.Base', {
	...
	fetchNewItems: function () {
		if (this.StreamSource) {
			this.StreamSource.fetchNewItems().then(items =>
	...
	onActivate: function () {
		...
		if (
			!this.isLoading &&
			(this.clearOnDeactivate || this.PAGES.length === 0)
		) {
			...
			this.errorCmp = cmp.add({
				...
				autoEl: {
					...
					cn: [
						{
							cls: 'container-error',
							cn: [{ cls: 'error-text', html: this.errorText }],
						},
					],
				},
			});
			...
			this.emptyCmp = cmp.add({
				...
				autoEl: {
					...
					cn: [
						{
							cls: 'container-empty',
							cn: [{ cls: 'empty-text', html: this.emptyText }],
						},
					],
				},
			});
			...
			this.doneCmp = cmp.add({
				...
				autoEl: {
					...
					cn: [
						{
							cls: 'container-done',
							cn: [{ cls: 'done-text', html: this.doneText }],
						},
					],
				},
			});

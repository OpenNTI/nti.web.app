	...
	'NextThought.app.assessment.ScoreboardTally',
	{
				...
				cn: [
					{
						...
						cn: [
							{ tag: 'span', cls: 'count' },
							' {{{NextThought.view.assessment.ScoreboardTally.correct}}}',
						],
					},
					{
						cls: 'stat incorrect',
						cn: [
							{ tag: 'span', cls: 'count' },
							' {{{NextThought.view.assessment.ScoreboardTally.incorrect}}}',
						],
					},
					{ cls: 'stat questions' },
				],
		...
		setGreyText: function (text) {
			if (!this.rendered) {
				this.on({
					single: true,
					afterrender: this.setGreyText.bind(this, [text]),
				});
				return;
			}
		...
		setGreenText: function (text) {
			if (!this.rendered) {
				this.on({
					single: true,
					afterrender: this.setGreenText.bind(this, [text]),
				});
				return;
			}
		...
		setRedText: function (text) {
			if (!this.rendered) {
				this.on({
					single: true,
					afterrender: this.setRedText.bind(this, [text]),
				});
				return;
			}
		...
		setTally: function (correct, total, noScore) {
			...
			if (total < correct) {
				console.warn(
					'Setting a strange talley where total is less than correct. Total:' +
						total +
						', Correct: ' +
						correct
				);
			...
			var me = this,
				...
				bucketId = Math.min(10, Math.max(Math.ceil(percent / 10), 0)),
				messageBucket = bucketId || 0,
				msg = '';

			try {
				msg = getString(
					'question_set_scoreboard_' + messageBucket,
					getString('default_scoreboard_message')
				);
			} catch (e) {
				console.error(
					'Error getting message: ' + e.message,
					Globals.getError(e)
				);
			}
			...
			function updateTpl() {
				...
				if (me.questionsBox) {
					me.questionsBox.update(
						Ext.util.Format.plural(total, 'question')
					);
				}

	...
	'NextThought.app.assessment.components.feedback.Item',
	{
		...
		beforeRender: function () {
			...
			this.renderData = Ext.apply(
				this.renderData || {},
				(this.record && this.record.getData()) || {}
			);
		},
		...
		update: function () {
			...
			this.getBody().then(function (html) {
		...
		onSave: function (editor, record, value) {
			...
			editor.mask(
				getString(
					'NextThought.view.assessment.AssignmentFeedback.editor-mask'
				)
			);
			if (!record) {
			...
			record
				...
				.catch(function (reason) {
					...
					alert({
						title: getString(
							'NextThought.view.assessment.AssignmentFeedback.error-title'
						),
						msg: getString(
							'NextThought.view.assessment.AssignmentFeedback.error-msg'
						),
					});
				});
		},
		...
		onFeedbackClick: function (e) {
			...
			if (whiteboard) {
				...
				(e.getTarget('.avatar') || e.getTarget('.name')) &&
				c &&
				c.getProfileUrl
			) {

...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.input.MultipleChoice',
	{
					...
					cn: [
						{
							...
							cn: [
								...
								}, //xindex is 1 based,
								{
									tag: 'span',
									cls: 'label',
									html: '{[String.fromCharCode(64+xindex)]}.',
								},
								{
									tag: 'span',
									cls: 'indexed-content',
			...
			if (images.length > 0) {
				Promise.all(
					images.map(
						img =>
							new Promise(
								next => (img.onerror = img.onload = next)
							)
					)
				)
		...
		choiceClicked: function (e) {
			...
			if (!this.getEl().query('.control.checked').length) {
		...
		setValue: function (checkedIndexes) {
			...
			Ext.each(this.getEl().query('.control'), function (e) {
				var check = Ext.Array.contains(
					checkedIndexes,
					parseInt(e.getAttribute('data-index'), 10)
				);
		...
		getSolutionContent: function (part) {
			...
			Ext.each(part.get('solutions'), function (x) {
				...
				Ext.each(x, function (s) {
					out.push(
						tpl.apply([String.fromCharCode(65 + s), choices[s]])
					);
				});
			});

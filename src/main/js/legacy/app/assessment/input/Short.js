...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.input.Short',
	{
		...
		alias: ['widget.question-input-fillintheblankshortanswerpart'],
		...
		afterRender: function () {
			...
			me.blankInputs.forEach(function (i) {
				...
				if (ctEl) {
					setup(
						ctEl.querySelector('input[name="' + name + '"]'),
						true
					);
				}
			});
				...
				Ext.each(x, function (s) {
					...
					for (k in s) {
						if (s.hasOwnProperty(k)) {
							...
							if (typeof o !== 'string') {
								console.warn(
									'Probably dropping question solution... its not a simple value. %o',
									o
								);
								o = o.solution;
							}
		...
		markGraded: function (yes) {
			...
			this.el
				.select('span.blank input')
				.set({ disabled: yes !== false ? true : undefined });
		},

...
function toMap(ar, attr) {
	var m = {};
	Ext.each(ar, function (e) {
		m[e.getAttribute(attr)] = e;
	});
	return m;
}
...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.input.Matching',
	{
		...
		inputTpl: Ext.DomHelper.markup([
			{
				...
				cn: {
					...
					cn: [
						{
							cls: 'target term drag',
							'data-match': '{[xindex-1]}',
							cn: [
								{
									cls: 'match',
									'data-term': '{.:htmlEncode}',
									cn: [{ cls: 'reset' }, '{.}'],
								},
							],
						},
...

			{
				...
				cn: [
					{
						cls: 'target choice',
						'data-target': '{[xindex-1]}',
						cn: [
							{
								cls: 'match blank dropzone',
								'data-term': '{parent.term:htmlEncode}',
								html: '{parent.term}',
							},
							{ cls: 'text', html: '{.}' },
						],
		...
		initComponent: function () {
			...
			this.bufferedUpdateLayout = Ext.Function.createBuffered(
				this.updateLayout,
				500
			);
		...
		setValue: function (value) {
			var q = '.drag.term',
				...
				terms = Ext.Array.unique(
					this.shelfEl.query(q).concat(this.inputBox.query(q))
				);
		...
		reset: function () {
			...
			function r(e) {
				var p = Ext.getDom(e).parentNode,
					doc =
						(p && (p.ownerDocument || p.documentElement)) ||
						document,

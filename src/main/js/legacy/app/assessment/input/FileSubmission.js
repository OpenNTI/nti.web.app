...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.input.FileSubmission',
	{
		...
		alias: [
			'widget.question-input-filepart',
			'widget.question-input-qfilepart',
		],
		...
		getEmptySubmissionTpl() {
			return new Ext.XTemplate(
				Ext.DomHelper.markup([
					{
						cls: 'empty-submission',
						cn: [
							{ cls: 'icon' },
							{ cls: 'title', html: t('noFile') },
						],
					},
				])
			);
		...
		beforeRender: function () {
			if (this.question.tallyParts() === 1) {
				this.up('assessment-question').addCls('file-submission');
			}
		...
		unsupported: function () {
			alert(
				getString(
					'NextThought.view.assessment.input.FileSubmission.unsupported-feature'
				)
			);
		},
		...
		attachInputListeners() {
			...
			const changeInput =
				this.changeInputField && this.changeInputField.dom;
			...
			if (changeInput) {
				changeInput.addEventListener(
					'change',
					this.onChangeFileInputChange.bind(this)
				);
			}
		},
		...
		removeInputListeners: function () {
			...
			if (changeInput) {
				changeInput.removeEventListener(
					'change',
					this.onChangeFileInputChange.bind(this)
				);
			}
		...
		*/
		...
		showEmptySubmission: function () {
			...
			this.clearView();
			if (submitted > 0) {
				this.emptySubmissionEl = this.getEmptySubmissionTpl().append(
					this.el
				);
			} else {
				this.up('assessment-question').addCls('no-data');
			}
		},

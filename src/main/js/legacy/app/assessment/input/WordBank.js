...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.input.WordBank',
	{
		...
		alias: ['widget.question-input-fillintheblankwithwordbankpart'],
		...
		renderTpl: Ext.DomHelper.markup([{ cls: 'wordbank-ct' }, '{super}']),

		solTpl: Ext.DomHelper.createTemplate({
			cls: 'multiple-choice-solution',
			cn: [
				'{0} ',
				{ tag: 'span', cls: 'solution-choice-text', html: '{1}' },
			],
		}),

		wordTpl: Ext.DomHelper.createTemplate({
			tag: 'span',
			cls: 'target wordentry drag graded',
			html: '{0}',
		}),
		...
		blankTpl: Ext.DomHelper.createTemplate({
			tag: 'span',
			cls: 'blank dropzone target',
		...
		beforeRender: function () {
			...
			Ext.apply(this.renderData, {
				lineWithBlank: this.buildContent(
					this.filterHTML(this.part.get('input'))
				),
			});

			this.maybeRelayout = Ext.Function.createBuffered(
				this.updateLayout,
				10,
				this,
				[]
		...
		setupDropZones: function (dropzones) {
			...
			function isValid(data) {
				return (
					data.question === me.question.getId() &&
					(!data.part || me.ordinal.toFixed(0) === data.part)
				);
			}

			this.dropZones = dropzones.map(function (zone) {
				return new Ext.dd.DropZone(
					zone,
					Ext.apply({}, dropOnAnswer, common)
				);
			});
		...
		setFieldValue: function (dragSource, dropTarget) {
			...
			if (
				!input ||
				input.getAttribute('name') !== dropTarget.dataset.input
			) {
		...
		checkSubmissionState: function () {
			...
			this[allFilledIn ? 'enableSubmission' : 'disableSubmission']();
		},
			...
			if (value) {
				for (inputName in value) {
					if (value.hasOwnProperty(inputName)) {
						...
						dropTarget = Ext.getDom(
							this.el
								.select(
									'.dropzone[data-input="' + inputName + '"]'
								)
								.first()
						);
		...
		getWordBankItem: function (wid) {
			var shared = this.up('assessment-question').contentComponents,
				...
				sharedWordBank = shared.filter(function (i) {
					return i.is('assessment-components-wordbank');
				})[0],
				item;
		...
		getSolutionContent: function (part) {
			...
			Ext.each(part.get('solutions'), function (sol) {
				...
				Ext.each(x, function (s) {
						...
						if (s.hasOwnProperty(k)) {
							...
							if (!w || !w.dataset.word) {
								console.error(
									'Solution for ' +
										k +
										'references a bad word bank entry: ' +
										s[k]
								);
							}
					...
					out.push(tpl.apply([line.apply(v)]));
				});
			});

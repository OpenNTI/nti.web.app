...
module.exports = exports = Ext.define(
	'NextThought.app.assessment.Scoreboard',
	{
		...
		items: [
			...
			{ xtype: 'assessment-tally', flex: 1 },
			{
				...
				scale: 'large',
				handler: function (b) {
					b.up('assessment-scoreboard').resetBasedOnButtonClick();
				},
			},
		],
		...
		initComponent: function () {
			...
			this.addDocked({
				dock: 'top',
				xtype: 'assessment-scoreboard-header',
		...
		doReset: function () {
			...
			wait().then(this.realignAnnotations.bind(this));
		},
		...
		updateWithScore: function (correct, total) {
			...
			this.down('assessment-score').setValue(
				Math.floor((100 * correct) / total) || 0
			);
		},
		...
		setPriorResults: function (assessedQuestionSet) {
			...
			this.down('assessment-scoreboard-header').setPriorResults(
				sortedSets
			);
			...
			//if(){
			this.show();
			this.questionSet.fireEvent('graded', assessedQuestionSet[0], {
				orgin: this,
			});
			//}
		},
	...
	function () {
		this.borrow(QuizSubmission, [
			'resetBasedOnButtonClick',
			'maybeDoReset',
		]);
	}
);

	...
	'NextThought.app.assessment.AssignmentFeedback',
	{
		...
		items: [
			{
				...
				name: 'title',
				autoEl: {
					cn: [{ tag: 'h1', html: 'Feedback' }, { cls: 'message' }],
				},
				renderSelectors: {
					messageEl: '.message',
				},
			...
			this.mon(this.editor, {
				...
				'no-body-content': function (editor, el) {
					editor.markError(
						el,
						getString(
							'NextThought.view.assessment.AssignmentFeedback.empty-editor'
						)
					);
					return false;
				},
		...
		beforeDestroy() {
			...
			if (this.feedbackList && this.feedbackList.el) {
				this.feedbackList.el.dom.removeEventListener(
					'load',
					this.afterImageLoad,
					true
				);
		...
		addFeedback: function (editor) {
			var item = new UsersCourseAssignmentHistoryItemFeedback({
			...
			item.saveData({ url: feedback.get('href') })
				...
				.catch(function (err) {
					...
					if (err.code === 'MaxAttachmentsExceeded') {
						err.message +=
							' Max Number of files: ' + err.constraint;
					}
		...
		setHistory: function (history) {
			...
			var s =
					getString(
						'NextThought.view.assessment.AssignmentFeedback.visibility-message'
					) + ' {0}.',
			...
			header.messageEl.update(
				Ext.String.format(
					...
					isMe(history.get('Creator'))
						? getString(
								'NextThought.view.assessment.AssignmentFeedback.visibility-instr'
						  )
						: getString(
								'NextThought.view.assessment.AssignmentFeedback.visibility-student'
						  )
				)
			);

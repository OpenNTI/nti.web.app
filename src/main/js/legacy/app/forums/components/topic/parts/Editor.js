...
module.exports = exports = Ext.define(
	'NextThought.app.forums.components.topic.parts.Editor',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				...
				cn: { cls: 'editor active', html: '{super}' },
			},
		]),
		...
		warnBeforeDismissingEditor: function () {
			var msg = getString(
				'NextThought.view.forums.topic.parts.Editor.dismisswarn'
			);
			Ext.defer(function () {
				alert({ msg: msg });
			}, 1);
		...
		syncHeight: function () {
			...
			if (
				...
				(!this.syncHeightRetries || this.syncHeightRetries < 10)
			) {
			...
			if (this.syncHeightRetries) {
				console.debug(
					'#syncHeight() Retried ' + this.syncHeightRetries + ' times'
				);
				delete this.syncHeightRetries;
			}
			el.setHeight(
				Ext.Element.getViewportHeight() -
					(top + this.footerEl.getHeight() + 10)
			);
		...
		isEmpty() {
			...
			const re = /((&nbsp;)|(\u200B)|(<br\/?>)|(<\/?div>))*/g;
			const str = x =>
				(Array.isArray(x) ? x : [x]).join('').replace(re, '');
			return (
				Ext.isEmpty(title) &&
		...
		isValid: function () {
			...
			if (
				!Ext.isArray(v.body) ||
				v.body.join('').replace(re, '') === ''
			) {
				console.error('bad forum post');
				me.markError(
					me.contentEl,
					getString(
						'NextThought.view.forums.topic.parts.Editor.emptycontent'
					)
				);
				return false;
			}
			...
			if (Ext.isEmpty(v.title)) {
				...
				me.markError(
					me.titleWrapEl,
					getString(
						'NextThought.view.forums.topic.parts.Editor.emptytitle'
					)
				);
			...
			if (/^@{1,}/.test(v.title)) {
				...
				me.markError(
					me.titleWrapEl,
					getString(
						'NextThought.view.forums.topic.parts.Editor.attitle'
					)
				);
				me.titleWrapEl.addCls('error-on-bottom');
				return false;
		...
		onSave: function (e) {
			...
			me.ForumActions.saveTopic(
				...
				.then(function (record) {
					unmask();
					dispatch(Forums.FORUM_TOPIC_CHANGE, {
						forum: me.forum.getId(),
					});
		...
		onHandleSaveFailure: function (reason) {
			var error =
					(reason && JSON.parse(reason && reason.responseText)) || {},
				msg;

			console.error('Failed to save the discussion: ', reason);
			if (error.code === 'MaxFileSizeUploadLimitError') {
				let current = FilePicker.getHumanReadableFileSize(
						error.provided_bytes
					),
					expected = FilePicker.getHumanReadableFileSize(
						error.max_bytes
					);
				...
				msg = getString(
					'NextThought.view.forums.topic.parts.Editor.longtitle'
				);
				alert({
					title: getString(
						'NextThought.view.forums.topic.parts.Editor.error'
					),
					msg: msg,
					icon: 'warning-red',
				});

...
module.exports = exports = Ext.define(
	'NextThought.app.forums.components.topic.parts.Topic',
	{
		...
		pagingCommentsNavTpl: new Ext.XTemplate(
			Ext.DomHelper.markup([{ cls: 'paging-comments-nav' }])
		),
		...
		updateContent: function () {
			var h = this.record.get('headline');
			h.compileBodyContent(
				this.setContent,
				this,
				this.mapWhiteboardData,
		...
		commentAdded: function () {
			...
			if (this.currentPostCount === oldPostCount) {
				this.replyCountEl.update(
					Ext.util.Format.plural(this.currentPostCount + 1, 'Comment')
				);
			}
		},
		...
		destroyWarningMessage: function () {
			return getString(
				'NextThought.view.forums.topic.parts.Topic.destrowarning'
			);
		},
		...
		onDeletePost: function (e) {
			...
			Ext.Msg.show({
				...
				buttonText: {
					ok: getString(
						'NextThought.view.forums.topic.parts.Topic.deletebutton'
					),
				},
				title: getString(
					'NextThought.view.forums.topic.parts.Topic.deletetitle'
				),
				fn: function (str) {
					if (str === 'ok') {
						me.fireDeleteEvent();
		...
		buildCommentPagingNav: function (commentCmp) {
			...
			this.onceRendered.then(function () {
				me.pagingCommentsNav = Ext.get(
					me.pagingCommentsNavTpl.append(me.el.down('.foot'))
				);
				...
				me.mon(me.pager, {
					'mask-view': function () {
						var commentRect =
								commentCmp.el &&
								commentCmp.el.dom.getBoundingClientRect(),

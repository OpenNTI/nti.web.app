...
module.exports = exports = Ext.define(
	'NextThought.app.forums.components.forum.parts.TopicListView',
	{
		...
		emptyText: Ext.DomHelper.markup({
			cls: 'empty-forum',
			html: getString(
				'NextThought.view.forums.forum.parts.TopicListView.empty'
			),
		}),
		...
		initComponent: function () {
			...
			if (!me.record.getLink('add')) {
				me.emptyText = Ext.DomHelper.markup({
					cls: 'empty-forum',
					html: getString(
						'NextThought.view.forums.forum.parts.TopicListView.emptynoadd'
					),
				});
			}
		...
		groupers: {
			active: {
				...
				sorterFn: function (a, b) {
					a = new Date(a.get('NewestDescendantCreatedTime')).setHours(
						0,
						0,
						0
					);
					b = new Date(b.get('NewestDescendantCreatedTime')).setHours(
						0,
						0,
						0
					);
				...
				getGroupString: function (val) {
					var created = new Date(
						val.get('NewestDescendantCreatedTime')
					);
		...
		setGrouper: function (by) {
			...
			if (grouper) {
				...
				me.store.getGroups(false).forEach(function (g) {
					if (Ext.isEmpty(g.children) || g.name === 'Today') {
						return;
					}
		...
		restoreState: function (state) {
			...
			state.currentPage[this.record.getId()] =
				state.currentPage[this.record.getId()] || 1;

...
module.exports = exports = Ext.define(
	'NextThought.app.forums.components.forum.Navigation',
	{
		...
		async setCurrentBundle(bundle) {
			if (
				!this.currentBundle ||
				this.currentBundle.getID() !== bundle.getId()
			) {
				if (this.forumList) {
				...
				const baseroute = `/app/${
					bundle.isCourse ? 'course' : 'bundle'
				}/${encodeForURI(bundle.getId())}/discussions`;
		...
		getRouteFor(object) {
			if (
				object.MimeType.includes('application/vnd.nextthought.forums')
			) {

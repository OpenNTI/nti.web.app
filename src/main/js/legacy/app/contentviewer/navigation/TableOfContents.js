...
const CSSUtils = require('internal/legacy/util/CSS');

require('internal/legacy/model/TopicNode');
	...
	'NextThought.app.contentviewer.navigation.TableOfContents',
	{
						...
						cn: [
							{
								tag: 'input',
								placeholder:
									'{{{NextThought.view.Navigation.search}}}',
								required: true,
							},
		...
		tpl: new Ext.XTemplate(
			Ext.DomHelper.markup({
				...
				cn: [
					{
						cls: 'outline-row {type}',
						cn: [{ cls: 'label', html: '{label}' }],
					},
				],
			})
		),
			...
			if (!v) {
				...
				this.store.filter({
					...
					fn: function (r) {
						...
						if (this.matchingIds[r.get('NTIID')]) {
							return true;
						}

						var matches = r.matches(v);

						this.matchingIds = Ext.apply(
							this.matchingIds || {},
							matches
						);
		...
		scrollSelectionIntoView: function () {
			...
			if (el) {
				...
				if (offset < 0 || offset > height) {
					scroll.scrollTo(
						'top',
						scroll.getScrollTop() + offset - height / 2,
						true
					);
				}
			}

	...
	'NextThought.app.contentviewer.navigation.assignment.Admin',
	{
			...
			},
			{
				...
				cn: [
					'{creator:avatar}',
					{
						...
						cn: [
							{
								cls: 'title name {presence}',
								cn: { html: '{displayName}' },
							},
							{ cls: 'username', cn: { html: '({Username})' } },
							{
								cls: 'subtitle actions',
								cn: [
									{
										tag: 'span',
										cls: 'profile link',
		...
		beforeRender: function () {
			...
			var status = this.status || 'Open',
				Username =
					this.student.get('OU4x4') || this.student.get('Username');
			...
			this.renderData = Ext.apply(this.renderData || {}, {
				...
				totalPoints:
					this.assignment && this.assignment.get('total_points'),
			});
		...
		afterRender: function () {
			...
			if (
				...
				this.assignmentHistoryItemContainer instanceof Promise
			) {
				this.assignmentHistoryItemContainer.then(
					this.setupHistoryItemContainer.bind(this)
				);
			} else {
				this.setupHistoryItemContainer(
					this.assignmentHistoryItemContainer
				);
			}
			...
			return {
				...
				doEdit: () => {
					if (this.handleEdit) {
						...
						this.doNavigation('', `${routePart}/edit`, {
							assignment: this.assignment,
						});
					}
				},
		...
		setupHistoryItemContainer(historyItemContainer) {
			if (!this.assignmentHistoryItemContainer && !historyItemContainer) {
				return;
			}
			if (!this.rendered) {
				return;
			...
			this.mon(
				this.assignmentHistoryItemContainer,
				'reset-assignment',
				this.markAssignmentAsReset.bind(this)
			);

			if (
				AssignmentStatus.hasActions(this.assignmentHistoryItemContainer)
			) {
			...
			this.assignmentHistoryItemContainer
				...
				.then(container => {
					const attempts = container.Items.filter(
						item => !!item.MetadataAttemptItem
					...
					this.attemptSwitcherComponent = Ext.widget({
						...
						getRouteFor: attempt => {
							...
							return () =>
								this.selectAttemptById(attempt.getID());
						},
					});
		...
		setActiveHistoryItem(historyItem) {
			...
			historyItem.getInterfaceInstance().then(active => {
		...
		setUpGradeBox: function (historyItem) {
			...
			var grade = historyItem && historyItem.get('Grade'),
				...
				maxTime =
					this.assignment.isTimed && this.assignment.getMaxTime(),
			...
			if (status.completed) {
				if (isNoSubmitAssignment) {
					...
					if (status.completed.date) {
						this.completedEl.dom.setAttribute(
							'data-qtip',
					...
					this.completedEl.update(
						TimeUtils.getNaturalDuration(
				...
				if (status.maxTime) {
					if (status.overtime) {
						...
						this.timedEl.dom.setAttribute(
							'data-qtip',
							status.overtime.qtip
						);
			...
			if (historyItem) {
				this.mon(
					historyItem,
					'excused-changed',
					this.excuseGradeStatusChanged.bind(this)
				);
				this.mon(
					historyItem,
					'reset-assignment',
		...
		showActionsMenu: function (e) {
			...
			var me = this,
				menu = AssignmentStatus.getActionsMenu(
					me.assignmentHistoryItemContainer
				);
		...
		changeGrade: function (number, letter) {
			...
			if (!historyItemContainer) {
				console.error(
					'No assignmentHistory set, cannot change the grade'
				);
				return;
			}
		...
		showGradeMenu: function () {
			if (this.letterEl.hasCls('disabled')) {
				return;
			}

			this.gradeMenu.showBy(
				this.letterEl,
				'tl-tl',
				this.gradeMenu.offset
			);
		},
		...
		openEmail: function (e) {
			var emailRecord = new Email(),
				mailLink =
					...
					this.studentEnrollment.getLink('Mail');

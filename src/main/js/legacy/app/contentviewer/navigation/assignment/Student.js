...
function getCurrentHistoryItem(container, assignment) {
	...
	if (!CurrentMetadataAttemptItem) {
		return historyItems[historyItems.length - 1];
	}

	return historyItems.find(item => {
		return (
			item.MetadataAttemptItem.NTIID === CurrentMetadataAttemptItem.NTIID
		);
	});
}
	...
	'NextThought.app.contentviewer.navigation.assignment.Student',
	{
			...
			if (this.assignmentHistory) {
				if (this.assignmentHistory instanceof Promise) {
					this.assignmentHistory
						.then(this.applyHistoryContainer.bind(this))
						.catch(
							e => e && console.error(e.stack || e.message || e)
						);
				} else {
		...
		afterRender: function () {
			...
			if (panel) {
				...
				me.on(
					'destroy',
					me.timeContainerEl.destroy.bind(me.timeContainerEl)
				);
			}
		...
		showRemainingTime: function (time, max, getSubmitFn) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					this.showRemainingTime.bind(this, time, max, getSubmitFn)
				);
				return;
			}
			...
			if (time < 0) {
				wait().then(this.showOverdueTime.bind(this, -1 * time, max));
			} else {
				wait().then(
					this.showDueTime.bind(this, time, max, getSubmitFn)
				);
			}
		...
		showOverdueTime: function (time) {
			...
			me.timer
				...
				.tick(function (t) {
					...
					me.timeMetaEl.dom.setAttribute(
						'data-qtip',
						TimeUtils.getNaturalDuration(t.time) + ' over'
					);
				});
			...
			wait(3034)
				.then(
					me.timeContainerEl.removeCls.bind(
						me.timeContainerEl,
						'recent'
					)
				)
		...
		showDueTime: function (time, max, getSubmitFn) {
			...
			me.timer
				...
				.tick(function (t) {
					...
					me.timeMetaEl.dom.setAttribute(
						'data-qtip',
						TimeUtils.getNaturalDuration(t.remaining)
					);
		...
		updateSubmitState: function (submitState) {
			...
			if (submitState.unanswered === 0) {
				...
				this.unansweredEl.update(
					Ext.util.Format.plural(submitState.unanswered, 'question') +
						' unanswered.'
				);
			}
		},
		...
		applyHistoryContainer(historyContainer) {
			...
			if (!this.rendered) {
				this.on(
					'afterrender',
					this.applyHistoryContainer.bind(this, historyContainer)
				);
				return;
			}
			...
			Promise.all([
				...
				.then(([container, assignment]) => {
					return [
						container,
						assignment,
						getCurrentHistoryItem(container, assignment),
					];
				})

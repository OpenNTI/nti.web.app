...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.overlay.Panel',
	{
		...
		ROOT_OBJECT_QUERY:
			'object[type*=naquestion]:not(object object),object[type*=napoll]:not(object object)',
		placementHolderTpl: Ext.DomHelper.createTemplate({
			type: 'application/vnd.nextthought.placeholder',
		}),
		...
		initComponent: function () {
			...
			if (!this.contentElement) {
				try {
					...
					const d = this.reader
						.getDocumentElement()
						.querySelectorAll(this.ROOT_OBJECT_QUERY);
			...
			if (this.contentElement && this.representsUserDataContainer) {
				Ext.fly(this.contentElement).set({
					'data-nti-container': true,
				});
			}
		...
		syncTopTillStable: function () {
			if (this.interval || !this.rendered) {
				return;
			}
		...
		maybeScrollIntoView: Ext.Function.createBuffered(function () {
			try {
				...
				if (
					reader.scrollToSelector &&
					this.is(reader.scrollToSelector)
				) {
					reader
						.getScroll()
						.toNode(this.contentElement, false, null, -offset);

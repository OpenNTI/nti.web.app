	...
	'NextThought.app.contentviewer.panels.Reader',
	{
		...
		onWindowResize: function () {
			...
			var r = this.body.down('reader-content'),
				...
				isInFullScreenMode =
					readerScroll &&
					readerScroll.isInFullScreenMode &&
					readerScroll.isInFullScreenMode();

			if (
				r &&
				r.scrollBeforeFullscreen !== undefined &&
				!isInFullScreenMode
			) {
		...
		getToolbarAndReaderConfig() {
			return [this.getToolbarConfig(), this.getReaderConfig()];
		},
		...
		applyReaderConfigs(config) {
			...
			this.flatPageStore =
				this.flatPageStore ||
				FlatPage.create({ storeId: 'FlatPage-' + this.id });
			...
			if (readerContent) {
				this.readerMons = this.mon(readerContent, {
					...
					'assignment-submitted': this.fireEvent.bind(
						this,
						'assignment-submitted'
					),
					'assessment-graded': this.fireEvent.bind(
						this,
						'assessment-graded'
					),
		...
		afterRender: function () {
			...
			if (this.pageInfo || this.pageInfoOverride) {
				this.setPageInfo(
					this.pageInfoOverride || this.pageInfo,
					this.bundle
				);
			} else {
				console.error(
					'No Page Info set on the reader. Everyone PANIC!!!!!!'
				);
			}
		...
		getToolbarConfig: function () {
			return {
				...
				doNavigation: (title, route, precache) => {
					return this.doNavigation(title, route, precache);
				},
			};
		},
		...
		setPageInfo: function (pageInfo, bundle) {
			...
			this.onceReadyForSearch().then(
				this.beginViewedAnalytics.bind(this)
			);
		},
		...
		beginViewedAnalytics: function () {
			...
			if (this.pageInfo.doNotSendAnalytics) {
				return;
			}
			...
			if (
				this.__lastAnalyticEvent &&
				this.__lastAnalyticEvent.resourceId === data.resourceId
			) {
				return;
			}
			...
			if (this.__lastAnalyticEvent) {
				console.warn(
					'Overwriting event %o with %o',
					this.___lastAnalyticEvent,
					data
				);
			}

...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.components.definition.Window',
	{
		...
		initComponent: function () {
			...
			if (p) {
				top =
					...
					p.bottom + this.getHeight() + nib;
				y = Math.round(
					top ? p.top - nib - this.getHeight() : p.bottom + nib
				);
		...
		unfixMask: function () {
			try {
				this.maskEl.removeCls('nti-clear');
			} catch (e) {
				swallow(e);
			}
		},
					...
					return me
						...
						.then(function (text) {
							return Promise.resolve()
								...
								.then(
									me._parse.bind(me, processor, text),
									me._ieParse.bind(me, processor, text)
								);
						})
						.then(function (o) {
							if (o.indexOf('&lt;/a&gt;') >= 0) {
		...
		queryDefinition: function () {
			...
			if (!u) {
				console.warn(
					'PageInfo ',
					this.pageInfo.raw,
					'did not contain a glocery rel link'
				);
				u = this.fallbackURL;
			}
		...
		_ieBuildProcessor: function (text) {
			try {
				...
				throw new Error(
					'Dictionary may not be installed, or not configured properly.' +
						(e.stack || e.message || e)
				);
			}
		},

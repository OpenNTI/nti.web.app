...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.components.editor.DiscussionEditor',
	{
		...
		async setupEditor() {
			...
			this.add({
				...
				extraData: {
					...
					ContainerId:
						rangeInfo?.container ||
						rangeInfo?.containerId ||
						page.getID(),
					style: lineInfo?.style || 'suppressed',
					selectedText: rangeInfo?.range
						? rangeInfo?.range.toString()
						: '',
				},
				afterSave: newNote => {
					this.UserData.onDiscussionNote(
						BaseModel.interfaceToModel(newNote)
					);
					this.afterSave?.();
				},
				onClose: async () => {
		...
		async getPage() {
			...
			if (!page.isMock) {
				return page.getInterfaceInstance();
			}
		...
		getApplicableRange() {
			...
			const doc = range
				...
				: null;
			const rangeDescription = Anchors.createRangeDescriptionFromRange(
				range,
				doc
			);

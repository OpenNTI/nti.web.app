...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.components.Reader',
	{
		...
		initComponent: function () {
			...
			//as well as this.
			this.getRangePositionAdjustments = this.forwardToModule(
				'annotations',
				'getRangePositionAdjustments'
			);
			this.rangesForSearchHits = this.forwardToModule(
				'annotations',
				'rangesForSearchHits'
			);
		...
		afterRender: function () {
			...
			this.splash = DH.doInsert(
				el,
				{ cls: 'no-content-splash initial' },
				true,
				'beforeEnd'
			);
			...
			items.add(
				(this.pageWidgets = Ext.widget({
					xtype: 'content-page-widgets',
					renderTo: this.el,
					reader: this,
				}))
			);
			...
			items.add(
				(this.notfound = Ext.widget({
					xtype: 'notfound',
					renderTo: this.splash,
					hideLibrary: true,
				}))
			);
		},
		...
		getScrollParent() {
			if (!this.doNotAssumeBodyScrollParent) {
				return Ext.getBody();
			}
		...
		calculateNecessaryAnnotationOffsets: function () {
			...
			if (
				!windowSizeStatics.hasOwnProperty('windowSize') ||
			...
			if (
				!scrollStatics.hasOwnProperty('lastScroll') ||
				!scrollStatics.top ||
			...
			return {
				...
				rect:
					this.el &&
					this.el.dom &&
					this.el.dom.getBoundingClientRect(),
		...
		setPageInfo: function (pageInfo, bundle, fragment, note) {
			if (!this.rendered) {
				return new Promise(fulfill => {
					const handle = () => {
						fulfill(
							this.setPageInfo(pageInfo, bundle, fragment, note)
						);
					};
			...
			if (!this.iframeReady) {
				return new Promise(fulfill => {
					...
					this.getIframe().on('iframe-ready', handle, {
						single: true,
					});
				});
			}
		...
		loadPageInfo: function (pageInfo) {
			...
			return new Promise(function (fulfill, reject) {
				function success(resp) {
					me.getContent().setContent(
						resp,
						pageInfo.get('AssessmentItems'),
						fulfill
					);
					me.splash.hide();
				}
				...
				function failure(r) {
					console.error(
						'Failed to load pageInfo content',
						r.status,
						r.responseText
					);
					me.splash.show();

...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.reader.Location',
	{
		...
		getRelated: function (givenNtiid) {
			...
			return ContentUtils.getNodes(ntiid, location.currentBundle).then(
				function (infos) {
					const info = infos[0];
					const related = info
						? info.location.getElementsByTagName('Related')
						: [];
						...
						function (r) {
							...
							do {
								...
								let tag = r.tagName,
									...
									target =
										tag === 'page'
											? ContentUtils.find(id)
											: null,
									locationInfo = target
										? target.location
										: null,
									label = locationInfo
										? locationInfo.getAttribute('label')
										: r.getAttribute('title'),
									href = (locationInfo || r).getAttribute(
										'href'
									);
								...
								if (!map[id]) {
									if (!info || !info.title) {
										console.warn(
											'skipping related item: ' +
												id +
												' because we could not resolve the ntiid ' +
												ntiid +
												' to a book'
										);
										return;

...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.reader.NoteOverlay',
	{
		...
		insertOverlay: function () {
			var me = this,
				...
				container = {
					...
					cn: [{ cls: 'note-here-control-box', onclick: 'void(0)' }],
				};

			me.container = container = Ext.DomHelper.insertAfter(
		...
		getTabPanel: function () {
			var targetEl = this.reader
					.getEl()
					.up('.x-container-reader.reader-container'),
				tabPanel;
		...
		openEditor(top, initialContent) {
			...
			this.editor = Ext.widget('reading-discussion-editor', {
				...
				location,
				lineInfo,
				rangeInfo: this.rangeForLineInfo(
					lineInfo,
					lineInfo?.style || 'suppressed'
				),
		...
		xopenEditor: function (top) {
			...
			var me = this,
				...
				readerRect = annotationOffsets.rect,
				left,
				currentBundle = location.currentBundle,
				targetEl = this.reader
					.getEl()
					.up('.x-container-reader.reader-container'),
			...
			function work(prefs) {
				...
				me.editor = Ext.widget('nti-editor', {
					...
					listeners: {
						...
						'no-title-content': function () {
							return true;
						}, //require title if notepad is a feature
						grew: function () {
							if (!annotationOffsets.isBodyScroll) {
								return;
							}
				...
				if (!annotationOffsets.isBodyScroll) {
					top +=
						annotationOffsets.scrollTop - annotationOffsets.viewTop;
					left -= annotationOffsets.left;
				}
		...
		saveNewNote: function (editor, r, v) {
			...
			if (DomUtils.isEmpty(note)) {
				me.editor.markError(
					...
					getString(
						'NextThought.view.content.reader.NoteOverlay.empty'
					)
				);
				return false;
			}
			...
			editor.el.mask(
				getString('NextThought.view.content.reader.NoteOverlay.saving')
			);
		...
		noteHere: function (range, rect, style, top, initialState) {
			this.positionInputBox(
				Ext.apply(this.lineInfoForRangeAndRect(range, rect), {
					style: style,
				})
			);

			return this.openEditor(top, initialState).catch(function () {
				alert(
					getString(
						'NextThought.view.content.reader.NoteOverlay.inprogress'
					)
				);
				return Promise.reject();
			});
		...
		lineInfoForRangeAndRect: function (range, rect, offsets) {
			return {
				range: range,
				rect: offsets
					? this.adjustContentRectForTop(rect, offsets.top)
					: rect,
			};
		},

		lineInfoForY: function (y) {
			var overlay = this.reader
					.getComponentOverlay()
					.overlayedPanelAtY(y),
				result = null,
				top;
			...
			if (overlay) {
				if (overlay.findLine) {
					...
					if (result && result.rect) {
						...
						result.rect = this.adjustContentRectForTop(
							result.rect,
							top
						);
					}
		...
		trackLineAtEvent: function (e) {
			...
			try {
				...
				if (
					...
					!lineInfo &&
					o.lastLine &&
					Math.abs(y - o.lastLine.rect.bottom) < 50
				) {
					lineInfo = o.lastLine;
					delete o.lastLine;
				}
				...
				if (lineInfo && (lineInfo !== o.lastLine || !o.lastLine)) {
					...
					if (
						e.target &&
						!e.target.matches('.video-controls-overlay')
					) {
						e.stopEvent();
					}
		...
		mouseOver: function (evt) {
			if (
				Ext.dd.DragDropManager.dragCurrent ||
				this.suspendMoveEvents ||
				this.reader.creatingAnnotation
			) {
				return false;
			}
		...
		rangeForLineInfo: function (line, style) {
			var range = line.range,
				maybeContainer =
					(range && range.commonAncestorContainer) || null,
			...
			container =
				!maybeContainer || maybeContainer.is(containerSelector)
					? maybeContainer
					: maybeContainer.up(containerSelector);

...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.reader.Assessment',
	{
		...
		uses: ['NextThought.app.contentviewer.reader.ComponentOverlay'],
		...
		makeAssessmentPoll: function (p, set, index) {
			...
			//See below
			p.getVideos = Ext.bind(DomUtils.getVideosFromDom, DomUtils, [
				contentElement,
			]);

			o.registerOverlayedPanel(
				p.getId(),
				Ext.widget('assessment-poll', {
					reader: this.reader,
					question: p,
					poll: p,
		...
		makeAssessmentQuestion: function (q, set, questionIndex) {
			...
			q.getVideos = Ext.bind(DomUtils.getVideosFromDom, DomUtils, [
				contentElement,
			]);

			o.registerOverlayedPanel(
				q.getId(),
				Ext.widget('assessment-question', {
					reader: this.reader,
					question: q,
					renderTo: o.componentOverlayEl,
					questionSet: set || null,
		...
		makeAssessmentSurvey: function (survey, guid) {
			...
			this.surveyHeader = o.registerOverlayedPanel(
				guid + 'submission',
				Ext.widget('assessent-survey-header', {
					reader: r,
					renderTo: c,
					survey: survey,
					tabIndexTracker: o.tabIndexer,
			...
			if (!historyLink && !survey.get('isClosed')) {
				this.submission = o.registerOverlayedPanel(
					guid + 'submission',
					Ext.widget('assessment-quiz-submission', {
						reader: r,
						renderTo: c,
						questionSet: survey,
						tabIndexTracker: o.tabIndexer,
				...
				Service.request(historyLink)
					...
					.then(function (history) {
						survey.fireEvent(
							'graded',
							me.getHistory(history).get('Submission')
						);
					});
			}
		},
		...
		getHistory(h) {
			return h && h.getMostRecentHistoryItem
				? h.getMostRecentHistoryItem()
				: h;
		},
		...
		makeAssessmentQuiz: function (set, guid) {
			...
			function getPendingAssessment(history) {
				...
				if (isInstructor) {
					temp =
						temp || AssessedQuestionSet.from(set, h.isPlaceholder);
				}
			...
			if (h) {
				...
				set.noMark = Boolean(
					pendingAssessment && pendingAssessment.noMark
				);
			}

			o.registerOverlayedPanel(
				guid + 'scoreboard',
				Ext.widget('assessment-scoreboard', {
					reader: r,
					renderTo: c,
					questionSet: set,
					tabIndexTracker: o.tabIndexer,
			...
			const isPracticeSubmission =
				this.injectedAssignment &&
				this.injectedAssignment.hasLink('PracticeSubmission');

			if (
				this.injectedAssignment &&
				this.injectedAssignment.isTimed &&
				!isPracticeSubmission &&
				!completed
			) {
				this.showAssignmentTimer(
					this.submission.shouldAllowSubmit &&
						this.submission.shouldAllowSubmit.bind(this.submission)
				);
			}
		...
		makeNumberedAssessmentQuestions: function (
			questions,
			set,
			questionIds
		) {
			...
			Ext.each(questions, function (q) {
				if (
					!questionIds ||
					(q.internalId && questionIds['' + q.internalId])
				) {
		...
		getPendingQuestionIds: function (pendingAssessment) {
			...
			let pendingQuestions =
				...
				pendingAssessment.data.questions;

			if (pendingQuestions) {
				pendingQuestions.forEach(function (question) {
					const id =
						question.get('questionId') || question.internalId;
					if (id) {
		...
		updateAssessmentHistory: function (history) {
			...
			if (historyItem) {
				...
				if (this.injectedAssignment.isTimed) {
					this.injectedAssignment.updateMetaData(
						historyItem.get('Metadata')
					);
				}
		...
		showAssignmentTimer: function (submitFn) {
			...
			me.injectedAssignment.getTimeRemaining().then(function (remaining) {
		...
		__goBackToAssignment: function (assignment) {
			...
			console.error(
				"__goBackToAssignment is no longer used, don't use it."
			);
		},
		...
		notSubmittedTimed: function () {
			...
			return assignment.getTimeRemaining().then(function (remaining) {
				if (remaining < 0) {
					...
					remaining =
						AssignmentStatus.getTimeString(time, true) +
						' Remaining';
				}
				...
				return new Promise(function (fulfill, reject) {
					Ext.Msg.show({
						...
						msg:
							title +
							' is a timed assignment, and is still in progress. Be sure to submit the assignment before time runs out.',
						buttons: {
							primary: {
								cls: 'caution',
								text: 'Leave',
		...
		progressLostAlert: function () {
			...
			let due = dueDate
				? 'It is due on ' + Ext.Date.format(dueDate, 'l, F j') + '.'
				: '';
			...
			return new Promise(function (fulfill, reject) {
				Ext.Msg.show({
					...
					msg:
						title +
						' has not been submitted for grading. ' +
						due +
						progress,
					buttons: {
						primary: {
		...
		showSavingProgress: function () {
			if (
				this.progressToast &&
				this.progressToast.el &&
				!this.progressToast.el.isDestroyed
			) {
		...
		showProgressSaved: function () {
			...
			toast.openLongEnough.then(function () {
		...
		shouldStudentAllowReset: function () {
			...
			if (this.isInstructorProspective) {
				...
				this.injectedAssignment &&
				!this.injectedAssignmentHistory
			) {
				allow = false;
		...
		resetAssignment: function () {
			var me = this,
				history =
					me.injectedAssignmentHistory.get('history') ||
					me.injectedAssignmentHistory,
				reset =
					history && history.resetAssignment
						? history.resetAssignment(isMe(history.get('Creator')))
						: Promise.reject();

			return reset.then(function (deleted) {
		...
		injectAssessments: function (reader, doc, items) {
			...
			new Ext.dom.CompositeElement(
				doc.querySelectorAll(
					'.x-btn-submit,[onclick^=NTISubmitAnswers]'
				)
			).remove();

			questionObjs = slice.call(
				doc.querySelectorAll('object[type*=naquestion][data-ntiid]')
			);

			//For polls just treat them exactly like we do questions
			pollObjs = slice.call(
				doc.querySelectorAll('object[type*=napoll][data-ntiid]')
			);

			Ext.Array.sort(items, function (ar, br) {
				var a = questionObjs.indexOf(
						me.getRelatedElement(ar.get('NTIID'), questionObjs)
					),
					b = questionObjs.indexOf(
						me.getRelatedElement(br.get('NTIID'), questionObjs)
					);
			...
			if (pollObjs.length) {
				Ext.Array.sort(items, function (ar, br) {
					var a = pollObjs.indexOf(
							me.getRelatedElement(ar.get('NTIID'), pollObjs)
						),
						b = pollObjs.indexOf(
							me.getRelatedElement(br.get('NTIID'), pollObjs)
						);
			...
			if (questionObjs.length) {
				Ext.each(
					this.cleanQuestionsThatAreInQuestionSets(
						items,
						questionObjs
					),
					function (q) {
						if (q.isSet) {
							me.makeAssessmentQuiz(q, guid);
						} else {
							me.makeAssessmentQuestion(q);
						}
					}
				);
			}
			...
			if (pollObjs.length) {
				Ext.each(
					this.cleanQuestionsThatAreInQuestionSets(items, pollObjs),
					function (p) {
						if (p.isSet) {
			...
			slice
				.call(
					doc.querySelectorAll(
						'object[type*=naquestion][data-ntiid]:not([data-used])'
					)
				)
				.forEach(function (e) {
			...
			if (me.injectedAssignment) {
				me.feedback = o.registerOverlayedPanel(
					guid + 'feedback',
					Ext.widget('assignment-feedback', {
						reader: r,
						renderTo: c,
		...
		cleanQuestionsThatAreInQuestionSets: function (items, objects) {
			...
			function pushSetQuestions(i) {
				if (i.isSet) {
					push.apply(questionsInSets, i.get('questions'));
				}
			}

...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.reader.Content',
	{
		...
		BODY_TEMPLATE: Ext.DomHelper.createTemplate({
			id: 'NTIContent',
			html: '{0}',
		}).compile(),
			...
			reader
				...
				.then(function (related) {
					...
					try {
						...
						try {
							position = Ext.fly(doc.body).query(
								'#NTIContent .page-contents'
							)[0];
							container = Ext.DomHelper.insertFirst(
								position,
								container
							);
						} catch (ffs) {
					...
					container = Ext.DomHelper.append(container, {
						tag: 'span',
						cls: 'related',
					});
					...
					if (!tpl) {
						tpl = Ext.DomHelper.createTemplate({
							...
							href: '{0}',
							onclick:
								"window.top.ReaderPanel.get('{3}').relatedItemHandler(this);return false;",
							cls: 'related c{2}',
							html: '{1}',
						}).compile();
					...
					if (last) {
						...
						if (c > 10) {
							Ext.DomHelper.append(container, {
								...
								cls: 'more',
								html: getString(
									'NextThought.view.content.reader.Content.showmore'
								),
							});
						}
				...
				.catch(function (e) {
					console.warn(
						'Could not insert related links due to: ',
						e.stack || e.message || e
					);
					return;
				});
		...
		pauseAllVideos: function () {
			...
			Ext.each(
				Ext.ComponentQuery.query('overlay-video-roll'),
				function (c) {
					c.pauseVideo();
				}
			);
		...
		resolveContainers: function () {
			...
			Ext.each(els, function (el) {
				var id =
					el.getAttribute('data-ntiid') || el.getAttribute('ntiid');
				//Depends on overlayed panels being constructed and setting the attribute...
				if (!el.hasAttribute('data-nti-container')) {
					return;
				}
				containers.push(id);
			});
		...
		setContent: function (resp, assessmentItems, finish) {
			...
			doc.getElementById('NTIContent').setAttribute(
				'data-page-ntiid',
				ntiid
			);
			...
			if (
				!pageInfo.isPartOfCourseNav() &&
				!pageInfo.get('DoNotLoadAnnotations')
			) {
		...
		parseHTML: function (request) {
			...
			function cssObj(m) {
				...
				for (let i = 0; i < m.length; i++) {
					const o = key.test(m[i])
						? basePath + key.exec(m[i])[1]
						: m[i];
					css[o] = {};
					if (!rc[o]) {
			...
			this.meta.title = (/<title[^>]*>(.*)<\/title>/gi.exec(head) ||
				[])[1];
			this.css = cssObj(
				head.match(/<link[^<>]*?href="([^"]*css)"[^<>]*>/gi) || []
			);
		...
		applyBodyStyles: function (bodyTag, basePath) {
			var styleMatches =
					bodyTag &&
					bodyTag[1] &&
					bodyTag[1].match(/style="([^"]+)"/i),
				bodyStyles = styleMatches && styleMatches[1],
				body = Ext.get(
					this.getDocumentElement().getElementById('NTIContent')
				),
				bodyStylesObj = {};
		...
		adjustNTIIDLinks: function (doc) {
			...
			Ext.Array.each(links, function (l) {
				var link = Ext.fly(l),
					ntiid = lazy.ParseUtils.parseNTIID(
				...
				if (ntiid) {
					...
					if (type && provider && typeSpecific) {
						externalLink = Ext.String.format(
							'/app/#!{0}/{1}/{2}',
							type,
							provider,
							typeSpecific
		...
		onClick: function (e, el) {
			...
			var m = this,
				...
				element =
					...
					doc.getElementsByName(target)[0] ||
					null,
				isExternalUri = ContentUtils.isExternalUri(r);

			if (p && p === 'popup') {
				Ext.widget('iframe-lightbox', {
					src: m.basePath.concatPath(
						el.getAttribute('data-source-wrapped')
					),
				}).show();
				return false;
			}
			...
			console.warn(
				'Unable to handle content link navigation for ',
				el,
				r
			);
			return undefined;

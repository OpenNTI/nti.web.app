...
module.exports = exports = Ext.define(
	'NextThought.app.contentviewer.reader.IFrame',
	{
		...
		displayPopover: function (href, html, node) {
			...
			me.popoverWidget = Ext.widget('simple-popover-widget', {
				reader: this.reader,
				text: html.innerHTML,
			});
		...
		initContentFrame: function () {
			...
			Ext.fly(doc.getElementsByTagName('body')[0]).addCls(
				this.getTopBodyStyles()
			);
			...
			on(doc, ['keypress', 'keydown', 'keyup'], function (e) {
				...
				if (k === e.BACKSPACE || e.getKey() === e.ESC) {
					...
					if (k === e.UP) {
						...
						me.fireEvent(
							k === e.LEFT ? 'page-previous' : 'page-next'
						);
					}
				}
				return true;
			...
			on(doc, 'click', function (e) {
				...
				if (
					...
					!Element.matches(
						target,
						'.application-highlight,.application-highlight ' +
							target.tagName
					) &&
			...
			on(doc, 'mouseout', function (e) {
				var evt = Ext.EventObject.setEvent(e || window.event),
					target =
						evt.getTarget('a.footnote') ||
						evt.getTarget('a.ntiglossaryentry');
			...
			on(doc, 'mouseover', function (e) {
				...
				function getPopoverContent(href2) {
					...
					Ext.each(Ext.fly(clonedFn).query('a'), function (d) {
						let href3 = d.getAttribute
							? d.getAttribute('href')
							: '';
				...
				targetType = Ext.fly(target).hasCls('footnote')
					? 'footnote'
					: 'ntiglossaryentry';
				...
				if (!popContent) {
					console.log(
						'Error: Could not find popover content for id:' +
							href +
							' from target: ' +
							target
					);
			...
			me.syncInterval = setInterval(
				me.checkFrame,
				this.baseFrameCheckIntervalInMillis
			);
		},
					...
					if (Ext.Date.now() - this.lastSyncFrame > 1000) {
						...
						if (!this.reader.isDestroyed) {
							this.syncInterval = setInterval(
								this.checkFrame,
								this.baseFrameCheckIntervalInMillis *
									this.frameCheckRateChangeFactor
							);
						}
					}
				} catch (er) {
		...
		syncFrame: function (content) {
			...
			if (Ext.isIE) {
				//This still seems wonky in IE10.  Make it just check IE so we can scroll the content
				contentHeight = 150;
		...
		getDocumentElement: function () {
			var iframe,
				win,
				dom,
				doc = this.contentDocumentElement;
		...
		fixDocumentRealPages(div) {
			...
			const firstPageTag = div.querySelector(
				`realpagenumber[value="${firstPage}"]`
			);
		...
		update: function (html, metaData) {
			...
			Ext.each(div.querySelectorAll('realpagenumber'), page => {
				...
				object.setAttribute(
					'type',
					'application/vnd.nextthought.app.realpagenumber'
				);
			...
			if (body) {
				...
				console.error(
					'We attempted to set the content of the IFrame and there was no BODY element to write to!'
				);
			}
		...
		elementAt: function (x, y) {
			...
			if (x < 706) {
				// If it's in the main content area
			...
			if (pickedElement) {
				console.log(
					'picking: (' + x + ',' + y + '): ' + pickedElement.tagName
				);
			}

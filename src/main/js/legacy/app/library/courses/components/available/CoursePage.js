...
module.exports = exports = Ext.define(
	'NextThought.app.library.courses.components.available.CoursePage',
	{
		...
		afterRender: function () {
			...
			this.setPageHeight();
			this.bufferedScroll = Ext.Function.createBuffered(
				this.onScroll,
				50
			);
			this.mon(
		...
		setItems: function (
			...
			if (upcoming && upcoming.length) {
				const activeState =
					!this.code && (!activeTab || activeTab === 'Upcoming');
				this.tabbedComponents['upcoming'] = this.addCourses(
					upcoming,
					getString(
						'library.courses.components.available.CoursePage.upcoming'
					),
					null,
					{ category: 'upcoming', xtype: 'course-catalog-collection' }
				);
				this.addTab({
					label: 'Upcoming',
					category: 'upcoming',
			...
			if (current && current.length) {
				const activeState =
					(Ext.isEmpty(upcoming) && !activeTab) ||
					activeTab === 'Current';
				this.tabbedComponents['current'] = this.addCourses(
					current,
					getString(
						'library.courses.components.available.CoursePage.current'
					),
					null,
					{ category: 'current', xtype: 'course-catalog-collection' }
				);
				this.addTab({
					label: 'Current',
					category: 'current',
			...
			if (archived && archived.length) {
				...
				this.tabbedComponents['archived'] = this.addBinnedCourses(
					...
					getString(
						'library.courses.components.available.CoursePage.archived'
					),
					{ category: 'archived', xtype: 'course-catalog-collection' }
				);
				this.addTab({
					label: 'Archived',
					category: 'archived',
				...
				this.addTab({
					label: 'Archived',
					category: 'archived',
					active: Ext.isEmpty(current) && Ext.isEmpty(upcoming),
				});
			}
			...
			if (this.rendered) {
				const activeState =
					...
					(Ext.isEmpty(current) &&
						Ext.isEmpty(upcoming) &&
						Ext.isEmpty(archived)) ||
					!!this.code;
				this.redeemWidget = this.add({
					xtype: 'library-redemption',
					redeemLink: 'link',
					code: this.code,
					onSuccess: this.onRedeemSuccess.bind(this),
				});
		...
		onRedeemSuccess(results) {
			...
			this.LibraryActions.reload()
				.then(() => {
					return this.CourseActions.findCourseInstance(
						newCourse.get('NTIID')
					);
				})
				.then(course => {
					...
					if (!courseTitle || courseTitle === '') {
						courseTitle =
							...
							course.getCourseCatalogEntry().get('title');
					}
					...
					this.showMessage(
						getFormattedString(
							'NextThought.view.library.available.CourseWindow.InvitationAcceptance',
							{ courseTitle }
						),
						false
					);
			...
			wait().then(function () {
				...
				if (
					...
					me.archivedLoader().then(items => {
						...
						me.tabbedComponents['archived'] = me.addBinnedCourses(
							containerCmp,
							me.binCourses(items),
							getString(
								'library.courses.components.available.CoursePage.archived'
						...
						me.el.unmask();
						me.query('course-catalog-collection').forEach(function (
							cmp
						) {
							me.relayEvents(cmp, ['show-course-detail']);
						});
		...
		showWelcomeMessage: function () {
			var targetEl = this.ownerCt
					? this.ownerCt.getTargetEl()
					: this.getTargetEl(),
				courseStore = Ext.getStore('courseware.EnrolledCourses'),
				enrollmentStatus =
					courseStore && courseStore.getCount() > 0
						? 'completed'
						: '';

			if (this.ownerCt && this.ownerCt.updateLabelText) {
				this.ownerCt.updateLabelText(
					getString(
						'NextThought.view.library.available.CourseWindow.Welcome'
					)
				);
			}
			this.welcomeCard = Ext.get(
				this.welcomeCardTpl.append(targetEl, {
		...
		onActionComplete: function (actionName) {
			var me = this,
				el = me.welcomeCard && me.welcomeCard.down('.' + actionName),
				nextAction;

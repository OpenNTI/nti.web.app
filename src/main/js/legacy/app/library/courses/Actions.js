	...
	'NextThought.app.library.courses.Actions',
	{
		...
		constructor: function () {
			...
			if (window.Service) {
				...
				this.LoginStore.registerLoginAction(
					this.onLogin.bind(this),
					'load-all-courses-link'
				);
			}
		},

		onLogin() {
			this.LoginStore.getService().then(service => {
				return this.setUpAllCourses(
					(service.getCollection('AllCourses', 'Courses') || {}).href
				);
			});
		},
		...
		loadAllUpcomingCourses() {
			return this.__loadCoursesOfType('AllCourses', 'Upcoming').then(
				items => {
		...
		loadAllCurrentCourses() {
			return this.__loadCoursesOfType('AllCourses', 'Current').then(
				items => {
					this.CourseStore.setAllCourses(items);
		...
		loadAllArchivedCourses() {
			return this.__loadCoursesOfType('AllCourses', 'Archived').then(
				items => {
					this.CourseStore.setAllCourses(items);
		...
		__loadCoursesOfType(courseLevel, courseType) {
			...
			return this.LoginStore.getService()
				.then(service => {
					...
					const courseCollection = service.getCollection(
						courseLevel,
						'Courses'
					);

					return this.__setupCourseByType(
						service.getLinkFrom(courseCollection.Links, courseType),
						courseLevel,
						courseType
					);
				})
		...
		__setupCourseByType(link, courseLevel, courseType) {
			...
			return StoreUtils.loadBatch(getURL(link))
				...
				.then(items => {
					return courseLevel === 'AdministeredCourses' ||
						courseLevel === 'EnrolledCourses'
						? this.__precacheItems(items)
						: items;
				})
				.then(items =>
					this.CourseStore.setCoursesByType(
						courseLevel,
						courseType,
						items
					)
		...
		loadFavorites() {
			...
			return this.LoginStore.getService()
				.then(service => {
					...
					const getFavoritesLink = collection =>
						collection &&
						service.getLinkFrom(collection.Links, 'Favorites');

					return Promise.all([
						this.setUpFavoriteAdminCourses(
							getFavoritesLink(adminCollection)
						),
						this.setUpFavoriteEnrolledCourses(
							getFavoritesLink(enrolledCollection)
						),
					]);
				})
				.then(() => {
		...
		__precacheItems: function (items) {
			...
			return Promise.all(precache).then(function (results) {
		...
		setUpFavoriteEnrolledCourses(link) {
			...
			return StoreUtils.loadBatch(getURL(link))
				...
				.then(items => this.__precacheItems(items))
				.then(items =>
					this.CourseStore.setFavoriteEnrolledCourses(items)
				);
		},
		...
		loadItems: function (collectionName, subName, type) {
			return this.LoginStore.getService().then(service => {
				...
				return StoreUtils.loadItems(
					getURL(service.getLinkFrom(courseCollection.Links, type))
				);
			});
		},

		loadItemsAndPrecache: function (collectionName, subName, type) {
			return this.loadItems(collectionName, subName, type).then(
				this.__precacheItems.bind(this)
			);
		},
		...
		loadAllCourses: function () {
			...
			return StoreUtils.loadItems(getURL(link)).then(
				this.CourseStore.setAllCourses.bind(this.CourseStore)
			);
		},

		findCourseInstance: function (id) {
			return this.CourseStore.onceLoaded().then(
				this.CourseStore.findCourseInstance.bind(this.CourseStore, id)
			);
		},

		findCourseByPriority: function (fn) {
			return this.CourseStore.onceLoaded().then(
				this.CourseStore.findCourseInstanceByPriority.bind(
					this.CourseStore,
					fn
				)
			);
		...
		findForNTIID: function (id) {
			return this.CourseStore.onceLoaded().then(
				this.CourseStore.findForNTIID.bind(this.CourseStore, id)
			);
		},
	}
);

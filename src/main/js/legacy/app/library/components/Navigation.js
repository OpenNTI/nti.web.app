	...
	'NextThought.app.library.components.Navigation',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				...
				cn: [
					{
						cls: 'label',
						html: getString('NextThought.view.Navigation.your'),
					},
					{
						cls: 'available',
						html: getString('NextThought.view.Navigation.find'),
					},
				],
			},
		]),
		...
		updateState: function (active, options) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					this.updateState.bind(this, active, options)
				);
				return;
			}
		...
		dropDownClick: function (e) {
			if (e.getTarget('.available')) {
				this.bodyView.showAvailable(
					this.activeItem.available.title,
					this.activeItem.available.route
				);

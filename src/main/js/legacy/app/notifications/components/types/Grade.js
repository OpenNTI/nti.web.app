...
module.exports = exports = Ext.define(
	'NextThought.app.notifications.components.types.Grade',
	{
		...
		addCourseData: async function () {
			...
			const asset = PresentationAssets.Asset.getPresentationAsset(
				catalogEntry,
				'thumb'
			);
			const img = Ext.DomHelper.markup({
		...
		fillInWording: function () {
			...
			return Service.getObject(assignmentId).then(function (assignment) {
				if (me.wordingEl && me.wordingEl.dom) {
					me.wordingEl.dom.innerHTML = t('label', {
						assignment: Ext.util.Format.htmlEncode(

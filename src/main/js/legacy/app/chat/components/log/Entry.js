...
module.exports = exports = Ext.define(
	'NextThought.app.chat.components.log.Entry',
	{
		...
		renderTpl: Ext.DomHelper.markup([
			{
				cls: 'log-entry-wrapper {me}',
				cn: [
					'{user:avatar}',
					{ tag: 'span', cls: 'control' },
					{
						cls: 'message-bounding-box',
						cn: [
							{
								cls: 'log-entry {me}',
								'data-qtip': '{timestamp:htmlEncode}',
								cn: [
									{ cls: 'name', html: '{name}' },
									' ',
		...
		update: function (m) {
			...
			me.renderData.timestamp = this.getTimeStamp(
				me.message.get('CreatedTime')
			);
		...
		click: function (event) {
			event.stopEvent();
			var me = this,
				t = event.getTarget('.whiteboard-container', null, true),
				a = event.getTarget('a');
		...
		getTimeStamp: function (date) {
			function isToday(t) {
				var today = new Date();
				return (
					today.getYear() === t.getYear() &&
					today.getMonth() === t.getMonth() &&
					today.getDate() === t.getDate()
				);
			}
			return isToday(date)
				? Ext.Date.format(date, 'g:i a')
				: Ext.Date.format(date, 'F j, Y, g:i a');
		},

		initializeDragZone: function (v) {
			v.dragZone = new Ext.dd.DragZone(v.getEl(), {
				getDragData: function () {
					var sourceEl = v.el.dom,
						d;
					if (sourceEl) {

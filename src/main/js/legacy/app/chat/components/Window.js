	...
	'NextThought.app.chat.components.Window',
	{
			...
			UserRepository.getUser(roomInfo.get('Occupants'), function (users) {
				Ext.each(users, function (u) {
					...
					if (presence?.isOnline()) {
						...
						if (!isMe(name)) {
							me.updateDisplayState(
								u,
								'unavailable',
								isGroupChat
							);
							if (logView.addStatusNotifcation) {
								logView.addStatusNotification(
									u.getName() + ' is unavailable'
								);
							}
						}
					}
				...
				if (
					...
					if (logView.addStatusNotification) {
						logView.addStatusNotification(
							getString(
								'NextThought.view.chat.Window.one-occupant'
							)
						);
					}
				} else if (
					me.onlineOccupants &&
					me.onlineOccupants.length <= 1
				) {
					chatView.disable();
					if (logView.addSatusNotification) {
						logView.addSatusNotification(
							getString(
								'NextThought.view.chat.Window.one-occupantb'
							)
						);
					}
				} else {
					if (Ext.isEmpty(me.query('chat-log-entry'))) {
						Ext.each(
							me.query('chat-notification-entry'),
							function (el) {
								el.destroy();
							}
						);
				...
				if (newOccupants.length > 1) {
					...
					Ext.each(whoLeft, function (aUser) {
						me.updateDisplayState(
							aUser,
							getString('NextThought.view.chat.Window.gone'),
							isGroupChat
						);
					});
				}
		...
		updateChatViews: function () {
			var me = this,
				...
				logView = me.down('chat-log-view'),
				chatView = me.down('chat-entry'),
				myPresence = me.ChatStore.getPresenceOf(
					$AppConfig.userObject.get('Username')
				);

			UserRepository.getUser(
				this.roomInfo.get('Occupants'),
				function (users) {
					Ext.each(users, function (u) {
						var name = u.getId(),
							presence = me.ChatStore.getPresenceOf(name);
		...
		presenceChanged: function (username, value) {
			...
			if (
				!entryView ||
				!logView ||
				!Ext.Array.contains(me.roomInfo.get('Occupants'), username)
			) {
				return;
			}
			...
			UserRepository.getUser(username, function (user) {
				...
				if (!value.isOnline()) {
					...
					logView.addStatusNotification(
						getFormattedString(
							'NextThought.view.chat.Window.user-unavailable',
							{ name: displayName }
						)
					);
					me.updateDisplayState(
						user,
						getString('NextThought.view.chat.Window.unavailable'),
						isGroup
					);
					...
					if (!Ext.Array.contains(me.onlineOccupants, username)) {
						...
						me.updateDisplayState(
							user,
							getString('NextThought.view.chat.Window.available'),
							isGroup
						);
						logView.clearChatStatusNotifications();
						logView.addStatusNotification(
							getFormattedString(
								'NextThought.view.chat.Window.user-available',
								{ name: displayName }
							)
						);
					}
		...
		addOrDropContact: function (menuItem) {
			...
			if (this.GroupStore.isContact(this.user)) {
				this.areYouSure(
					'The following action will remove this contact.'
				).then(function (str) {
					if (str === 'ok') {
						me.GroupActions.deleteContact(me.user).then(
							function () {
				...
				this.GroupActions.addContact(this.user).then(function () {
		...
		updateChatState: function (sender, state, room, isGroupChat) {
			...
			this.updateDisplayState(
				sender,
				Ext.Array.contains(this.activeStates, state) ? 'active' : state,
				isGroupChat
			);
		},
		...
		setTitleInfo: function (users) {
			...
			if (title.length === 1) {
				...
				title = getFormattedString(
					'NextThought.view.chat.Window.occupantcount',
					{ number: title.length }
				);
			}
		...
		setChatStatesMap: function () {
			this.chatUserStatesMap = {
				composing: getString(
					'NextThought.view.chat.Window.componsingstate'
				),
				inactive: getString(
					'NextThought.view.chat.Window.inactivestate'
				),

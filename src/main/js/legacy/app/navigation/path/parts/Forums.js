	...
	'NextThought.app.navigation.path.parts.Forums',
	{
		...
		getPathToForum: function (forum, getPathTo) {
			...
			return this.LibraryActions.findBundleByPriority(function (bundle) {
				...
				section =
					(section && section.get('href')) ||
					bundle.getLink('DiscussionBoard');
				parent =
					(parent && parent.get('href')) ||
					bundle.getLink('ParentDiscussionBoard');
		...
		getPathToTopic: function (topic, getPathTo) {
			return (
				Service.getObject(topic.get('ContainerId'))
		...
		getPathToTopicComment: function (comment, getPathTo) {
			return Service.getObject(comment.get('ContainerId'))
				...
				.catch(function (reason) {
					console.error(
						'Failed to get path to topic comment: ',
						reason
					);
					return Promise.reject();
				});

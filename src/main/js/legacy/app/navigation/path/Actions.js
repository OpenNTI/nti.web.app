...
module.exports = exports = Ext.define(
	'NextThought.app.navigation.path.Actions',
	{
		...
		__doRequestForNoLink: function (obj) {
			...
			return this.__doRequestForLink(link).then(function (path) {
				const lastItem = path && path.last();
				const lastItemId =
					lastItem && lastItem.get && lastItem.get('NTIID');
		...
		__doRequestForLink: function (link, bundle) {
			...
			const cache = Service.request(link).then(function (response) {
		...
		__doHandledRequest: function (obj, handler) {
			...
			cache.catch(
				this.PathStore.removeFromCache.bind(this.PathStore, id)
			);
		...
		getBreadCrumb: function (record, scope) {
			...
			const rootObject =
				scope ||
				me.ContextStore.getRootBundle() ||
				me.ContextStore.getRootProfile();

			//Get the path for the record
			return me
				.getPathToObject(record, rootObject)
				.then(function (path) {
					//if the first path item is the root bundle, take it off
					if (
						path &&
						rootObject &&
						path[0].getId() === rootObject.getId()
					) {
						path.shift();
					}
					...
					return Promise.all(
						path.map(async item => {
							const title = item.getBreadCrumbTitle
								? await item.getBreadCrumbTitle()
					...
					return Service.getObject(containerId)
						...
						.catch(function (reason) {
							...
							if (resp && resp.status === 403) {
								return me.__resolveForbiddenBreadCrumb(
									resp.responseText
								);
							}

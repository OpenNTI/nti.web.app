...
module.exports = exports = Ext.define(
	'NextThought.app.context.components.VideoContext',
	{
		...
		isInPageContext: function () {
			...
			return this.PathActions.getPathToObject(this.record).then(function (
				path
			) {
				var pageInfo, i;
		...
		afterRender: function () {
			...
			if (me.isInContext()) {
				...
				if (me.doNavigate && !isCourseContentModalOpen()) {
					me.isInPageContext().catch(function () {
						me.seeMoreEl.removeCls('hidden');
						me.mon(
							me.seeMoreEl,
							'click',
							me.doNavigate.bind(me, me.record)
						);
					});
				}
			}

...
module.exports = exports = Ext.define(
	'NextThought.app.content.components.Navigation',
	{
		...
		updateEls: function (data, catalogEntry) {
			this.titleEl.update(
				(catalogEntry && catalogEntry.Title) || data.title
			);
			...
			if (isPreview) {
				...
				if (catalogEntry.getStartDate) {
					data.preview +=
						getString(
							'NextThought.view.content.Navigation.startsOn'
						) +
						Ext.Date.format(catalogEntry.getStartDate(), 'l, F j');
				}
				this.previewTagTpl.append(this.labelEl, {
		...
		onBodyClick: function (e) {
			if (
				!e.getTarget('.content-switcher') &&
				this.ContentSwitcher.isVisible()
			) {
				this.ContentSwitcher.hide();
			}
		},

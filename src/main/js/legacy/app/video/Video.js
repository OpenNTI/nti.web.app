...
module.exports = exports = Ext.define(
	'NextThought.app.video.Video',
	{
		...
		playerHeight: function () {
			return (
				Math.round(this.playerWidth * this.ASPECT_RATIO) +
		...
		onClassExtended: function (cls, data, hooks) {
			...
			data.listeners = Ext.applyIf(
				data.listeners || {},
				cls.superclass.listeners
			);
			if (data.cls) {
			...
			hooks.onBeforeCreated = function (clazz, classData) {
				...
				classData.listeners = Ext.applyIf(
					classData.listeners || {},
					clazz.superclass.listeners
				);
		...
		constructor: function (config) {
			if (!Ext.isEmpty(config.url)) {
				config.playlist =
					config.playlist || this.self.urlToPlaylist(config.url);
				delete config.url;
			}

			this.playerWidth =
				config.width || config.playerWidth || this.playerWidth;
			Ext.apply(config, {
		...
		afterRender: function () {
			...
			if (this.loadFirstEntry) {
				...
				this.maybeSwitchPlayers(
					item &&
						item.activeSource &&
						(item.activeSource() || {}).service
				);
				if (item) {
					this.setVideoAndPosition(
						item.activeSource && (item.activeSource() || {}).source
					);
				}
			} else {
		...
		playerConfigOverrides: function (name) {
			return {};
		},
		...
		issueCommand: function (
			...
			command,
			args,
			force /*Only to be used internally*/
		) {
		...
		deactivatePlayer: function () {
			if (this.activeVideoService) {
				try {
					this.log(
						'Clearing command queue for ',
						this.activeVideoService,
						' as part of deactivate'
					);
					...
					console.warn(
						'Error caught deactivating video',
						e.stack || e.message || e
					);
				}
				return true;
		...
		playBackSpeedChanged: function (oldSpeed, newSpeed) {
			...
			if (!state || this.doNotCaptureAnalytics || !id) {
				return;
			}
		...
		onHeartBeat: function () {
			...
			if (
				!this.hasWatchEvent &&
				(state.state === this.self.states.PLAYING ||
					state.state === this.self.states.BUFFERING)
			) {
		...
		openExternally: function () {
			var cls = this.activeClass(),
				...
				html = cls
					? cls.contentForExternalViewer.apply(
							cls,
							this.currentVideoId
					  )
					: '<body><h1>Unable to load video</h1></body>',
				w;

			w = window.open(
				'',
				'nti_external_video',
				'height=768,width=1050,toolbar=no,resizable=no,location=no,status=no,titlebar=no, directories=no'
			);
		...
		setVideoAndPosition: function (videoId, startAt) {
			...
			if (compareSources(this.currentVideoId, videoId)) {
				...
				this.issueCommand(this.activeVideoService, 'seek', [
					startAt,
					true,
				]);
			} else {
				...
				if (videoId) {
					this.issueCommand(this.activeVideoService, 'load', [
						videoId,
						startAt,
						'medium',
						playlistItem.getCaptions(),
					]);
				} else {
					this.log('stopping');
					this.stopPlayback();
		...
		maybeSwitchPlayers: function (service) {
			...
			if (!this.playerIds[service]) {
				console.warn(
					'Attempting to switch to non-existent player ' + service
				);
				service = 'none';
			}
			...
			Ext.Object.each(me.playerIds, function (k, id) {
				...
				if (!v) {
					console.warn(
						'Skipping ' + id + ' because dom does not contain it.'
					);
				} else {
		...
		playlistSeek: function (newIndex) {
			...
			if (newIndex >= 0 && newIndex < this.playlist.length) {
				if (
					...
					this.log(
						'Playlist seek setting active service to ',
						service
					);
					while (
						item &&
						Ext.Array.contains(this.playerBlacklist, service)
					) {
	...
	function () {
		window.swfobject = window.swfobject || {
			hasFlashPlayerVersion: function (v) {
				return false;
			},
		};

...
module.exports = exports = Ext.define(
	'NextThought.app.whiteboard.shapes.Base',
	{
		...
		draw: function (ctx, renderCallback) {
			...
			ctx.lineWidth =
				((parseFloat(this.strokeWidth) * w) / scale) *
				(scale < 0 ? -1 : 1);
		...
		getJSON: function () {
			...
			for (i in this) {
				if (this.hasOwnProperty(i)) {
					if (
						!Ext.isFunction(this[i]) &&
						!Ext.Array.contains(keys, i)
					) {
						data[i] = this[i];
						if (
							...
							data[i + 'RGBAColor'] = data[i].replace(
								colorRe,
								convertRGBA
							);
							delete data[i];
		...
		modify: function (nib, x1, y1, x2, y2, dx, dy) {
			var m = new NTMatrix(this.transform),
				map = {
					l: function () {
						return this.nibUpdate(m, x1, y1, dx, dy, -1, 0);
					},
					t: function () {
						return this.nibUpdate(m, x1, y1, dx, dy, 0, -1);
					},
					't-l': function () {
						return this.nibUpdate(m, x1, y1, dx, dy, 1, 1);
					},
					r: function () {
						return this.nibUpdate(m, x1, y1, dx, dy, 1, 0);
					},

	...
	'NextThought.app.whiteboard.editor.mixins.ShapeManipulation',
	{
		...
		onToolbarClick: function (e) {
			...
			function togglePressed() {
				var b = Ext.Array.filter(
					me.toolbar.query('[isEditAction]'),
					({ pressed }) => pressed
				);
				if (b.length > 0) {
					b[0].toggle();
			...
			if (e.getTarget('.move', undefined, true)) {
				...
				if (!this.canvas.hasCls('moveSelected')) {
					this.canvas.addCls('moveSelected');
				}
			} else {
				if (this.canvas.hasCls('moveSelected')) {
					this.canvas.removeCls('moveSelected');
				}
			}
		},
		...
		onFillColorChange: function (e) {
			...
			this.selected.fill =
				e.value !== 'NONE' ? Color.toRGBA('#' + e.value) : null;
		...
		onPathStrokeWidthChange: function (e) {
			...
			var pressed = this.toolbar
					.down('[pathSelectStrokeWidth]')
		...
		onCanvasMouseDown: function (e) {
			...
			this.clickedNib = s
				? s.isPointInNib(this.mouseInitialPoint)
				: false;
		...
		selectShape: function (e) {
			...
			Ext.each(
				...
				function (o) {
					if (!s && o.isPointInShape(sp[0], sp[1])) {
						s = o;
						o.selected = this.currentTool || true;
					} else {
						delete o.selected;
					}
				},
			...
			if (this.selected) {
				...
				const l = Ext.isString(sw)
					? this.selected.strokeWidth.replace('%', '')
					: sw;
		...
		doMove: function (e) {
			...
			if (m === true) {
				m = this.mouseDown = this.mouseInitialPoint.slice();
			}
		...
		doShape: function (e) {
			...
			if (
				!s ||
				s.Class !== 'Canvas' + Ext.String.capitalize(ttype) + 'Shape' ||
				!s.isNew
			) {
				this.deselectShape();
				this.currentTool = this.toolbar.getCurrentTool().forTool;
			...
			if (s.sides === 4) {
				distX =
					WBUtils.getDistance(p) *
					2 *
					Math.cos(WBUtils.toRadians(WBUtils.getDegrees(p)));
				distY =
					WBUtils.getDistance(p) *
					2 *
					Math.sin(WBUtils.toRadians(WBUtils.getDegrees(p)));
		...
		addShape: function (shape) {
			var opts = this.toolbar.getCurrentTool().getOptions(),
				...
				defs = {
					Class:
						'Canvas' +
						Ext.String.capitalize(shape.toLowerCase()) +
						'Shape',
					fill: opts.fill,
					stroke: opts.stroke,

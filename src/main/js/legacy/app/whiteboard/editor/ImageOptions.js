...
require('internal/legacy/common/menus/file/BrowserItem');
...
module.exports = exports = Ext.define(
	'NextThought.app.whiteboard.editor.ImageOptions',
	{
		...
		dropImage: function (e) {
			...
			if (!dt) {
				alert(
					'Please use the toolbar, your browser does not support drag & drop file uploads.'
				);
			} else {
		...
		insertImage: function (dataUrl) {
			...
			function addImageToWhiteboard() {
				...
				if (max === height && max > canvasHeight) {
					scale = (canvasHeight * 0.9) / max;
				} else if (max === width && max > canvasWidth) {
					scale = (canvasWidth * 0.9) / max;
				} else {
					scale = 1.0;
				}
			...
			function scaleImage(maxW, maxH) {
				var aspectRatio = 1.0,
					nw,
					nh,
					cs = Ext.DomHelper.append(Ext.getBody(), {
						tag: 'canvas',

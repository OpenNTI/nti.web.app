...
module.exports = exports = Ext.define(
	'NextThought.app.contacts.components.Grouping',
	{
		...
		initComponent: function () {
			...
			this.settingsTool.setDisabled(
				!(this.menu && this.menu.items && this.menu.items.length > 0)
			);
			...
			if (Ext.is.iPad) {
				Ext.getBody().on(
					...
					function (e) {
						if (
							this.menu.el &&
							!this.menu.el.isAncestor(e.target)
						) {
							if (
								this.settingsTool.el &&
								!this.settingsTool.el.isAncestor(e.target)
							) {
								this.menu.hide();
							}
						}
		...
		afterRender: function () {
			...
			if (this.record && this.record.isGroup) {
				...
				this.mon(
					this.nameEl,
					'click',
					this.navigateToProfile.bind(this, this.record)
				);
			}
		},
		...
		setTitle: function (newTitle) {
			if (!this.rendered) {
				this.on(
					'afterrender',
					Ext.bind(this.setTitle, this, [newTitle])
				);
				return;
			}
			...
			this.countEl.update(
				Ext.String.format('{0}', this.itemsList.length)
			);
		},
		...
		updateMore: function () {
			...
			if (c > 0) {
				this.moreEl = this.showMoreTpl.append(
					el || this.getTargetEl(),
					{ count: c },
					true
				);
		...
		updateList: function (dirtyIndex) {
			...
			if (dirtyIndex >= rendered && rendered > limit) {
				console.debug(
					'Nothing to do, item changed beyond rendered index'
				);
				return;
			}

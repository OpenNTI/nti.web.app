...
module.exports = exports = Ext.define(
	'NextThought.app.contacts.components.Card',
	{
			...
			cn: [
				...
				},
				{
					...
					cn: [
						...
						{
							cls: 'add-to-contacts',
							html: '{{{NextThought.view.contacts.Card.add}}}',
						},
						{
							tag: 'tpl',
							if: '!hideProfile && email',
							cn: [
								{
									cls: 'email',
									html: '{email}',
						...
						{
							tag: 'tpl',
							if: '!hideProfile && (role || affiliation)',
							cn: [
								{
									cls: 'composite-line',
									cn: [
				...
				{
					cls: 'nib',
					'data-qtip': '{{{NextThought.view.contacts.Card.options}}}',
				},
			],
		}),
		...
		initComponent: function () {
			...
			this.userObject.addObserverForField(
				this,
				'Presence',
		...
		beforeRender: function () {
			...
			this.renderData = Ext.apply(this.renderData, {
				hideProfile: $AppConfig.disableProfiles === true,
			});
		...
		afterRender: function () {
			...
			this.mon(
				this.userObject,
				'avatarChanged',
				this.setAvatar.bind(this, this.record)
			);
		...
		addToContactsClicked: function (e) {
			...
			Ext.each(
				...
				function (o) {
					if (
						o.record.getId() !== id ||
						me.userObject.modelName !== o.record.modelName
					) {
						o.destroy();
					} else {
						open = true;
			...
			pop = ManagementPopout.create({
				record: me.userObject,
				refEl: refEl,
				anchor: 'tl-tr?',
				offsets: offsets,
			});
		...
		isOnline: function (val) {
			return (
				(val ||
					(this.userObject.getPresence &&
						this.userObject.getPresence().toString())) === 'Online'
			);
		},

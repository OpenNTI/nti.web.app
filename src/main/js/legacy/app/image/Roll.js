...
module.exports = exports = Ext.define('NextThought.app.image.Roll', {
	...
	preload: function (data) {
		...
		function fin(img) {
			...
			if (img) {
				...
				console.debug(
					'Image Roll Preloaded Image: Width: ' +
						w +
						', Height: ' +
						h +
						', Aspect Ratio: ' +
			...
			if (data.length <= loaded) {
				console.debug(
					'Image Roll finished preloading, max aspect ratio:',
					maxAspect
				);
		...
		Ext.each(data, function (i) {
			...
			o.onerror = function () {
				fin();
				console.warn('Failed to load: ' + i.url);
			};
			o.src = i.url;
		});
	},
	...
	updateAspect: function (aspect) {
		...
		console.debug(
			'Image Roll: setting new height based on max aspect ratio: ',
			h
		);
